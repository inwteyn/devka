...
//    }
    var div_name = $('opacity') ? 'opacity' : 'activity-feed';
    ...
    Wall.request(url, data, function (response) {
        ...
        if ($$('.wall-list-button') && !pinfeed_module)  $('wall_more').set('html', en4.core.language.translate('More') + '<i class="hei hei-chevron-down"></i>');

    });
    var i = 1,
        update = $('hashtag-interval-check') ? update_options_hashtag : 120000;
...
function click_hashtags_menu(id, rev,self) {
    if ($('hash' + id).hasClass('box_for_hashtag display_none')) {
        $('hash' + id).removeClass('display_none');
        self.style.width = $('hash' + id).getSize().x - 16 + "px";
...
function click_hashtags_closes() {
    ...
    feed.loadFeed('', '', function(){feed.maxid=0;console.log(feed.maxid)}, 'hashtag', $('activity-feed'));
    feed.maxid=0;
  console.log(feed.maxid);
