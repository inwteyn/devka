...
class Hashtag_Api_Core extends Activity_Api_Core
{
    ...
    public function createHashtag($body, $action, $comment_id = 0)
    {
        $text = explode(' ', $body);
        ...
        $select = $mapsTable->select()
            ...
            ->where('hashtagger_type=?', $user->getType())
            ->where('hashtagger_id=?', $user->getIdentity());
        $row = $mapsTable->fetchRow($select);

        if ($row) {
            $hashTagTable->delete(array(
                'map_id = ?' => $row->getIdentity()
            ));
            $mapsTable->delete(array(
                'map_id = ?' => $row->getIdentity()
            ));
        }
