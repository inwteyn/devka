...
/*   $item = $event->getPayload();
   ...
   $activity =  Engine_Api::_()->getDbtable('actions', 'activity');
    $select = $activity-> select()->where('type = ?', $item->type)->where('action_id = ?', $item->getIdentity());
    $act = $activity->fetchAll($select);
    foreach($act as $code){

      $new = Engine_Api::_()->getDbTable('maps', 'hashtag');
      $select_b = $new->select('resource_id = ?', $code['action_id'])->where('resource_type = ?', $item->getType());
      ...
      $body =  'HASH-TAG ID Action = '.$code['action_id'].';  Resource Type  = '.$item->type;
      $log = new Zend_Log();
      $log->addWriter(new Zend_Log_Writer_Stream(APPLICATION_PATH . '/temporary/log/Hashtag_log_deleted.log'));
      $log->log($body . "\n\r\n\r", Zend_Log::INFO);
