...
Wall.Composer.Plugin.Checkin = new Class({
    ...
    attach : function() {
        ...
        this.$share_btn_clear = this.$checkin_cont.getElement('.checkinClear');
        ...
        this.$share_btn.addEvent('click', function() {
            ...
            if (!self.navigator_shared || $$('.checkinLocationInfo').hasClass('disabled')[0]) {
                console.log('ASD');
      ...
      this.$share_btn_clear.addEvent('click', function() {
            //self.$share_edit_info.setValue('');
            if (self.edit_mode) {
                return;
            }
            self.edit_mode = true;
    ...
    detach : function() {
      if (!this.isValidPosition()) {
        this.$share_btn_clear.setStyle('display', 'none');
        this.$share_info.set('html',en4.core.language.translate('CHECKIN_Where are you?'));
        this.$share_info.removeClass('display_none');
      }
      this.toggleCheckin(this.isValidPosition());
      return this;
    },
    ...
    enableLocation: function() {
      var self = this;
            ...
            navigator.geolocation.getCurrentPosition(function (position) {
                ...
                geocoder.geocode({'latLng':latlng}, function (results, status) {
                    if (status == google.maps.GeocoderStatus.OK) {
                        adde.name = results[0].formatted_address;
                        adde.accuracy = position.coords.accuracy;
                        adde.latitude = position.coords.latitude;
                        adde.longitude = position.coords.longitude;
                        var data = {
        ...
        /*
         var data = {
         'accuracy': adde.accuracy,
         'latitude': adde.latitude,
         'longitude': adde.longitude,
         'name': adde.name,
         'vicinity': adde.name
         };
        ...
        /*      self.watchID = navigator.geolocation.watchPosition(function(position) {
         ...
         if (status == google.maps.GeocoderStatus.OK) {
         if (results[1]) {
         adde =  results[0].formatted_address;
    ...
    GetNameFromLatLon: function(lat, lng) {
    ...
    setPosition: function(position) {
        ...
        this.$share_btn.addClass('checkinShareLocAct');
        this.checkStatus();
        this.suggest.setOptions({'postData':loc});
    },
    ...
    getLocationText: function() {
      if (this.position.name) {
            ...
            this.$share_btn_clear.setStyle('display', 'block');
        } else {
    ...
    getCurrentLocationCheckIn:function (callback) {
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function (position) {
    ...
    getLocationCheckIn: function() {
        this.getCurrentLocation(function (results) {
