...
<?php if ($this->contest) : ?>
    <?php if (!$this->ajax): ?>
        ...
        <script type="text/javascript" src="//s7.addthis.com/js/300/addthis_widget.js#pubid=ra-561259b19da59473" async="async"></script>
    ...
    <div class="active-contest-wrapper">
        <input type="hidden" id="hecontest-page" class="hecontest-page" value="<?php echo $this->nextPage; ?>">
        <?php if ($this->participants && $this->participants->getTotalItemCount()): ?>
            <ul id="hecontest-items" class="hecontest-items">
                <?php foreach ($this->participants as $participant): $t = $participant->getHref(); ?>
                    ...
                    <li>
                        <a class="hecontest-item hecontest-item-content"
                           href="<?php echo $this->url(array(), 'hecontest_general') . '#' . $participant->getIdentity(); ?>"
            ...
            <div style="clear: both"></div>
            <div style="display:none;" id="hecontest-load-more-loader" class="load-more-loader"></div>
        <?php else : ?>
            <div class="tip">
                <?php $href = '<a href="javascript://" onclick="hecontestCore.join(this, 1);">' . $this->translate('HECONTEST_join') . '</a>'; ?>
                <?php if ($this->contest->allowJoin()): ?>
                    <span><?php echo $this->translate("HECONTEST_No participants here", $href); ?></span>
                <?php else: ?>
...
<?php else : ?>
    <div class="tip">
        <span><?php echo $this->translate("HECONTEST_No active contest"); ?></span>
    </div>
<?php endif; ?>