...
class Hecontest_View_Helper_HecontestLikeButton extends Engine_View_Helper_HtmlImage
{
    protected $_page;

    public function hecontestLikeButton($item,$btn = '', $requireUser = true, $viewer_liked = null)
    {

        $hecoreTbl = Engine_Api::_()->getDbTable('modules', 'hecore');
        if(!$hecoreTbl->isModuleEnabled('like')) {
            return '';
        }
...
en4.core.runonce.add(function(){
  ...
  var options = {
    ...
    likeBtn: '_{$btn}',
    loader: 'like_loader_{$btn}_{$item->getGuid()}',
    menuId: 'like_menu_{$btn}_{$item->getGuid()}',
    suggestBtn: '#like_menu_{$btn}_{$item->getGuid()} .like_suggest',
    ...
    switcher: 'like_switcher_{$btn}_{$item->getGuid()}'
  };

  var btn = new HecontestLikeButton(options);
});
</script>
HTML;
        ...
        $wrapper .= ' '
            . '<div class="like_button_container hecontest_like_button_container">'
            . '<div id="like_loader_' .$btn.'_' . $item->getGuid() . '" class="hecontest_like_button_loader like_button_loader hidden"></div>'
            . '<a ' . $attributes . '  id="_' . $btn . '" ' . $onClick . ' onFocus="this.blur();" class="hecontest_like_button like_button_link ' . $action . '" href="javascript:void(0)">'
        ...
        if ($allowed) {
            $wrapper .= ''
                . '<a class="hecontest_like_switcher like_menu_switcher like_button_link" id="like_switcher_'  .$btn.'_' . $item->getGuid() . '" href="javascript:void(0)" onFocus="this.blur();"><span class="show_options">&nbsp;</span></a>';
        }
    ...
    /*public function hecontestLikeMenu($subject = null)
    {
        $viewer = Engine_Api::_()->user()->getViewer();
        if ($subject === null) {
