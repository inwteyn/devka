...
class Core_AdminContentController extends Core_Controller_Action_Admin
{
  ...
  public function widgetAction()
  {
    ...
    // TODO Michael's modifications
    $details_url = '<a href="' . $this->view->baseUrl() . '/admin/optimizer/">' . $this->view->translate('see here') . '</a>';
    $description = 'Transferring a widget to asynchronous loading increases speed of displaying of page to user. Warning: not all widgets support this method, more %s';
    $label = 'Load the widget by asynchronously with loading of page';

    $form->addElement('select', 'ajaxPostLoading', array(
      'label' => $label,
      'description' => $this->view->translate($description, array($details_url)),
      'multiOptions' => array(
        '' => 'Don\'t use Ajax',
        '1' => 'Use Ajax'
      )
    ...
    $form->getElement('ajaxPostLoading')->getDecorator('Label')->setOption('style' , 'font-weight:bold;padding-top:10px;display:block;border-top:#DDD 1px dotted;margin-top:20px;');
    $form->getElement('ajaxPostLoading')->getDecorator('Description')->setEscape(false);
