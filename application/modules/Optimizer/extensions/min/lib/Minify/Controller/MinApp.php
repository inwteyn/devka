...
/*            if ($files != array_unique($files)) {
          ...
          /**
           * Define whitelist js and css files which can be to connect
           * others files is not allowed
           */
          $whitelist = array();
          $allowed_files = array();
          @include './../../settings/whitelist.php';

          foreach ($whitelist as $item){
            // generate short key
            $item = '/' . trim($item, '/');
            $key = substr(md5($item), 0, 3) . '.js';
            $allowed_files[$key] = $item;
          }

          $baseUrl = str_replace('/application/modules/Optimizer/extensions/min', '', dirname($_SERVER['SCRIPT_FILENAME']));
          ...
          foreach ($files as $file){
            ...
            // TODO modification by Michael
            if (isset($allowed_files) && !empty($allowed_files) && !empty($allowed_files[$file])){
