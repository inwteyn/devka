...
class Rate_Api_Core extends Core_Api_Abstract
{
  ...
  public function getStoreCategories(){
    $tbl_store_product_meta = Engine_Api::_()->fields()->getTable('store_product', 'meta');
    $select = $tbl_store_product_meta->select()
        ->from($tbl_store_product_meta->info('name'), 'field_id')
        ->where('type = ?', 'profile_type', 'STRING')
        ->limit(1);
    $field_id = $tbl_store_product_meta->getAdapter()->fetchOne($select);

    $tbl_store_product_option = Engine_Api::_()->fields()->getTable('store_product', 'options');
    $select = $tbl_store_product_option->select()
        ->where('field_id = ?', $field_id);

    return $tbl_store_product_option->fetchAll($select);
}
  ...
  public function getProductTypes($prod_id)
  {
    $prod = Engine_Api::_()->getItem('store_product', $prod_id);
    if (!$prod) {
      return array();
    }
  ...
  public function isAllowRemoveProductReview($id, $viewer)
  {
    /*$item = Engine_Api::_()->getItem('store_product', $id);

    if ($item && $viewer->getIdentity()) {
      $is_permission = Engine_Api::_()->getApi('settings', 'core')
        ->getSetting('rate.reviewteamremove', 1);
