 ...
 * @license    http://www.hire-experts.com
 */
class Rate_Model_DbTable_Productreviews extends Engine_Db_Table
{
  protected $_rowClass = 'Rate_Model_Productreview';

  public function getPaginator($id, $user_id, $page = 1, $limit = 0)
  {
    ...
    $select = $this->select()
      ...
      ->joinLeft(array('v' => $tbl_vote->info('name')), 'v.review_id = r.productreview_id', 'AVG(v.rating) AS rating')
      ->where('r.product_id = ?', $id)
      ->group('r.productreview_id')
      //->order('is_owner DESC')
  ...
  public function isAllowedPost($id, $viewer)
  {
  ...
  public function getReviewsPaginator($params = array())
  {
      ...
      switch ($params['sort']) {
        case 'recent' :
          $select
            ...
            ->join(array('v' => $votesTbl->info('name')), 'v.review_id = pr.productreview_id', array('v.vote_id', 'v.review_id', 'v.rating', 'AVG(v.rating) AS avg_rating', 'COUNT(v.vote_id) AS count_rates'))
            ->join(array('p' => $pagesTbl->info('name')), 'p.product_id = v.product_id', array('p.displayname', 'p.url', 'p.photo_id'))
            ->group('pr.productreview_id')
            ->where("pr.creation_date IN (SELECT MAX(creation_date) FROM {$pagereviewsTblName} GROUP BY product_id)");
          break;
        case 'rated' :

          $firstQuery = $this->select()
            ->setIntegrityCheck(false)
            ->from(array('v1' => $votesTbl->info('name')), array('v1.review_id', 'v1.product_id', 'v1.rating', 'AVG(v1.rating)  AS  avg_rating', 'COUNT(v1.vote_id)  AS  count_rates'))
            ->group('v1.review_id')
            ->group('v1.product_id')
          ...
          $select
            ->from(array('v2' => $firstQuery), array('v2.*', 'COUNT(v2.product_id) as count_reviews'))
            ->join(array('pr' => $this->info('name')), 'pr.productreview_id = v2.review_id')
            ->join(array('p' => $pagesTbl->info('name')), 'p.product_id  =  pr.product_id', array('p.displayname', 'p.url', 'p.photo_id'))
            ->group('v2.product_id')
            ->order('avg_rating  DESC')
    ...
    if (isset($params['category']) && $params['category']) {
      $select->join(array('fv' => 'engine4_page_fields_values'), "fv.item_id=p.product_id AND fv.value={$params['category']}");
    }*/
