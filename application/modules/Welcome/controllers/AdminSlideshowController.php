...
class Welcome_AdminSlideshowController extends Core_Controller_Action_Admin
{
  ...
  public function createAction()
  {
   ...
   //Check Size SlideShow-----------------------------------
    if (!empty($animation)) {
      $errorSize = $this->checkSizeSlideShow($animation, $width = $form->getValue('width'), $form->getValue('height'));
      if (!$errorSize === false) {
        $form->addError($errorSize);
        return;
      }
    }
  //----------------------------------------------------------
...
// Check license
  ...
  public function deleteAction()
  {
   ...
   //Check Size SlideShow-----------------------------------
  private function checkSizeSlideShow($animation, $width, $height)
  {
    switch ($animation) {
      case 'slider':
        if ($width < 150 || $height < 150) {
          return "WELCOME_For Slider animtion minimum permission of the Slideshow: 150 X 150";
        }
        break;
      case 'tabs':
        if ($width < 200 || $height < 200) {
          return "WELCOME_For Tabs animtion minimum permission of the Slideshow: 200 X 200";
        }
        break;
      case 'popup':
        if ($width < 150 || $height < 150) {
          return "WELCOME_For Accordeon animtion minimum permission of the Slideshow: 150 X 150";
        }
        break;
      case 'curtain':
        if ($width < 100 || $height < 100) {
          return "WELCOME_For Curtain animtion minimum permission of the Slideshow: 100 X 100";
        }
        break;
