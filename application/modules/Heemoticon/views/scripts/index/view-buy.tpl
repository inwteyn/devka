<div id="main_content_smiles_view">
  ...
  <div class="main_contaner_smiles">
    <div class="tabs_smiles">
      ...
      <span class="return_to_all_collection"
            onclick="window.heemotion.addNewSmiles()"><?php echo $this->translate('Sticker Store'); ?></span>
    ...
    <div class="sticker-description">
      ...
      <div class="stickers-description-text">
        ...
        <?php if ($this->emoticons[0]['author']) { ?>
          <div class="heemoticon_author"
               style="margin: 5px"> <?php echo $this->translate('By') . ' - ' . $this->emoticons[0]['author']; ?></div><?php } ?>
        ...
        <?php if (in_array($this->viewer->level_id, $this->privacy) || in_array(0, $this->privacy)): ?>
          <?php if (!$this->purchased->getUsed($this->emoticons[0]['collection_id'], $this->viewer->getIdentity())): ?>
            ...
            <?php $notEnoughBalance = (int)$this->balances < (int)$this->price; ?>
            <?php if (!$this->buyed && $this->collectionTypeStatus) { ?>

              <button
                <?php if (!$notEnoughBalance) { ?>
                  onclick="window.heemotion.buyCollectionSticers(<?php echo $this->emoticons[0]['collection_id'] ?>, this)"
                <?php } ?>
                <?php echo $notEnoughBalance ? "disabled" : ""; ?> >
                <?php echo $this->buyButtonLabel; ?>
              </button>
            ...
            <span
              class="warning-not-enough-credits <?php echo $notEnoughBalance ? "display-block" : "display-none"; ?>">
              <?php echo $this->translate("Not enough credits"); ?>
              <?php echo $this->htmlLink(array('route' => 'credit_general', 'action' => 'index'), $this->translate('Buy')) ?>
          ...
          <?php else: ?>
            ...
            <button
              onclick="window.heemotion.removeCollectionSticers(<?php echo $this->emoticons[0]['collection_id'] ?>, this)"
          ...
          <?php if (in_array($this->viewer->level_id, $this->privacy) || in_array(0, $this->privacy)) {
            if (!$this->photoSmiles) {
              foreach ($this->emoticons as $k => $smile) {
                ?>
                <li style=" float: left">
                  <a title="<?php echo $smile['name'] ?>" href="javascript:void(0)" rev="<?php echo $smile['name'] ?>"
                     class="smiles_NEW">
