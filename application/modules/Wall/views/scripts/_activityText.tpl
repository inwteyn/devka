...
<script type="text/javascript">
  ...
  var feed = $$('.wallFeed').get('id');
  if (feed) {
    var vallfeed = Wall.feeds.items[feed];
    if (vallfeed) {
      if (vallfeed.compose.getPlugin('tag')) vallfeed.compose.getPlugin('tag').second_activete();
    }
  }
...
foreach ($actions as $action): // (goes to the end of the file)
  ...
  try { // prevents a bad feed item from destroying the entire page
    ...
    ?>
    <?php if (!$this->noList): ?>
      <li id="wall-action-item-<?php echo $action->action_id ?>" rev="item-<?php echo $action->action_id ?>" class="wall-action-item">
    <?php endif; ?>
    ...
    <?php if (isset($action->grouped_subjects) && count($action->grouped_subjects) > 1): ?>
      ...
      <div class='feed_item_body'>
        ...
        <span
          ...
          $other_num = count($action->grouped_subjects);
    ...
    <?php endif; ?>
      ...
      ?>
            ...
            <?php $cur_var = ($action->getAttachments() == null) ? array() : $action->getAttachments(); ?>
        <div
            class='feed_item_body feed_item_body_<?php $_item = is_object(current($cur_var)) ? current($cur_var)->item : null;
      echo $_item ? strtolower($_item->getModuleName()) . '_' . $_item->getType() : 'none' ?>'>
      ...
      if (!empty($people_tags)) {
        ...
        if ($object_count == 0) {
          ...
          foreach ($object_fetch as $item) {
            if ($object_fetch[0]->getTitle() == $item->getTitle()) {
              continue;
            }
          ...
          <?php if ($action->attachment_count > 0 && count($action->getAttachments()) > 0): ?>
            ...
            <?php else: ?>
              ...
              foreach ($action->getAttachments() as $attachment):
                if ($attachment->item->getType() == 'album_photo' && $counter == 4 && $action->attachment_count != 4) {
                  ?>
                  <span class="feed_attachment_album_photo">

                        <div class="feed_attachment_photo">
                          <a href="<?php echo $attachment->item->getHref()?>" class="feed_item_thumb wp_init" style="position: relative"
                             onclick="">
                            <img src="<?php echo $attachment->item->getPhotoUrl()?>" alt=""
                                 class="thumb_profile item_photo_album_photo">
                            <div style="    width: 100%;height: 100%;top: 0;left: 0;position: absolute;background-color: rgba(0,0,0,0.7);color: #fff;font-size: 40px;text-align: center;vertical-align: middle;padding-top: 40px;">
                              <?php echo ' + '.($action->attachment_count - 4) ?>
                            </div>
                          </a>
                        </div>
                ...
                if ($attachment->item->getType() == "storage_file") {
                  ?>
                  <a rev="heemoticon_<?php echo $attachment->item->getIdentity() ?>"
                     class="wall_liketips wp_init" href="<?php echo $attachment->item->map() ?>"><img
                      class="thumb_profile heemoticon item_nophoto " alt=""
                      src="<?php echo $attachment->item->map() ?>"></a>
                  <?php
                  continue;
                }
                ...
                <?php elseif ($attachment->meta->mode == 1): // Thumb/text/title type actions ?>
                  <div>
                    ...
                    if ($attachment->item->getType() == 'storage_file') {
                      $photo = $attachment->item->map();
                    } else {
                      $photo = $attachment->item->getPhotoUrl();
                    }
                    if ($photo):
                ...
                <?php
                elseif ($attachment->meta->mode == 2): // Thumb only type actions
                  ?>
      ...
      if ($moduleTable->isModuleEnabled('hashtag')) {
        ...
        $select = $hasgtags;
        $hashforthisAction = array();
        foreach ($select as $ker => $hash) {
          if ($hash['resource_id'] == $action->action_id) {
            $hashforthisAction[$ker] = $hash;
          }
        }


        $all = $hashforthisAction;
          ...
          <li class="link_hashtag_div">
            <a href="javascript:void(0)" id='link_hashtag'
              ...
              <?php if ($count > 1) { ?>
                ...
                onClick="javascript:click_hashtags_menu_clikc('<?php echo $id_hash ?>','<?php echo $action->action_id ?>');          click_hashtags('<?php echo $url ?>','<?php echo $name; ?>','<?php echo $this->translate("WALL_RECENT") ?>','','<?php echo $action->action_id ?>');" title="<?php echo $this->translate('TITLE_LINK_HASHTAG') ?>"
              <?php
              } else {
                ?>
                onClick="javascript:  click_hashtags('<?php echo $url ?>','<?php echo $name; ?>','<?php echo $this->translate("WALL_RECENT") ?>','','<?php echo $action->action_id ?>');" title="<?php echo $this->translate('TITLE_LINK_HASHTAG') ?>"
              <?php } ?>
              >#<?php echo $name; ?></a>
          </li>
        <?php
      ...
      <?php if (isset($checkins[$action->getIdentity()]) && ($checkin = $checkins[$action->getIdentity()])) { ?>
        ...
        <script type="text/javascript">
          var w = $$('.feed_item_body')[0].getSize().x;
          var h = parseInt($$('.feed_item_body')[0].getSize().x / 3);
          var smapEl = $('checkin_by_<?php echo $action->action_id ?>');
          if (!smapEl) {
            setTimeout(function () {
              $('checkin_by_<?php echo $action->action_id ?>').setAttribute('src', 'http://maps.googleapis.com/maps/api/staticmap?center=<?php echo urlencode($location) ?>&zoom=15&size=' + w + 'x' + h + '&markers=color:red|<?php echo urlencode($location) ?>&sensor=false');
            }, 1000);
          } else {
            smapEl.setAttribute('src', 'http://maps.googleapis.com/maps/api/staticmap?center=<?php echo urlencode($location) ?>&zoom=15&size=' + w + 'x' + h + '&markers=color:red|<?php echo urlencode($location) ?>&sensor=false');
          }
        </script>
