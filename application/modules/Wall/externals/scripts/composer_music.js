...
Wall.Composer.Plugin.Music = new Class({
  ...
  activate : function() {
    ...
    if( this.options.fancyUploadEnabled && this.options.fancyUploadOptions ) {
      ...
      var self = this;
      this.elements.select_soundcloud = new Element('div',{
        'class': 'select_soundcloud_link',
        'style': 'cursor: pointer;float: left;margin-left: 5px;position: relative;width: 280px;z-index: 10000;',
        'html': this._lang('or paste SoundCloud link above')+' <i style="display: inline-block;" class="wall-soundcloud-button_composer"></i>'
      }).inject(this.elements.body);
      this.elements.select_soundcloud.addEvent('click',function(){
...
'<div style="display:none;">\n\
  ...
  <div class="current-text"></div>\n\
      ...
      var opts = $merge({
        ...
        container : this.elements.formFancyContainer,
        // Events
        onLoad : function() {
        ...
        onFileSuccess : function(file, response) {
          var json = new Hash(JSON.decode(response.trim(), true) || {});
