...
if (!window.Wall) {
  ...
  document.addEventListener('mousemove', function (e) {
    Coordinates.x = e.clientX || e.pageX;
    Coordinates.y = e.clientY || e.pageY;
  });
...
// settings
  ...
  Wall.injectAbsolute_hash = function (element, container, direction) {
    ...
    var build = function () {
      ...
      try {
        ...
        if (direction) {
          container
            .setStyle('right', page_width - left - width - 11);
        } else {
          container
            .setStyle('left', left);
        }
  ...
  Wall.TipFx = new Class({
    ...
    createDom: function () {
      ...
      this.$top_arrow_container = new Element('div', {'class': 'arrow_container top_arrow_container'});
      this.$top_arrow = new Element('div', {'class': 'arrow top', 'style': 'display:none;'});
      this.$top_arrow.inject(this.$top_arrow_container);

      this.$inner.inject(this.$container);
      this.$top_arrow_container.inject(this.$inner, 'before');
      ...
      this.$container.addEvent('mouseover', con_mouseover);
      ...
      this.$container.addEvent('mouseout', con_mouseout);
      ...
      var loadContent = function () {
        ...
        if (element) {
          ...
          if (self.mouseActive) {
            ...
            var overflow_x = Coordinates.x + 422 - document.getSize().x,
              overflow_y = self.$container.getSize().y + 50 - Coordinates.y;
            bottom_arrow = self.$container.getElement('.arrow.bottom');
            top_arrow = self.$container.getElement('.arrow.top');
            if (overflow_x > 0) {
              self.$container.setStyle('left', Coordinates.x - 272);
              right = bottom_arrow.getStyle('left');
              bottom_arrow.setStyle('left');
              bottom_arrow.setStyle('right', right);
            }
            if (overflow_y > 0) {
              top_arrow.setStyle('right', bottom_arrow.getStyle('right'));
              top_arrow.setStyle('left', bottom_arrow.getStyle('left'));
              top_arrow.setStyle('display', 'block');
              bottom_arrow.setStyle('display', 'none');

              self.$container.setStyle('top', parseInt(self.$container.getStyle('top')) + self.$container.getSize().y + 50);

            } else {
              bottom_arrow.setStyle('display', 'block');
              top_arrow.setStyle('display', 'none');
            }
  ...
  Wall.Feed = new Class({
      ...
      if (Wall.elements) {
        ...
        if (tips) {
          ...
          tips.$inner.getElements('select').addEvent('change', function () {
            ...
            Wall.request(en4.core.baseUrl + 'wall/facebook/select-page', {
              'fbpage_id': value,
              'page_id': en4.core.subject.id
            });
    ...
    initStream: function () {
      ...
      wall_types_event = this.wall_types_event = $('wall_types');
      $link.addEvent('click', function () {
        self.injectAbsoluteMenu($link,$types);
        $(this).toggleClass('is_active');
        $types.toggleClass('is_active');
      });
      ...
      this.feed.getElements('.wall-stream-type-social').addEvent('click', function () {
        ...
        if ($$('.wall-stream-type')) $$('.wall-stream-type').removeClass('is_active')
        self.feed.getElements('.wall-stream-type').getParent().removeClass('active');
        if ($$('.wall-stream-type')) $$('.wall-stream-type').getParent().removeClass('active')
        if ($$('.wall-stream-type')) $$('.wall-stream-type').removeClass('active')
        $(this).addClass('is_active');
        $(this).getParent().addClass('active');
        self.feed.getElements('.wall-stream-option').removeClass('is_active');
        self.feed.getElement('.wall-stream-option-social').getParent().addClass('active');
        ...
        self.feed.getElements('.wall-stream-option-social').addClass('is_active');
        self.feed.getElements('.wall-stream-option-social').addClass('is_active');
    ...
    initPrivacy: function () {
      ...
      $link.addEvent('click', function () {
        ...
        self.injectAbsoluteMenu($link,$privacy);
        if ($(this).hasClass('is_active')) {
          $(this).removeClass('is_active');
    ...
    initActionPrivacy: function (action) {
      ...
      $$('body')[0].addEvent('click', function (e) {
        if (!$(e.target).getParent('.wall-privacy-container')) {
          $link.removeClass('is_active');
          if($privacy) $privacy.removeClass('is_active');
        }
      });
    ...
    initActionMenu: function (action) {
      ...
      $link.addEvent('click', function () {
        ...
        self.injectAbsoluteMenu($link,$menu);
      ...
      $$('body')[0].addEvent('click', function (e) {
        if (!$(e.target).getParent('.wall-menu-container')) {
          $link.removeClass('is_active');
          if($menu)$menu.removeClass('is_active');
        }
      });
    ...
    injectAbsoluteMenu: function(element, container) {
      element = $(element);
      container = $(container);
    ...
    prepareData: function () {
      ...
      var link = document.querySelector('li.utility-viewall > .pagination > a[rev^=next]'),
        data = {};
      if (link) {
        if ($(link).get('rev').indexOf('page') != -1) {
          data = $merge(this.params, {
            'page': $(link).get('rev').substr(5).toInt()
          });
        } else {
          data = $merge(this.params, {
            'maxid': $(link).get('rev').substr(5).toInt()
          });
        }
      }
      //console.log($(link).get('rev').substr(5).toInt());
      return data;
    },
    initAction: function ($container, params) {
      ...
      $container.getElements('.action-remove').addEvent('click', function () {
        if(window.action_remove == 1){
          return;
        }
        window.action_remove = 1;
        setTimeout(function(){
          window.action_remove = 0;
        },600);
      ...
      $container.getElements('.action-comment').addEvent('click', function () {
        if(window.action_comment == 1){
          return;
        }
        window.action_comment = 1;
        setTimeout(function(){
          window.action_comment = 0;
        },600);
        var element = Wall.getActionInfo(this);
      ...
      $container.getElements('.action-like').addEvent('click', function () {
        if(window.action_like == 1){
          return;
        }
        window.action_like = 1;
        setTimeout(function(){
          window.action_like = 0;
        },600);
        var element = Wall.getActionInfo(this);
        var data = {
          'action_id': element.action_id,
        ...
        Wall.request(en4.core.baseUrl + 'wall/index/like/subject/' + self.options.subject_guid, data, function (obj) {
          if (obj.status) {
            element.action.set('html', obj.body);
            self.initAction(element.action);
            if(self.compose && self.compose.getPlugin('tag'))  self.compose.getPlugin('tag').second_activete()
          }
        });
      ...
      $container.getElements('.action-unlike').addEvent('click', function () {
        if(window.action_unlike == 1){
          return;
        }
        window.action_unlike = 1;
        setTimeout(function(){
          window.action_unlike = 0;
        },600);
        var element = Wall.getActionInfo(this);
        ...
        Wall.request(en4.core.baseUrl + 'wall/index/unlike/subject/' + self.options.subject_guid, data, function (obj) {
          if (obj.status) {
            element.action.set('html', obj.body);
            self.initAction(element.action);
            if(self.compose && self.compose.getPlugin )  self.compose.getPlugin('tag').second_activete()
          }
        });
      ...
      $container.getElements('.wall-comment-form-reply textarea').each(function (item) {
        $(item).autogrow();
      });
      ...
      $container.getElements('.wall-comment-form').removeEvent('submit').addEvent('submit', function (e) {
        ...
        var comment_container = $('comment_attach_preview_image_wall'+action_element.action_id);
        comment_container.getChildren('a').removeClass('wp_init');
        if(comment_container.get('html').length>0) {
          comment_container.getChildren('a').removeClass('wp_init');
          if(comment_container.getChildren('#delete_'+action_element.action_id)[0]){
            var elem = comment_container.getChildren('#delete_'+action_element.action_id)[0];
            elem.parentNode.removeChild(elem);
          }
          var img = comment_container.get('html');
          if($('select_photo_'+action_element.action_id)){ $('select_photo_'+action_element.action_id).setStyle('display','block');}
        }
        else{
          var img = '';
        }
        values += '&comment_pagination=' + action_element.comment_pagination + '&checkin=' + element.checkin+'&img='+img;
       ...
       // Post Comment Reply
      $container.getElements('.wall-comment-form-reply').removeEvent('submit').addEvent('submit', function (e) {
        ...
        $(this).addClass('wall_loading');
        var newid = this.get('new-id');
        var element = Wall.getActionInfo(this);
        var action_element = Wall.getActionInfo(this);
        var values = $(this).toQueryString();
        var comment_container = $('comment_attach_preview_image_wall'+newid);
        comment_container.getChildren('a').removeClass('wp_init');
        if(comment_container.get('html').length>0) {
          comment_container.getChildren('a').removeClass('wp_init');
          if(comment_container.getChildren('#delete_'+newid)[0]){
            var elem = comment_container.getChildren('#delete_'+newid)[0];
            elem.parentNode.removeChild(elem);
          }
          var img = comment_container.get('html');
          if($('select_photo_'+action_element.action_id)){ $('select_photo_'+newid).setStyle('display','block');}
        }
        else{
          var img = '';
        }
      ...
      $container.getElements('.comment-like').addEvent('click', function () {
        if(window.comment_like == 1){
          return;
        }
        window.comment_like = 1;
        setTimeout(function(){
          window.comment_like = 0;
        },600);
      ...
      $container.getElements('.comment-unlike').addEvent('click', function () {
        if(window.comment_unlike == 1){
          return;
        }
        window.comment_unlike = 1;
        setTimeout(function(){
          window.comment_unlike = 0;
        },600);
        var element = Wall.getCommentInfo(this);
        var action_element = Wall.getActionInfo(this);
        var rev = $(this).get('rev');
      ...
      $container.getElements('.comment-reply').addEvent('click', function () {
        if(window.comment_reply == 1){
          return;
        }
        window.comment_reply = 1;
        setTimeout(function(){
          window.comment_reply = 0;
        },600);
        var id = this.get('rev');
        var form = $('comment-form-reply_'+id);
        if(form.getStyle('display') == 'none'){
          form.setStyle('display','block');
          form.getElement('form').body.focus();
        }else{
          form.setStyle('display','none');
        }
      });
      ...
      $container.getElements('.comment-edit').addEvent('click', function () {
        if(window.comment_edit == 1){
          return;
        }
        window.comment_edit = 1;
        setTimeout(function(){
          window.comment_edit = 0;
        },600);
        $$('.wall-comment-item').setStyle('opacity','1');
        var element = Wall.getCommentInfo(this);
        var action_element = Wall.getActionInfo(this);

        var post_id = action_element.action_id;
        var comment_id = element.comment_id;

        this.getParent('.wall-comment-item').setStyle('opacity', '0.4');

        if($$('.comment_body_' + comment_id).length){
          var preview_container = $('comment_attach_preview_image_wall' + post_id);
          var comment_form = $('activity-comment-form-' + post_id);
          var comment_clone = $$('.comment_body_' + comment_id)[0].clone();

          preview_container.set('html','');

          if(comment_form.getElement('#is_edit_comment')){
            comment_form.getElement('#is_edit_comment').remove();
          }

          if(comment_form.getElement('#cancel_' + post_id)){
            comment_form.getElement('#cancel_' + post_id).remove();
          }

          if(comment_clone.getElements('.view_more').length) {
            comment_clone = comment_clone.lastElementChild;
          }

          if(comment_form.getElement('#select_photo_' + post_id)){
            comment_form.getElement('#select_photo_' + post_id).show();
          }
          ...
          if(comment_clone.getElement('.comment_photo')){
            ...
            comment_form.getElement('#select_photo_' + post_id).hide();
          }
          ...
          if(comment_clone.getElement('.smiles_NEW')){
            ...
            delete_btn.addEvent('click', function(e){
              Heemoticons.deleteImage(used_id,sticker_id, post_id);
              comment_form.getElement('#select_photo_' + post_id).show();
            });
          ...
          var comment_text = comment_clone.innerHTML.trim().replace(/\s{2,}/g, ' ').replace(/\<br\>/g," ").replace('&nbsp;',' ');
          ...
          cancel_btn.addEvent('click', function () {
            ...
            preview_container.set('html','');
            comment_form.getElement('#is_edit_comment').remove();
            comment_form.getElement('button').hide();
            comment_form.getElement('textarea').value = '';
            this.remove();
          }).inject(comment_form);
      ...
      $container.getElements('.comment-remove').addEvent('click', function () {
        if(window.comment_remove == 1){
          return;
        }
        window.comment_remove = 1;
        setTimeout(function(){
          window.comment_remove = 0;
        },600);
        var element = Wall.getCommentInfo(this);
        var action_element = Wall.getActionInfo(this);
        var rev = $(this).get('rev');
        ...
        if ($$('.comment_body_' + element.comment_id)) {

          var link = $$('.comment_body_' + element.comment_id).getChildren('a');
        }else {
          var link = 0;
        }
        var photo_id = 0;
        if(link && link[0]){
          if(link[0].get('href')[0]){
            photo_id = link[0].get('href')[0].split('/').pop();
          }
        }
        var plink  = $$('.comment_body_' + element.comment_id).getChildren('a.smiles_NEW');
        var used_id = 0;
        if(plink[0] && plink[0].length){

          var urlImg = plink[0].getChildren('img')[0].get('src');
          var sticker_id = plink[0].getChildren('img')[0].get('sticker_id');
          var row = urlImg[0].split('?').pop();
      ...
      $container.getElements('.utility-viewall .pagination a').addEvent('click', function () {
        ...
        var _params = {'viewall': true, preInject: function () {}};

        var preLoad = document.querySelectorAll('.pre-load');
        if (preLoad.length>0) {
          for (var i = 0; i < preLoad.length; i++) {
            preLoad[i].removeClass('pre-load');
          }
          this.getParent().getParent().destroy();
        }

        var data =self.prepareData();
        if(data.maxid == self.maxid  && window.rewuest_wall != 1){
          window.rewuest_wall =1;

        }


        if(!!data.maxid && (data.maxid<self.maxid || !self.maxid)){
          self.maxid = data.maxid;

          self.loadFeed(data, 'bottom', function () {
            self.maxid = data.maxid;
            window.rewuest_wall =0;
          }, _params);
        }
    ...
    loadList: function (html) {
      ...
      this.$types.getElements('.item').addEvent('click', function () {
        ...
        if (rev.substr(0, 4) == 'type') {
          ...
          wall_types_event.getElements('a').removeClass('active');
          wall_types_event.getElements('li').removeClass('active');
          $(this).addClass('is_active');
          $('wall_menu_more').addClass('active');
          $$('.wall-list-button').addClass('active');

          var text_more = $(this).getElement('.wall_text').get('html') + '<i class="hei hei-chevron-down"></i>';
           ...
           */
          $('wall_more').set('html', '<span class="wall_loader" style="display: inline-block;  margin-top: -2px;"></span><i class="hei hei-chevron-down"></i>');
          $loader.addClass('wall_loading');
          self.loadFeed(self.params, null, function () {
            $loader.removeClass('wall_loading');
            $('wall_more').set('html', text_more);
          });
          ...
          /*$loader.getElement('.wall_icon').set('class', 'wall_icon').addClass('wall-type-list');
           $loader.getElement('.wall_text').set('html', $(this).getElement('.wall_text').get('html'));*/
          $$('.wall_loader').setStyle('display', 'block')
          $loader.addClass('wall_loading');
          self.loadFeed(self.params, null, function () {
            $loader.removeClass('wall_loading');
            $$('.wall_loader').setStyle('display', 'none')
          });
      ...
      this.wall_types_event.getElements('.item').addEvent('mousedown', function () {

        var rev = $(this).get('rev');

        if (rev.substr(0, 4) == 'type') {

          /*self.wall_types_event.getElements('a').removeClass('is_active');
           $(this).addClass('active');*/
          self.wall_types_event.getElements('li').removeClass('active');
          self.wall_types_event.getElements('a').removeClass('active');
          $(this).getParent().addClass('active');
          $(this).addClass('active');
          // $('wall_more').setStyle('color','#555555');
          $('wall_more').setStyle('color', null);
          $('wall_more').set('html', en4.core.language.translate('More') + '<i class="hei hei-chevron-down"></i>');
    ...
    loadFeed: function (options, where, oncomplete, params, element, effect_item) {
      ...
      var request_fn = function (html) {
          ...
          $items.each(function (item) {
            ...
            if (Wall.feedLoaded) {
              ...
              if(where == 'top'){
                ...
                item.className = item.className + ' pre-load';
              }
        ...
        self.checkEmptyFeed();
        if (!Wall.feedLoaded) {
          Wall.feedLoaded = true;
          var _data =self.prepareData();

          if(!!_data.maxid && (_data.maxid<self.maxid || !self.maxid)){
            self.maxid = _data.maxid;

            self.loadFeed(_data, 'bottom', function () {
  ...
  Wall.Stream.Abstract = new Class({
    ...
    init: function () {
      ...
      service.addEvent('change', function () {
        ...
        if (service.isEnabled()) {
          ...
          if (self.feed.feed.getElement('.wall-stream-' + self.getName()).isVisible()) {
            ...
            self.feed.feed.getElement('.wall-stream-option-' + self.getName()).addClass('active');
            if (self.getName() == 'twiter' || self.getName() == 'facefacebook' || self.getName() == 'linkedin') {
              self.feed.feed.getElement('.wall-stream-option-social').removeClass('active');
          ...
          if (self.feed.feed.getElement('.wall-stream-' + self.getName()).isVisible()) {
            ...
            self.feed.feed.getElement('.wall-stream-option-' + self.getName()).removeClass('is_active');
            if (self.getName() == 'twiter' || self.getName() == 'facebook' || self.getName() == 'linkedin') {
              self.feed.feed.getElement('.wall-stream-option-social').addClass('is_active');
      ...
      self.feed.feed.getElements('.wall-stream-type-' + self.getName()).addEvent('click', function () {
        ...
        self.feed.feed.getElements('.wall-stream-type').getParent().removeClass('active');
        $(this).addClass('is_active');
    ...
    onTabClick: function () {
      ...
      if (this.$tab.getElement('.wall_new_update_count')) this.$tab.getElement('.wall_new_update_count').set('html', 0);
      /* console.log(this.$tab);*/
      //this.$tab.hasClass('wall-stream-type-facebook')
    },
      ...
      Wall.request(en4.core.baseUrl + 'wall/' + this.getName() + '/checknew', {'since': this.options.since}, function (obj) {
        ...
        if (obj.ok) {
          ...
          if (obj.count && !self.$tab.hasClass('is_active')) {
            ...
            if (self.$tab.getElement('.wall_new_update_count')) self.$tab.getElement('.wall_new_update_count').set('html', obj.count);
          }
    ...
    initComposer: function () {
      ...
      container.getElement('form').addEvent('submit', function (e) {
        ...
        if (!$(this).body.get('value') || !$(this).body.get('value').trim().length) {
          return;
        }
  ...
  Wall.Composer = new Class({
    ...
    initialize: function (options) {
      ...
      this.getForm().getElement('.submitMenu button').addEvent('click', function (e) {
        ...
        if ($$('.compose-content')[0].textContent.trim() == '' && !this.pluginReady && !this.ready && (!this.getPlugin('people') || !this.getPlugin('people').peoples.length)) {
          return;
        }
        var button = container.getElement('.submitMenu button');
  ...
  Wall.Composer.Plugin.Interface = new Class({
    ...
    makeActivator: function () {
      {
        ...
        if(this.getName()=='photo'){
          ...
          t = ' hei-film';
        }else if(this.getName()=='mp3music'){
          t = ' hei-film';
        }
        if ($$('.wall-compose-' + this.getName() + '-activator').length == 0) {

          this.elements.activator = new Element('a', {
            'class': 'wall-compose-activator wall-compose-' + this.getName() + '-activator wall_blurlink hei hei-' + this.getName()+t,
    ...
    makeMenu: function () {
      ...
      if (!this.elements.menu) {
        ...
        if(this.options.title !='Add Photo') {
...
//            this.elements.menuTitle.appendText(')');
    ...
    makeLoading: function (action) {
      if (!this.elements.loading) {
        if (action == 'empty') {
          ...
          this.elements.body.getChildren().each(function (element) {
            element.setStyle('height', '0px').setStyle('visibility', 'hidden').setStyle('width', '0').setStyle('overflow', 'hidden')
          });
        }
  ...
  Wall.showReplyComments = function(comment_id, elem_this){
    elem_this.hide();
    $$('.reply_comment_id-' + comment_id).show();
    $('hide_reply_comments_button-' + comment_id).show();
  };

  Wall.hideReplyComments = function(comment_id, elem_this){
    elem_this.hide();
    $$('.reply_comment_id-' + comment_id).hide();
    $('show_reply_comments_button-' + comment_id).show();
  };
  ...
  function WallPreload(images) {
    ...
    Wall.EditAction = function (action_id) {
        var self = this;

        var action = $('wall-action-item-' + action_id);

        var to_edit = action.getElement('.feed_item_posted');

        if(to_edit.hasClass('in_editing')) {
            return;
        }

        to_edit.addClass('in_editing');

        var edit_box = new Element('div', {
            'class': 'edit-action-edit-box'
        }).inject(to_edit);

        var text_clone = to_edit.getChildren('.feed_item_bodytext').clone()[0];

        /* Search and replace hashtags */

        if(text_clone.getElements('.hashtag_link').length){
            text_clone.getElements('.hashtag_link').each(function (el) {
                var textnode = document.createTextNode(el.get('html'));

                text_clone.replaceChild(textnode, el);
            });
        }

        var text = text_clone.get('html');

        to_edit.getChildren('.feed_item_bodytext').hide();

        var textarea = new Element('textarea', {
            'id': 'edit-action-input-' + action_id,
            'class': 'edit-action-textarea'
        }).set('html', text).inject(edit_box);

        textarea.style.height = "1px";
        textarea.style.height = (textarea.scrollHeight - 10) + "px";

        textarea.addEvent('keyup', function (e) {
        ...
        var submit_button = new Element('button', {
            'class': 'edit-action-submit',
            'html': 'Edit Post'
        }).inject(edit_box);

        submit_button.addEvent('click', function () {

            if (action.getElement('#edit-action-input-' + action_id).value.trim() == '') {
                return;
            }

            var get_wall_feed_id = $$('.wallFeed').get('id');

            if(get_wall_feed_id.length > 1){
                get_wall_feed_id = get_wall_feed_id[0];
            }

            var feed = Wall.feeds.get(get_wall_feed_id);
            var element = Wall.getActionInfo(to_edit);

            var data = {
                'action_id': action_id,
                'content': action.getElement('#edit-action-input-' + action_id).value
            };

            Wall.request(en4.core.baseUrl + 'wall/index/edit/', data, function (obj) {
                if (obj.status) {
                    element.action.set('html', obj.body);
                    feed.initAction(element.action);
                }
            });
        });

        var cancel_btn = new Element('div', {
            'class': 'edit-action-cancel-btn',
            'html': en4.core.language.translate('Cancel')
        }).inject(edit_box);

        cancel_btn.addEvent('click', function () {
            to_edit.removeClass('in_editing');
            edit_box.destroy();
            to_edit.getChildren('.feed_item_bodytext').show();
        });
    };
