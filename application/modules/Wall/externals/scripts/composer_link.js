  ...
  Wall.Composer.Plugin.Link = new Class({
    ...
    poll: function () {
      ...
      if ($type(m) && $type(m[0]) && this.monitorLastMatch != m[0]) {
        if (window.preview_link == 1) return;
    ...
    doAttach: function () {
      ...
      if (window.preview_link == 1) return;
      window.preview_link = 1;

      if($$('.wall-compose-link-body') && window.pinfeed_page == 1 ) {
        $$('.wall-compose-link-body').set('html', 'loading...');
        setTimeout(function(){
          $$('.wall-compose-link-body').set('html', 'loading...');
        },500);
        setTimeout(function(){
    ...
    doShowPreview: function () {
      ...
      if (this.params.loadedImages.length > 0) {
        ...
        if (sc && arruri.length > 4) {


          this.elements.body.empty();
          this.elements.new = new Element('div', {
            'class': 'preview',
            'html': '<iframe width="100%" height="400" frameborder="no" src="https://w.soundcloud.com/player/?visual=true&amp;url=' + uri + '&amp;show_artwork=true" scrolling="no"></iframe>'
          }).inject(this.elements.body);
