...
Wall.Composer.Plugin.Video = new Class({
    ...
    activate: function (no_focus) {
        ...
        this.elements.container_video = new Element('div', {
            'class': 'wall-compose-video-container'
        }).inject(this.elements.body);

        this.elements.video_youtube = new Element('div', {
            'id': 'wall-compose-youtube-video',
            'html': '<i class="hei hei-youtube hei-2x"></i><span>'+en4.core.language.translate('YouTube')+'</span>',
            'events': {
                'click': function () {
                    self.updateVideoFields(1)
                }
            }
        }).inject(this.elements.container_video);

        this.elements.video_vimeo = new Element('div', {
            'id': 'wall-compose-vimeo-video',
            'html': '<i class="hei hei-vimeo-square hei-2x"></i><span>'+en4.core.language.translate('Vimeo')+'</span>',
            'events': {
                'click': function () {
                    self.updateVideoFields(2)
                }
            }
        }).inject(this.elements.container_video);

        this.elements.video_computer = new Element('div', {
            'id': 'wall-compose-comp-video',
            'html': '<i class="hei hei-desktop hei-2x"></i><span>'+en4.core.language.translate('My Computer')+'</span>',
            'events': {
                'click': function () {
                    self.updateVideoFields(3)
                }
            }
        }).inject(this.elements.container_video);
    ...
    doAttach: function (e) {
        ...
        var video_element = feed.container.getElement(".wall-compose-video-form-input");
        var type = video_element.get('vtype');
    ...
    updateVideoFields: function (element) {
        ...
        if (element == 1 || element == 2) {
            if(element == 1){
                this.elements.formInput.set('placeholder', en4.core.language.translate('YouTube'));
                this.elements.video_youtube.set('class','video-type-active');
                this.elements.video_vimeo.set('class','');
                url_element.set('vtype','1');
                this.elements.video_computer.set('class','');
            }else {
                this.elements.formInput.set('placeholder', en4.core.language.translate('Vimeo'));
                this.elements.video_youtube.set('class','');
                this.elements.video_vimeo.set('class','video-type-active');
                url_element.set('vtype','2');
                this.elements.video_computer.set('class','');
            }
        ...
        if (element == 3) {
            this.elements.video_youtube.set('class','');
            this.elements.video_vimeo.set('class','');
            this.elements.video_computer.set('class','video-type-active');
