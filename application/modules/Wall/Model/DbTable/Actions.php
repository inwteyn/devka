...
class Wall_Model_DbTable_Actions extends Activity_Model_DbTable_Actions
{
  ...
  public function getActivity(User_Model_User $user, array $params = array())
  {
    ...
    $album = false;
    $attachmentType = (empty($params['attachmentType'])) ? null : $params['attachmentType'];

    if (!Zend_Controller_Action_HelperBroker::getStaticHelper('requireAuth')->setAuthParams('album', $user, 'view')->checkRequire()) {
      $album = 1;
    }
    ...
    if ($album) {
      $select->where('`module` not like (?) and `type` not like (?) and `body` not like (?)', '%album%');
      $select->where('`type` not like (?) and `body` not like (?)', '%photo%');
      $select->where('`type` not like (?) and `body` not like (?)', '%tagged%');
      $select->where('`type` not like (?) and `body` not like (?)', '%shared%');
    }
    ...
    foreach ($responses as $response) {
      ...
      //--- Modification 21.07.2015 Mirlan
      if ($attachmentType) {
        $idsHiddenArray = $this->getHiddenActionIdPostSelf($attachmentType);
        if (is_array($idsHiddenArray)) {
          if (count($idsHiddenArray) != 0) {
            $select
              ->where('action_id NOT IN (?)', $idsHiddenArray);
          }
        }
      }
      //----------------------------------
                                   ...
                                   array $params = array())
  {
    ...
    foreach ($responses as $response) {
      ...
      //Removal is not appropriate event type post_self
      if (!empty($hideIds) && is_array($hideIds)) {
        $select->where('action_id NOT IN (?)', $hideIds);
      }
...
//Search of excess actions at the filter of video, a photo, music. Mod 21.07.2015. Mirlan
  private function getHiddenActionIdPostSelf($attachmentType)
  {
    $idsHiddenArray = array();
    ...
    $select = $attachmentTable->select()
      ->setIntegrityCheck(false)
      ->from(array('actions' => $actionsTable->info('name')))
      ->join(array('attachments' => $attachmentTable->info('name')), 'actions.action_id = attachments.action_id')
      ->where('actions.`type` = ?', 'post_self')
      ->where('attachments.`type` != ?', $attachmentType);

    $hideAction_ids = $attachmentTable->fetchAll($select)->toArray();
    if (count($hideAction_ids) != 0) {
      foreach ($hideAction_ids as $id) {
        array_push($idsHiddenArray, $id['action_id']);
      }
    }
