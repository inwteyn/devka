...
// Resize image (main)
  ...
  public function TagPeople($body,$viewer,$subject=null){
    $text = explode(' ',$body);
    ...
    foreach ($text as $str){
      ...
      if( $pos!== false && $pos==0){
        ...
        $userHtml = $this->getTagSuggestPeople($viewer,$item,$subject);
        if($userHtml) {
          $body = str_ireplace('@' . $item, $userHtml, $body);
        }
      }
    }
    return $body;
  ...
  public function getTagSuggestPeople(Core_Model_Item_Abstract $user, $username ,$subject )
  {
    ...
    $select_data = array();
      ...
      $select = $table->select()
        ->from(array('u' => $table->info('name')), new Zend_Db_Expr("'user' AS `type`, u.user_id AS id, u.username AS username,  u.username AS username, u.displayname AS displayname"))
        ->joinLeft(array('l' => $like->info('name')), "l.resource_type = 'user' AND l.resource_id = u.user_id AND l.poster_type = 'user' AND l.poster_id = $user_id", array())
        ->joinLeft(array('m' => $membership->info('name')), "m.resource_id = u.user_id AND m.user_id = $user_id AND m.active = 1", array())
        ->where(new Zend_Db_Expr('NOT ISNULL(l.resource_id) OR NOT ISNULL(m.resource_id) OR (u.user_id = '. $user_id .')'));
    ...
    if($row){
     $user_select = $row->toArray();
      $html  = '<a rev="'.$user_select['username'].'" class="tag_people" href="/profile/'.$user_select['id'].'">'.$user_select['displayname'].'</a>';
    }else{
      $html = false;
    }

      $settings = Engine_Api::_()->getApi('settings', 'core');
      if($settings->getSetting('hecomment.notification.enabled', 0)) {
          if($subject) {
              $subjectOwner = Engine_Api::_()->user()->getUser($user_select['id']);
              $notifyApi = Engine_Api::_()->getDbtable('notifications', 'activity');
              $notifyApi->addNotification($subjectOwner, $user, $subject, 'he_comment_tag', array(
  ...
  public function getCommentReply($action, $comment)
  {
    if ($comment) {
      ...
      if ($proxyTable instanceof Engine_ProxyObject) {
        ...
        if (get_class($table) == 'Core_Model_DbTable_Comments') {
          $select = $table->select()->where('resource_type = ?', 'comment')->where('resource_id=?', $comment->getIdentity());
        }

        if (get_class($table) == 'Activity_Model_DbTable_Comments') {
          $table = Engine_Api::_()->getDbTable('comments','wall');
          $select = $table->select()->where('poster_type = ?', 'comment')->where('resource_id=?', $comment->getIdentity());
        }
