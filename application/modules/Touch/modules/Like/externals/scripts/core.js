/* $Id: core.js 2011-04-14 16:02 idris $ */
  ...
  select_list: function(list, user_id, $link){
	...
	send_update: function($form){
		...
		new Request.JSON({
			...
			onSuccess: function(response){
				if (response.error == 0){
					$$('body')[0].innerHTML = response.message;
					setTimeout(function(){window.parent.Smoothbox.close()}, 1000);
				}else{
					$('send_update_errors').innerHTML = response.message;
					$('send_update_errors').removeClass('hidden');
					setTimeout(function(){$('send_update_errors').addClass('hidden')}, 1000);
				}
			}
		}).send();
