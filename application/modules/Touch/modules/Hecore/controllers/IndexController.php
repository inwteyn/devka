...
class Hecore_IndexController extends Touch_Controller_Action_Standard
{
  ...
  public function contactsAction()
  {
    ...
    if (is_array($this->view->items)) {
      $this->view->need_pagination = (isset($this->view->items['all']->getPages()->next) && $this->view->items['all']->getPages()->next)? 1:0;
    } else {
      $this->view->need_pagination = (isset($this->view->items->getPages()->next) && $this->view->items->getPages()->next)? 1:0;
    }

    if ( isset($this->view->params['scriptpath']) ) {
			$this->view->setScriptPath($this->view->params['scriptpath']);
    }
