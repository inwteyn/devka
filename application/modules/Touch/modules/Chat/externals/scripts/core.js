...
var ChatHandler = new Class({
  ...
  initialize : function(options) {
    sound = new Html5AudioEngine('html5_audio');
    if(sound.canPlayMP3())
      sound.setMediaSource(en4.core.staticBaseUrl + 'application/modules/Chat/externals/ding.mp3', true);
    else
      sound = null
    new Element('div', {
      'class': 'im_menu_wrapper_container'
      ...
      if( $type(responseJSON) == 'object' ) {
        ...
        if( $type(responseJSON.events) == 'object' ) {
          for( var x in responseJSON.events ) {
            ...
            if (type == 'chat' && responseJSON.events[x].sender_id != en4.user.viewer.id && 'object' == $type(sound)) {
              // ding!
              sound.play();
            }
...
var ChatHandler_Room = new Class({
  ...
  '
    ...
    window.scrollTo(0, this.elements.messagesWrapper.getScrollSize().y);
  },
...
var ChatHandler_Whispers = new Class({
  ...
  render : function() {
    ...
    if(Touch.isOperaMini()){
      this.elements.container.setStyle('position', 'relative');
    }
    // Settings
    ...
    this.items.friends = new ChatHandler_Whispers_UI_Friends(this, this.elements.container, {
      'name' : 'friends',
      'uid' : 'friends',
      'title' : en4.core.language.translate('Online'),
      'showClose' : false
    });
  ...
  resize : function() {
    if( this.elements.container.getCoordinates().left < 250 ) {
      this.elements.container.addClass('im_container_crunched touch_tab_group');
    } else {
...
var ChatHandler_Whispers_UI_Abstract = new Class({
  ...
  render : function() {
    ...
    this.elements.main = new Element('li', {
      'class' : 'touch_tab_dark touch_tab im_main im_main_' + name + ' im_main_inactive',
      'events' : {
    ...
    this.elements.main.addEvent('swipe', function(e){
      if(e.direction=='left' && this.getNext('.im_main_convo')){
          this.getNext().fireEvent('click');
      }
      if(e.direction=='right' && this.getPrevious('.im_main_convo')){
          this.getPrevious().fireEvent('click');
      }
    });
    // Menu
    this.elements.menu = new Element('div', {
      'class' : 'im_menu_wrapper im_menu_'  + name + '_wrapper im_main_menu_inactive from_left',
      'styles' : {
        // Can't position correctly if hidden
        //'display' : 'none'
      }
    }).inject(document.body.getElement('.im_menu_wrapper_container'));//*/this.elements.main);
    //this.elements.menu.setStyle('min-height', window.innerHeight-35);
    ...
    if( this.options.showHide ) {
      ...
      if( uid ) this.elements.menuHide.set('id', 'im_item_' + uid + '_hide');
        ...
        var html = '<img src="' + en4.core.staticBaseUrl + 'application/modules/Chat/externals/images/window_hide.png" />'
//       if(this.options.href){
//           href = this.options.href;
//           html = en4.core.language.translate('TOUCH_View_Profile');
//       }
    ...
    const self = this;
    this.elements.menu.addEvent('swipe', function(e){
      if(e.direction=='left' && self.elements.main.getNext('.im_main_convo')){
          self.elements.main.getNext().fireEvent('click');
      }
      if(e.direction=='right' && self.elements.main.getPrevious('.im_main_convo')){
          self.elements.main.getPrevious().fireEvent('click');
      }
    });
    // Item
  ...
  show : function(e) {
    ...
    if( !this.isVisible() ) {
      ...
      this.elements.menu.getAllPrevious('.im_menu_wrapper').removeClass('from_right').addClass('from_left');
      this.elements.menu.getAllNext('.im_menu_wrapper').removeClass('from_left').addClass('from_right');
      this.elements.menu.addClass('im_main_menu_active').removeClass('im_main_menu_inactive');
      $('global_bind').setStyle('display','none');
      if(this.elements.menu.hasClass('im_menu_friends_wrapper'))
        window.scrollTo(0,0);
      else{
        window.scrollTo(0,window.scrollTo(0,window.scrollMaxY));
      }
    }
  },
  ...
  hide : function(e) {
    ...
    if( this.isVisible() ) {
      ...
      $('global_bind').setStyle('display','block');
      this.elements.main.addClass('im_main_inactive').removeClass('im_main_active');
      this.elements.menu.addClass('im_main_menu_inactive').removeClass('im_main_menu_active')
...
var ChatHandler_Whispers_UI_Settings = new Class({
  ...
  render : function() {
    ...
    // Main
    this.elements.main = new Element('li', {
      'class' : 'touch_tab touch_tab_dark im_main im_main_' + name + ' im_main_settings_online'
    }).inject(this.container);
  ...
  toggleOnline : function(state) {
    ...
    if( state == 0 ) {
      $('im_container').setStyle('width', 'auto');
      ...
      $('global_bind').setStyle('display','block');
        $$('.im_menu_wrapper_container').setStyle('display','none');
      document.body.getElements('.im_menu_wrapper').addClass('hide_im_menu_wrapper');
      window.scrollTo(0,0);
    } else {
      $('im_container').setStyle('width', '100%');
      ...
      this.handler.items.each(function(item) {
        ...
        $('global_bind').setStyle('display','none');
        $$('.im_menu_wrapper_container').setStyle('display','block');
      document.body.getElements('.im_menu_wrapper').removeClass('hide_im_menu_wrapper');
      window.scrollTo(0,0);
...
var ChatHandler_Whispers_UI_Friends = new Class({
  ...
  Extends : ChatHandler_Whispers_UI_Abstract,
  unread_span: null,
  unread_notyfier_container: null,
  attach : function() {
    this.parent();
  ...
  render : function() {
    ...
    this.unread_notyfier_container = new Element('div', {'class': 'unread_notyfier_container no_unread'}).inject(this.elements.main);
    this.unread_span = new Element('span', {
      'class':'unread_notify_span'
    }).inject(this.unread_notyfier_container);
    new Element('span').inject(this.unread_notyfier_container);
  },
...
var ChatHandler_Whispers_UI_Conversation = new Class({
  ...
  render : function() {
    ...
    this.elements.menuInput.addEvents({
      ...
      'click': function(event){
        var raz = event.target.getWidth()-event['event'].layerX;
        if(raz<=30){
          this.send();
        }
      }.bind(this)
  ...
  focus : function() {
    this.parent();
    this.elements.menuInput.focus();
    //window.scrollTo(0, this.elements.menuBody.y);
  ...
  show : function(e) {
    ...
    if( this.flasher ) {
      ...
      document.body.getElement('.unread_notyfier_container').addClass('no_unread');
      $('im_user_'+this.options.uid.substr(5)).removeClass('unread');
    }
    ...
    if( $type(this.elements.menuBody) ) {
      window.scrollTo(0, this.elements.menuBody.getSize().y - window.innerHeight + 100);
    }
  },
  ...
  onChat : function(data, user, ref) {
    var self =this.handler.getSelf();
    var sender_pref = user.identity == self.identity?' sender ':'';
    this.handler.handler._log({'type' : 'ui.convo.chat', 'data' : data, 'user' : user});
    ...
    else
    {
      ...
      messageEl.set('class', sender_pref);
      (new Element('span', {'class' : 'im_convo_messages_author '+sender_pref, 'html' : '<img src="' + (user.photo!=null?user.photo:en4.core.staticBaseUrl + 'application/modules/User/externals/images/nophoto_user_thumb_icon.png') + '" alt="" />'}).inject(messageEl));
      (new Element('span', {'class' : 'im_convo_messages_arrow '+sender_pref, 'html' : ''}).inject(messageEl));
      (new Element('span', {'class' : 'im_convo_messages_body '+sender_pref, 'html' : body}).inject(messageEl));
    }
  ...
  checkFlasher : function() {
    if( !this.flasher && this.unread > 0 ) {
      ...
      var user_ur = $('im_user_'+this.options.uid.substr(5));
      this.flasher = (function() {
        var text = this.unread==1?this.unread + en4.core.language.translate('TOUCH_Unread_Message'):this.unread + en4.core.language.translate('TOUCH_Unread_Messages');
        document.body.getElement('.unread_notyfier_container').removeClass('no_unread');
        document.body.getElement('.unread_notyfier_container').getElement('span').set('html', '<b>'+this.options.title+' </b> <br/>'+ text);
        if( user_ur.hasClass('unread') ) {
          user_ur.removeClass('unread');
        } else {
          user_ur.addClass('unread');
        }
      }).periodical(500, this);
    }
  }
...
var ChatHandler_Utility = {
  ...
  imageSpec : 'application/modules/Touch/modules/Chat/externals/images/smilies/:name:.png',
  ...
  replaceSmilies : function(text) {
    this.smilies.each(function(name, val) {
      ...
      var image = '<img src="' + en4.core.baseUrl + this.imageSpec.replace(':name:', name).escapeQuotes() + '" alt="' + val.escapeQuotes() + '" />';
