...
foreach( $actions as $action ): // (goes to the end of the file)
  ...
  <?php if( !$this->noList ): ?><li rev="item-<?php echo $action->action_id ?>" class="wall-action-item"><?php endif; ?>
    ...
    <div class='checkin_item_body'>
      <?php // Main Content ?>
    <span class="<?php echo ( empty($action->getTypeInfo()->is_generated) ? 'checkin_item_posted' : 'feed_item_generated' ) ?>" id="action_<?php echo $action->action_id?>">
      ...
      $isEventEnabled = Engine_Api::_()->touch()->isModuleEnabled('event');
      ?>
      <div class="checkin_item_locations">
              <span class="checkin_location_item">
                <div>
                  <?php if ($action->checkin && $action->checkin->object_id && (($isPageEnabled && $action->checkin->object_type == 'page') || ($isEventEnabled && $action->checkin->object_type == 'event'))) : ?>
                  <?php $checkinObject = Engine_Api::_()->getItem($action->checkin->object_type, $action->checkin->object_id); ?>
                  ...
                  <?php else :?>
                  <a href="<?php echo $this->url(array('module' => 'checkin', 'controller' => 'index', 'action' => 'view-map', 'place_id' => $action->place_id), 'default', true)?>" class="smoothbox">
                    <img src="<?php echo ($action->checkin->icon) ? $action->checkin->icon : $noPhoto; ?>" class="thumb_normal">
                  </a>
                  <?php endif; ?>
                  <div>
                    <div class="checkin_item_link_title">
                      <?php
                      if (($isPageEnabled && $action->checkin->object_type == 'page') || ($isEventEnabled && $action->checkin->object_type == 'event')) : ?>
                        <?php echo $this->htmlLink($checkinObject->getHref(), $checkinObject->getTitle()); ?>
                        <?php else : ?>
                        <?php echo $this->htmlLink($this->url(array('module' => 'checkin', 'controller' => 'index', 'action' => 'view-map', 'place_id' => $action->place_id), 'default', true), $action->checkin->name, array('class' => 'smoothbox')); ?>
                        <?php endif; ?>
                    </div>
                    <div class="checkin_item_vicinity">
                      <span>
                        <?php if ($isPageEnabled && $action->checkin->object_type == 'page') : ?>
                        <?php echo $checkinObject->country . ', ' . $checkinObject->city . ', ' . $checkinObject->street ?>
                        <?php else : ?>
                        <?php echo $action->checkin->vicinity; ?>
                        <?php endif; ?>
                      </span>
                    </div>
                    <span class="checkin_like_count">
                      ...
                      <i class="checkin_item_users_count">
                        <span><?php echo $this->translate(array('CHECKIN_%s was here', 'CHECKIN_%s were here', $action->checkin->visitors), $action->checkin->visitors);?></span>
                      </i>
                    </span>
                  </div>
                </div>
    ...
    <div class='checkin_item_attachments'>
      ...
      <?php if( count($action->getAttachments()) == 1 && current($action->getAttachments())->item &&
        ...
        !(Engine_Api::_()->touch()->isModuleEnabled('album') && current($action->getAttachments())->item instanceof Album_Model_Photo)): ?>
        <?php if (
          (Engine_Api::_()->touch()->isModuleEnabled('video') &&
          current($action->getAttachments())->item instanceof Video_Model_Video) ||
          (Engine_Api::_()->touch()->isModuleEnabled('music') &&
