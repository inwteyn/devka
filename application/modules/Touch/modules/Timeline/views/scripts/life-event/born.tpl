...
<li class="born tli starred le">
  ...
  <div class="info">
    <div>
      <?php echo $this->htmlImage('application/modules/Timeline/externals/images/born_icon.png', ''); ?>
    </div>
    <div class="date">
      <?php echo $this->translate('TIMELINE_Born on %1s', $this->timestamp($this->birthdate)); ?>
    </div>
  </div>
  ...
  <div class="photo <?php if (!$this->photoExists): ?>add<?php endif; ?>">
    ...
    <div id="tl-born">
      <a href="javascript:void(0);"
        <?php if ($this->photoExists && Engine_Api::_()->getApi('settings', 'core')->getSetting('wall.content.slideshow', true) && $this->albumPhoto): ?>
         onclick="tl_born.slideShow('<?php echo $this->albumPhoto->getPhotoUrl(); ?>', '<?php echo $this->albumPhoto->getGuid(); ?>', this)"
        <?php endif; ?>
        >
        <?php
        if ($this->photoExists)
          echo $this->subject()->getTimelinePhoto('born');
        else
          echo $this->translate('TOUCH_tl_no_born_photo');
        ?>
      </a>
    </div>
