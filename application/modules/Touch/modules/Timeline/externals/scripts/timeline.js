  ...
  items:{
    ...
    alignElement:function (el, params) {
      ...
      if (params.left <= params.right) {
        params.left += el.getHeight() + 30;
        el.removeClass('r').addClass('l');
      } else {
        params.right += el.getHeight() + 30;
        ...
        if ($type(el) == 'element') {
          if (el.hasClass('utility-viewall')) {
            ...
            if (!el.hasClass('d')) {
              ...
              if (null != self.separator.lookup(el)) {
...
//        }
  ...
  composer:{
    ...
    highlight:function (el) {
      ...
      setTimeout(function () {
        ...
        myFx.start('background-color', fromBg, toBg).chain(function () {el.removeClass('just_loaded');});
      }, '2000');
  ...
  scroller:{
    ...
    initEvents:function (items) {
      ...
      var tl_layout = document.body.getElement('.layout_page_timeline_profile_index');
      tl_layout.removeEvent('click');
      tl_layout.addEvent('click', function(event){
        var target = event.target;
        var tl_dates = $('tl-dates');
        if((target.hasClass('tl-arrow') || target.hasClass('tl-marker')) && tl_dates.getStyle('display') == 'none'){
          tl_dates.setStyle('display', 'block');
        } else {
          tl_dates.setStyle('display', 'none');
        }
        target.hasClass('tl-arrow')
      })
    },
    ...
    checkPosition:function () {
      ...
      var y = document.getScroll().y
      if (y >= self.content.getTop()) {
        scroller.setStyles({'top': (y-80)+'px'});
      } else {
        scroller.setStyles({'top':'5px'});
      }
    },
