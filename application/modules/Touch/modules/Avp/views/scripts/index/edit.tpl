...
<script type="text/javascript">
  ...
  en4.core.runonce.add(function()
  {
    var canView = <?php echo $this->can_view ?>;
    var canViewTotal = <?php echo count($this->can_view) ?>;
    
    for (var i = 0; i < canViewTotal; i++)
    {
      var myElement = new Element("span");
      myElement.id = "tospan_"+canView[i].title+"_"+canView[i].id;
      myElement.setAttribute("class", "tag");
      myElement.innerHTML = canView[i].title + " <a href='javascript:void(0);' onclick='this.parentNode.destroy();removeFromToValue(\""+canView[i].id+"\");'>x</a>";
...
{
      var tags_id = avp.tag.register(avpGetById('tags_visible'), avpGetById('tags_container'), avpGetById('tags'), avpGetById('form-upload'));
      
      <?php if (!empty($this->video->tags)): ?>
      var tags = <?php echo $this->video->tags; ?>;

      tags.each(function(tag)
      {
            avp.tag.addTag(tags_id, tag);
      });
      <?php endif; ?>
});
//]]>
