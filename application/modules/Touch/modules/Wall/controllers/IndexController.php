...
class Wall_IndexController extends Touch_Controller_Action_Standard
{
  ...
  public function postAction()
  {
    ...
    try
    {
      ...
      if( !$attachment && $viewer->isSelf($subject) )
      {
        ...
        $action = Engine_Api::_()->getDbtable('actions', 'wall')->addActivity($viewer, $subject, 'status', $body, array('is_mobile' => true), $this->_getParam('privacy'));
      }
      ...
      else
      {
        ...
        // Add activity
        $action = $this->_helper->api()->getDbtable('actions', 'wall')->addActivity($viewer, $subject, $type, $body, array('is_mobile' => true), $this->_getParam('privacy'));
        ...
        if(!empty($stream_services)){
          foreach ($stream_services as $provider => $enabled){
            ...
            if (!$service->check($tokenRow)){
              $this->view->check = 'fail';
              continue ;
            }
            $this->view->check = 'success';
            $this->view->tokenRow__ = get_class($tokenRow);
            $this->view->action__ = get_class($action);
            $this->view->viewer__ = get_class($viewer);

            $this->view->service_result = $service->postAction($tokenRow, $action, $viewer);

          }
        }

        if ($action && method_exists(Engine_Api::_()->getDbTable('userSettings', 'wall'), 'saveLastPrivacy')){
    ...
    if ($action){
      $this->view->body = $this->view->wallActivity($action, array('comment_pagination' => $this->_getParam('comment_pagination'),'module' => $this->_script_module));
  ...
  public function likeAction()
  {
    ...
    {
      $this->view->body = $this->view->wallActivity($action, array('checkin' => $checkin,'noList' => true,'comment_pagination' => $this->_getParam('comment_pagination'),'module' => $this->_script_module,));
    }
  }
  ...
  public function viewcommentAction()
  {
    ...
    {
      $this->view->body = $this->view->wallActivity($action, array('viewAllComments' => true,'noList' => $this->_getParam('nolist', false),'comment_pagination' => $this->_getParam('comment_pagination'),'module' => $this->_script_module,));
    }
  }
  ...
  public function shareAction()
  {
    ...
    try
    {
      ...
      $action = $api->addActivity($viewer, $viewer, 'post_self', $body, array('is_mobile' => true), $this->_getParam('privacy'));
      if($action){
      ...
      $tableToken = Engine_Api::_()->getDbTable('tokens', 'wall') ;
      $stream_services = $this->_getParam('share');
      if($this->getParam('privacy') != 'owner')
      foreach ($stream_services as $provider => $enabled){
  ...
  public function servicesRequestAction()
  {
    ...
    foreach (Engine_Api::_()->wall()->getManifestType('wall_service', true) as $service){
      if ($this->_getParam($service)){
        ...
        $this->view->$service = array_merge(array('enabled' => true), $token->publicArray());
      }
    }
  ...
  public function suggestAction()
  {
    ...
    if( !$viewer->getIdentity() ) {
      ...
      if( $this->_getParam('includeSelf', false) ) {
        ...
        if (!$text || strpos($viewer->displayname, $text) !== false){
          $data[] = array(
            'type' => 'user',
