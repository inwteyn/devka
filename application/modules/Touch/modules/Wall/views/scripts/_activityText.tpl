 ...
 * @version    $Id: _activityText.tpl 8466 2011-02-15 21:43:50Z john $
 * @author     Jung
 */
?>
...
?>
<span class="touch_lang_4_js Attach"><?php echo $this->translate('Attach'); ?></span>
  ...
  <?php if( !$this->noList ): ?><li rev="item-<?php echo $action->action_id ?>" class="wall-action-item"><?php endif; ?>
      ...
      ?>
        ...
        <span class="wall_grouped_feed_item <?php echo $icon_type?> <?php echo ( empty($action->getTypeInfo()->is_generated) ? 'feed_item_posted' : 'feed_item_generated' ) ?>">
          ...
          <?php
            ...
            $profile_url = $this->htmlLink($action->getSubject()->getHref(), $action->getSubject()->getTitle(), array('class' => 'wall_liketips touchajax', 'rev' => $action->getSubject()->getGuid()));
            ...
            if ($count > 2){
              for ($i=0; $i<$count; $i++){
                ...
                $items_str .= '<a href="'.$subject->getHref().'" class="wall_liketips touchajax" rev="'.$subject->getGuid().'">'.$subject->getTitle().'</a>';
                if ($i < $count-1){
                  $items_str .= ', ';
                }
              }
              ...
              $other_link = '<a href="'.$subject->getHref().'" class="wall_liketips touchajax" rev="'.$subject->getGuid().'">'.$subject->getTitle().'</a>';
            }
            ...
            $first_item_link = $this->htmlLink($first_item->getHref(), $first_item->getTitle(), array('class' => 'wall_liketips touchajax', 'rev' => $first_item->getGuid()));
    ...
    <div class='feed_item_photo'>
      ...
      <?php if (Engine_Api::_()->wall()->isOwnerTeamMember($action->getObject(), $action->getSubject())): ?>
        ...
        <?php echo $this->htmlLink($action->getObject()->getHref(),
          $this->itemPhoto($action->getObject(), 'thumb.icon', $action->getObject()->getTitle()), array('class' => 'wall_liketips  touchajax', 'rev' => $action->getObject()->getGuid())) ?>

      <?php else :?>

        <?php echo $this->htmlLink($action->getSubject()->getHref(),
          $this->itemPhoto($action->getSubject(), 'thumb.icon', $action->getSubject()->getTitle()), array('class' => 'wall_liketips  touchajax', 'rev' => $action->getSubject()->getGuid())) ?>
    ...
    <div class='feed_item_body'>
      ...
      <div class='feed_item_date feed_item_icon <?php echo $icon_type ?>'>
        ...
        <?php
        if ($pageEnabled && $checkin->object_type == 'page') {
          ?>
          <?php $page = Engine_Api::_()->getItem($checkin->object_type, $checkin->object_id); ?>
            <li>
              ...
              <li class="feed_item_option_is_checkin">
                <a class="feed_item_checkin wall_tips" href="<?php echo $page->getHref()?>" title="<?php echo $page->country . ', ' . $page->city . ', ' . $page->street ?>" target="_blank"><?php echo $page->getTitle()?></a>
              </li>
          <?php } else if ($eventEnabled && $checkin->object_type == 'event') {
          ?>
          ...
          <li class="feed_item_option_is_checkin">
            <a class="feed_item_checkin wall_tips" href="<?php echo $event->getHref()?>" title="<?php echo $checkin->vicinity; ?>" target="_blank"><?php echo $event->getTitle()?></a>
          </li>
          <?php } else { ?>
            <li>
              <span class="timestamp">
                <?php echo '&nbsp;&nbsp;' . $this->translate($this->wallLocationTypes($checkin->types))?>
              </span>
            </li>
              <li class="feed_item_option_is_checkin">
            <a href="<?php echo $this->url(array('module' => 'checkin', 'controller' => 'index', 'action' => 'view-map', 'place_id' => $checkin->place_id), 'default', true)?>" class="smoothbox feed_item_checkin wall_tips" title="<?php echo $checkin->vicinity?>"><?php echo $checkin->name?></a>
              </li>
          <?php } ?>
          <?php endif; ?>
      ...
      <div class='comments wall-comments'>
        ...
        <?php
          ...
          if (method_exists($action, 'canChangePrivacy')  && $action->canChangePrivacy($this->viewer())):
            ...
            if ($privacy_active):
              ...
              <div class="wall-privacy-container">
                <a href="javascript:void(0);" class="wall-privacy-action-link wall_blurlink"><?php echo $this->translate('WALL_PRIVACY_' . strtoupper($privacy_type) . '_'. strtoupper($privacy_active));?></a>
