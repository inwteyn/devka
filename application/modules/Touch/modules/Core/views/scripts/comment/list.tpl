...
<?php endif; ?>
  <div class='comments_options'>
    ...
    <?php if( isset($this->form) ): ?>
      - <a href='javascript:void(0);' onclick="$('comment-form').style.display = '';$('comment-form').body.focus(); Touch.comment.bindBox($('comment-form'));">
    ...
    <?php if( isset($this->delete) && $this->delete): ?>
      - <a href="<?php echo $this->delete['url']; ?>" class="touchconfirm redirect">
					<?php echo $this->delete['text']; ?>
				</a>
    <?php endif; ?>

  </div>
  <ul onmouseover="Touch.comment.bindLikes(
  $$('.comments_comment_likes'),
  '<?php echo $this->url(array('module' => 'core', 'controller' => 'comment', 'action' => 'get-likes'), 'default', true) ?>',
