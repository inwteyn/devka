...
class Suggest_IndexController extends Touch_Controller_Action_Standard
{
  ...
  public function suggestAction()
  {
    ...
    if (empty($uids)) {
      ...
      $this->view->message = Zend_Registry::get('Zend_Translate')->_('TOUCH_Choose friends from list.');
      return ;
    }
    ...
    if ($api->isAllowed($suggest_type)) {
      if ($api->suggest($viewer, $uids, $object)) {
        $this->view->type = 'text';
        $this->view->message = Zend_Registry::get('Zend_Translate')->_('TOUCH_Your suggestions were successfully sent.');
      } else {
        $this->view->type = 'error';
        $this->view->message = Zend_Registry::get('Zend_Translate')->_('TOUCH_Your suggestions were not sent.');
      }
    } else {
      $this->view->type = 'error';
      $this->view->message = Zend_Registry::get('Zend_Translate')->_('TOUCH_Suggestion of this type was turned off by site admin.');
    }
  }
  ...
  public function contactsAction()
  {
    ...
    $this->view->params = $params = $this->_getParam('params', array());
    $this->view->action_url = $this->_getParam('action_url');
    ...
    $this->view->viewer = $viewer = Engine_Api::_()->user()->getViewer();
    $this->view->disabled_label = $disabled_label = !empty($this->view->params['disabled_label']) ? $this->view->params['disabled_label'] : "";
    $this->view->ipp = $ipp = (int)$this->_getParam('ipp', 3);
    ...
    $this->view->checkedItems = Engine_Api::_()->suggest()->getNotMutualFriendsDisabled($params);
  ...
  public function acceptSuggestAction()
  {
    ...
    $this->view->redirect = $object->getHref(array(
      'reset' => true
    ));
  }
  ...
  {
    $this->view->content = Engine_Api::_()->getApi('settings', 'core')->__get('suggest.fb.invite.text', 'Join me here there is a lot of stuff.');
    $this->view->type = Engine_Api::_()->getApi('settings', 'core')->__get('core.general.site.title', 'My Community');
    $this->view->actionText = Engine_Api::_()->getApi('settings', 'core')->__get('suggest.fb.invite.title', 'Invite Your Friends from Facebook');
    $this->view->appId = Engine_Api::_()->getApi('settings', 'core')->__get('core.facebook.appid');
  }
