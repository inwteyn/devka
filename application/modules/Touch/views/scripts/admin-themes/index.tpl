...
<div class="admin_theme_editor_wrapper">
    ...
    <div class="admin_theme_edit">
      ...
      <div class="admin_theme_header_controls">
        ...
        <div>
          ...
          <?php echo $this->htmlLink(array('route'=>'admin_default', 'module'=>'touch',
          'controller'=>'themes', 'action'=>'export','name'=>$this->activeTheme->name),
          $this->translate('Export'), array(
          'class' => 'buttonlink admin_themes_header_export',
          )) ?>
          <?php echo $this->htmlLink(array('route'=>'admin_default', 'module'=>'touch',
          'controller'=>'themes', 'action'=>'clone', 'name'=>$this->activeTheme->name),
      ...
      <div class="admin_theme_editor_edit_wrapper">
        ...
        $thumb = $this->manifest[$theme->name]['package']['thumb'];
          ...
          <div class="theme_wrapper_selected"><img src="<?php echo $thumb ?>" alt="<?php echo $theme->name?>">
          </div>
          <div class="theme_selected_info">
            ...
            <h4><?php echo $this->translate('by %s',
              $this->manifest[$theme->name]['package']['author']) ?></h4>
            <?php endif; ?>
            <div class="theme_edit_file">
              <h4>
                <?php echo $this->translate("Editing File:") ?>
              </h4>
              <?php echo $this->formSelect('choosefile', $this->activeFileName, array('onchange' =>
              'changeThemeFile(this.value);'), $this->activeFileOptions) ?>
            </div>
          </div>
          <?php break; endif; ?>
          <?php endforeach; ?>
        ...
        <div class="admin_theme_editor">
          <?php echo $this->formTextarea('body', $this->activeFileContents, array('onkeypress' =>
          'pushModification("body")', 'spellcheck' => 'false')) ?>
        </div>
        <button class="activate_button" onclick="saveFileChanges();return false;"><?php echo $this->
  ...
  <div class="admin_theme_chooser">
    ...
    <div class="admin_theme_header_controls">
      ...
      <div>
        <?php echo $this->htmlLink(array('route'=>'admin_default', 'module' => 'touch', 'controller'=>'themes','action'=>'upload'),
        $this->translate("Upload New Theme"), array('class'=>'buttonlink admin_themes_header_import')) ?>
      </div>
    </div>
    ...
    <div class="admin_theme_editor_chooser_wrapper">
      <ul class="admin_themes">
        ...
        ?>
        <li
        <?php echo ($alt_row) ? ' class="alt_row"' : "";?>>
        ...
        <div class="theme_chooser_info">
          ...
          <?php endif; ?>
          <?php if ( !empty($this->manifest[$theme->name]['package']['author'])): ?>
          <h4><?php echo $this->translate('by %s', $this->manifest[$theme->name]['package']['author'])
            ?></h4>
          <?php endif; ?>
    ...
    <br>
    <?php if(count($this->touchSupportedThemes) > 0 ): ?>
    <h2>Core themes</h2>
    <div class="admin_theme_editor_chooser_wrapper">
      <ul class="admin_themes">
        <?php
        $alt_row=true;
        foreach( $this->touchSupportedThemes as $theme ):
          $thumb = 'application/modules/Core/externals/images/anonymous.png';
          if( !empty($theme['package']['thumb']) )
          $thumb = $theme['package']['thumb'];
        ?>
        <li
        <?php echo ($alt_row) ? ' class="alt_row"' : "";?>>
        <div class="theme_wrapper"><img src="<?php echo $thumb ?>" alt="<?php echo $theme['package']['name']; ?>"></div>
        <div class="theme_chooser_info">
          <h3><?php echo $theme['package']['title']; ?></h3>
          <?php if ( !empty($theme['package']['version'])): ?>
          <h4 class="version">v<?php echo $theme['package']['version'] ?></h4>
          <?php endif; ?>
          <?php if ( !empty($theme['package']['author'])): ?>
          <h4><?php echo $this->translate('by %s', $theme['package']['author']) ?></h4>
          <?php endif; ?>
        </div>
        <form action="<?php echo $this->url(array('action' => 'copy')) ?>" method="post">
          <button class="activate_button"><?php echo $this->translate('TOUCH_Copy theme to touch') ?></button>
          <?php echo $this->formHidden('theme', $theme['package']['name'], array('id'=>'')) ?>
        </form>
        </li>
        <?php $alt_row = !$alt_row; ?>
        <?php endforeach; ?>
