...
<script type="text/javascript">
  ...
  window.addEvent('load', function() {
	...
	var checkForColumn = function(element){
		if (element.getParent().hasClass('left_column_contents') || element.getParent().hasClass('right_column_contents')){
				var column = (element.getParent().hasClass('left_column_contents'))? 'left':'right';
				element.addClass(column+'_column_content');
				var tmpParams = element.retrieve('contentParams') || {};
				tmpParams['params'] = tmpParams['params'] || {};
				tmpParams['params']['column'] = column;
				element.store('contentParams', tmpParams);
		}
	}
  ...
  {
    if(document.body.getElement('.left_column_contents') && document.body.getElement('.left_column_contents').getElements('.admin_content_buildable'))
      document.body.getElement('.left_column_contents').getElements('.admin_content_buildable').each(function(element){
        checkForColumn(element);
      });
    if(document.body.getElement('.right_column_contents') && document.body.getElement('.right_column_contents').getElements('.admin_content_buildable'))
      document.body.getElement('.right_column_contents').getElements('.admin_content_buildable').each(function(element){
        checkForColumn(element);
      });
    var data = [];
    ...
    var url = '<?php echo $this->url(array('action' => 'update', 'controller' => 'content', 'module' => 'touch'), 'admin_default', true)?>';
  ...
  var openWidgetParamEdit = function(name, element) {
    ...
    var url = '<?php echo $this->url(array('action' => 'widget', 'controller' => 'content', 'module' => 'touch'), 'admin_default', true)?>';
  ...
  var saveCurrentPageInfo = function(formElement) {
    var url = '<?php echo $this->url(array('action' => 'save', 'controller' => 'content', 'module' => 'touch'), 'admin_default', true)?>';
  ...
  /* Tab & Column container and other special block handling */
  var expandSpecialBlock = function(element)
  {
    if( element.hasClass('admin_content_widget_touch.container-tabs') ) {
      ...
      new Element('span', {
        ...
        'html' : contentByName["touch.container-tabs"].childAreaDescription
      }).inject(element);
      // Edit area
      ...
      ContentSortables.addLists(list);
		...
		if (element.hasClass('admin_content_widget_touch.container-columns')){
      element.addClass('admin_layoutbox_widget_tabbed_wrapper');
      // Empty
      element.empty();
      // Title/edit
      new Element('span', {
        'class' : 'admin_layoutbox_widget_tabbed_top',
        'html' : '<?php echo $this->string()->escapeJavascript($this->translate("Column Blocks")) ?><span class="open"> | <a href=\'javascript:void(0);\' onclick="openWidgetParamEdit(\'core.container-tabs\', $(this).getParent(\'li.admin_content_cell\')); (new Event(event).stop()); return false;"><?php echo $this->string()->escapeJavascript($this->translate("edit")) ?></a></span> <span class="remove"><a href="javascript:void(0)" onclick="removeWidget($(this));">x</a></span>'
      }).inject(element);
      // Desc
      new Element('span', {
        'class' : 'admin_layoutbox_widget_column_overtext',
        'html' : contentByName["touch.container-columns"].childAreaDescription
      }).inject(element);
      // Edit area
      var tmpDivContainer = new Element('div', {
        'class' : 'admin_layoutbox_widget_tabbed'
      }).inject(element);
      var list1 = new Element('ul', {
        'class' : 'sortablesForceInclude admin_content_sortable admin_layoutbox_widget_tabbed_contents left_column_contents'
      }).inject(tmpDivContainer);

			var list2 = new Element('ul', {
        'class' : 'sortablesForceInclude admin_content_sortable admin_layoutbox_widget_tabbed_contents right_column_contents'
      }).inject(tmpDivContainer);

      ContentSortables.addLists(list1);
			ContentSortables.addLists(list2);
		}
  }
  ...
  var deleteCurrentPage = function() {
    ...
    var url = '<?php echo $this->url(array('action' => 'delete', 'controller' => 'content', 'module' => 'touch'), 'admin_default', true)?>';
    var request = new Request.JSON({
      'url' : url,
      'data' : {
        'format' : 'json',
...
<?php echo $this->content()->renderWidget('touch.admin-main-menu', array('active'=>'touch_admin_main_content')) ?>

<h3><?php echo $this->translate('TOUCH_Layout Editor'); ?></h3>
<p>
  <?php echo $this->translate('TOUCH_LAYOUT_DESC'); ?>
</p>
                    ...
                    <td id="admin_content_<?php echo $structTwo['identity'] ?>" class="admin_content_column admin_content_buildable admin_content_<?php echo $structTwo['type'] . '_' . $structTwo['name'] ?>">
                      ...
                      <ul class="admin_content_sortable">
                        ...
                        ?>
                          ...
                          <?php elseif ($structThreeInfo['name'] == 'touch.container-columns'): ?>
                            <!-- column widgets -->
                            <li id="admin_content_<?php echo $structThree['identity'] ?>" class="admin_content_cell admin_content_buildable admin_content_draggable admin_layoutbox_widget_tabbed_wrapper admin_content_<?php echo $structThree['type'] . '_' . $structThree['name'] ?>">
                              ...
                              <div class="admin_layoutbox_widget_tabbed">
																	<ul class="sortablesForceInclude admin_content_sortable admin_layoutbox_widget_tabbed_contents left_column_contents">
																		<script type="text/javascript">
																			window.addEvent('domready', function() {
																				$("admin_content_<?php echo $structThree['identity'] ?>").store('contentParams', <?php echo Zend_Json::encode($structThreeNE) ?>);
																			});
																		...
																		<?php
																				...
																				if($structFour['params']['column'] == 'left'):
																	...
																	<ul class="sortablesForceInclude admin_content_sortable admin_layoutbox_widget_tabbed_contents right_column_contents">
																		...
																		<?php
																				...
																				if($structFour['params']['column'] == 'right'):
														...
														<div class="admin_layoutbox_widget_tabbed">
																<table><tr>
																	...
																	?>
																		...
																		<?php if ($structFourInfo['name'] == 'touch.container-left'):?>
