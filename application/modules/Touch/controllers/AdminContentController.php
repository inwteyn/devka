 ...
 * @version    $Id: AdminContentController.php 2011-04-26 11:18:13 mirlan $
 * @author     Mirlan
 */
...
class Touch_AdminContentController extends Core_Controller_Action_Admin
{
  ...
  public function indexAction()
  {
    ...
    $action_url = Zend_Controller_Front::getInstance()->getRouter()->assemble(array('action' => 'save', 'controller' => 'content', 'module' => 'touch'), 'admin_default', true);
    $pageForm->setAction($action_url);
  ...
  {
    if (_ENGINE_ADMIN_NEUTER) {
      return $this->_helper->redirector->gotoRoute(array('module' => 'touch', 'controller' => 'content', 'action' => 'index'), 'admin_default', true);
    }
    // Get page param
    $page = $this->_getParam('page');
    $pageTable = Engine_Api::_()->getDbtable('pages', 'touch');
  ...
  public function saveAction()
  {
    ...
    if( $this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost()) )
    {
      ...
      // Изменения. Кирилл, 20.05.2011
      $pageTable = Engine_Api::_()->getDbtable('pages', 'touch');
      $row = $pageTable->find($page_id);
      // end of Изменения. Кирилл, 20.05.2011

      $pageObject =
        $pageTable->fetchRow($pageTable->select()->where('name = ?', $page_id)->orWhere('page_id = ?', $page_id));
  ...
  public function getContentAreas()
  {
    ...
    $modules = Zend_Controller_Front::getInstance()->getControllerDirectory();
		$touch = Zend_Controller_Front::getInstance()->getControllerDirectory('touch');
    foreach( $modules as $module => $path) {
			$contentManifestFile = dirname($touch) . '/settings/contents/' . $module . '-content.php';
    ...
    // From widgets
    $it = new DirectoryIterator(dirname($touch) . '/widgets/widgets');
    foreach( $it as $dir ) {
