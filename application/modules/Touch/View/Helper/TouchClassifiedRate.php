 ...
 * @version    $Id: TouchItemRate.php 2011-02-14 06:58:57 mirlan $
 * @author     Mirlan
 */
...
class Touch_View_Helper_TouchClassifiedRate extends Engine_View_Helper_HtmlElement
{
  public function touchClassifiedRate($item_type, $item_id, $show_score = false, $score_br = true)
  {
    if (!Engine_Api::_()->touch()->isModuleEnabled('rate')) {
      return '';
    }
    if (!Engine_Api::_()->getApi('settings', 'core')->getSetting('touch.classified.rate-browse', 1)) {
      return '';
    }
    ...
    if ($item_type == 'page') {
      ...
      for ($i = 0; $i < $maxRate; $i++)
      {
        ...
        $stars_str .= $this->view->htmlImage($this->view->baseUrl() . '/application/modules/Touch/externals/images/rate/small_' . $star_value . '.png');
      }
    ...
    if ($show_score) {
      ...
      $vote_lang_var = $translate->_(array('vote', 'votes', (($rate_info['rate_count']) ? $rate_info['rate_count']
                                       : 0)));
