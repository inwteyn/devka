...
<script type="text/javascript">
  ...
  window.addEvent('load', function(){
    Cookie.write('windowwidth', 324);
    window.onfocus = function () {
    Cookie.write('windowwidth', 324);
    };

    $('ips_browser').contentWindow.location  = "<?php echo $protocol.$site_url ?>/members/home";
  });
  window.addEvent('domready', function(){
    ...
    var showHide = function(){
      ...
      if(!iphoneSimulator.hasClass('simulator_hidden')){
        ...
        Cookie.write('<?php echo $this->setting_pref.'change.time'?>', new Date().getTime());
        iphoneSimulatorToolbar.getElement('.show_hide span').set('text', '<?php echo strtoupper($this->translate('TOUCH_SHOW')) ?>')
      } else{
        iphoneSimulator.removeClass('simulator_hidden');
        iphoneSimulatorToolbar.getElement('.show_hide span').set('text', '<?php echo strtoupper($this->translate('TOUCH_HIDE')) ?>')
    ...
    iphoneSimulator.getElement('.back_control').addEvent('click', function(){ipsBrowser.contentWindow.history.back()});
    iphoneSimulator.getElement('.forward_control').addEvent('click', function(){ipsBrowser.contentWindow.history.forward()});
  ...
  <div id="ip_simulator_toolbar" class="metal_cover">
    <div class="plastic_cover">
      <div class="sensor_glass">
        <div class="ipst_button ipst_button_close" onclick="$$('.layout_touch_admin_iphone_simulator')[0].setStyle('display', 'none');">
          &#215;
        </div>
        ...
        <div class="ipst_button show_hide">
          <span><?php echo $this->settings[$this->setting_pref.'showing']=='true'? strtoupper($this->translate('TOUCH_HIDE')): strtoupper($this->translate('TOUCH_SHOW')) ?></span>
        </div>
      </div>
    </div>
  </div>
<div id = 'phone_container' class="phone_container<?php echo $this->settings[$this->setting_pref.'showing']=='false'? ' simulator_hidden':''?>" style="left:<? echo $this->settings[$this->setting_pref.'pos.x']?>px; top: <? echo $this->settings[$this->setting_pref.'pos.y']?>px;">
