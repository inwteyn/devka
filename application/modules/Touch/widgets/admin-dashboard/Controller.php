 ...
 * @license    http://www.socialengine.net/license/
 */
class Touch_Widget_AdminDashboardController extends Engine_Content_Widget_Abstract
{
...
//    // Plugins
//    $moduleTable = Engine_Api::_()->getDbtable('integrations', 'touch');
//    $select = new Zend_Db_Select($moduleTable->getAdapter());
//    $select->from($moduleTable->info('name'), 'COUNT(TRUE) as count')->where('type <> ?', 'core')->where('type <> ?', 'standard');
//    $data = $select->query()->fetch();
//    $this->view->integrationCount = (int) $data['count'];
