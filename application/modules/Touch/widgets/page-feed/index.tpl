...
<?php if( $this->enableComposer ): ?>
	<div>
		...
		<form method="post" action="<?php echo $this->url(array('module' => 'activity', 'controller' => 'index', 'action' => 'post'), 'default', true) ?>" class="global_form_box" enctype="application/x-www-form-urlencoded">
			<?php echo $this->translate("TOUCH_What's on your mind?") ?><br/>
			...
			<div class="feed-submit">
				<button id="touch-compose-submit" type="submit"><?php echo $this->translate("Share") ?></button>
			</div>
		</form>
	</div>
<?php endif; ?>
    ...
    <ul class="paginationControl">
			<li class="paginator_previous">
				<?php if( $this->previd > 0): ?>
						...
						'<img src="application/modules/Touch/themes/' . $this->touchActiveTheme()->name . '/images/prev.png" alt="' . $this->translate('Prev') . '"/>') ?>
				<?php else: ?>
					<span><img src="application/modules/Touch/themes/<?php echo $this->touchActiveTheme()->name; ?>/images/prev_disabled.png" alt="<?php echo $this->translate('Prev') ?>"/></span>
				<?php endif; ?>
			</li>

			<li class="paginator_middle">
				<span>
					<?php echo $this->translate('TOUCH_Page %1$s of %2$s', $this->current, $this->pageCount) ?>
				</span>
			</li>
			...
			<li class="paginator_next">
				<?php if( $this->nextid > 0 && !$this->endOfFeed ): ?>
						...
						'<img src="application/modules/Touch/themes/' . $this->touchActiveTheme()->name . '/images/next.png" alt="' . $this->translate('Next') . '"/>') ?>
				<?php else: ?>
					<span><img src="application/modules/Touch/themes/<?php echo $this->touchActiveTheme()->name; ?>/images/next_disabled.png" alt="<?php echo $this->translate('Next') ?>"/></span>
				<?php endif; ?>
			</li>
