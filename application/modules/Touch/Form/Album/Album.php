...
class Touch_Form_Album_Album extends Touch_Form_Standard
{
  public function init()
  {
    ...
    $albumCatsTbl = Engine_Api::_()->getDbTable('categories', 'album');
    ...
    $this->album->addMultiOptions($albumOptions);
    ...
    // prepare categories
    if (method_exists($albumApi, 'getCategories')) {
      $category_list = $albumApi->getCategories();
      $categories = array();
      foreach ($category_list as $category){
        $categories[$category->category_id] = $category->category_name;
      }
    } else {
      $categories = $albumCatsTbl->getCategoriesAssoc();
    }

    if( count($categories) != 0 ) {
      $categories_prepared[0] = "";
      foreach ($categories as $category_id => $category_name){
        $categories_prepared[$category_id] = $category_name;
      }
