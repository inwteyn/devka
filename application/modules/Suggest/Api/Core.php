...
class Suggest_Api_Core extends Core_Api_Abstract
{
    ...
    protected $_itemTypes = array(
        ...
        'document' => 'Document',
        'job' => 'Job',
        'hecontest_photo' => 'Hecontest Photo'
    );
    ...
    public function getItemTypes()
    {
        ...
        foreach ($modules as $module) {
            ...
            if ($module == 'hecontest') {
                $data['hecontest_photo'] = $this->_itemTypes['hecontest_photo'];
            }
    ...
    public function isAllowed($suggestType)
    {
        ...
        switch ($suggestType) {
            case 'fr_sent':
                return (bool)$settings->getSetting('suggest.friend.add', 1);
                break;
            case 'fr_confirm':
                return (bool)$settings->getSetting('suggest.friend.confirm', 1);
                break;
            case 'blog_new':
                return (bool)$settings->getSetting('suggest.popup.create.blog', 1);
                break;
            case 'video_new':
                return (bool)$settings->getSetting('suggest.popup.create.video', 1);
                break;
            case 'classified_new':
                return (bool)$settings->getSetting('suggest.popup.create.classified', 1);
                break;
            case 'poll_new':
                return (bool)$settings->getSetting('suggest.popup.create.poll', 1);
                break;
            case 'music_playlist_new':
                return (bool)$settings->getSetting('suggest.popup.create.music_playlist', 1);
                break;
            case 'quiz_new':
                return (bool)$settings->getSetting('suggest.popup.create.quiz', 1);
                break;
            case 'quiz_take':
                return (bool)$settings->getSetting('suggest.popup.take.quiz', 1);
                break;
            case 'page_create':
                return (bool)$settings->getSetting('suggest.popup.create.page', 1);
                break;
            case 'event_create':
                return (bool)$settings->getSetting('suggest.popup.create.event', 1);
                break;
            case 'group_create':
                return (bool)$settings->getSetting('suggest.popup.create.group', 1);
                break;
            case 'article_new':
                return (bool)$settings->getSetting('suggest.popup.create.article', 1);
                break;
            case 'question_new':
                return (bool)$settings->getSetting('suggest.popup.create.question', 1);
                break;
            case 'album_photo_new':
                return (bool)$settings->getSetting('suggest.popup.create.album', 1);
                break;
            case 'gr_join':
                return (bool)$settings->getSetting('suggest.group.join', 1);
                break;
            case 'gr_accept':
                return (bool)$settings->getSetting('suggest.group.accept', 1);
                break;
            case 'ev_accept':
                return (bool)$settings->getSetting('suggest.event.accept', 1);
                break;
            case 'ev_join':
                return (bool)$settings->getSetting('suggest.event.join', 1);
                break;
            case 'link_page':
                return (bool)$settings->getSetting('suggest.link.page', 1);
                break;
            case 'link_event':
                return (bool)$settings->getSetting('suggest.link.event', 1);
                break;
            case 'link_group':
                return (bool)$settings->getSetting('suggest.link.group', 1);
                break;
            case 'link_blog':
                return (bool)$settings->getSetting('suggest.link.blog', 1);
                break;
            case 'link_poll':
                return (bool)$settings->getSetting('suggest.link.poll', 1);
                break;
            case 'link_user':
                return (bool)$settings->getSetting('suggest.link.user', 1);
                break;
            case 'link_classified':
                return (bool)$settings->getSetting('suggest.link.classified', 1);
                break;
            case 'link_music_playlist':
                return (bool)$settings->getSetting('suggest.link.music_playlist', 1);
                break;
            case 'link_video':
                return (bool)$settings->getSetting('suggest.link.video', 1);
                break;
            case 'link_album_photo':
                return (bool)$settings->getSetting('suggest.link.photo', 1);
                break;
            case 'link_album':
                return (bool)$settings->getSetting('suggest.link.album', 1);
                break;
            case 'link_quiz':
                return (bool)$settings->getSetting('suggest.link.quiz', 1);
                break;
            case 'link_article':
                return (bool)$settings->getSetting('suggest.link.article', 1);
                break;
            case 'link_question':
                return (bool)$settings->getSetting('suggest.link.question', 1);
                break;
            case 'suggest_profile_photo':
                return (bool)$settings->getSetting('suggest.profile.photo', 1);
                break;
            case 'link_store_product':
                return (bool)$settings->getSetting('suggest.store.product', 1);
                break;
            case 'link_playlist':
                return (bool)$settings->getSetting('suggest.link.playlist', 1);
                break;
            case 'link_pagealbum':
                return (bool)$settings->getSetting('suggest.link.pagealbum', 1);
                break;
            case 'link_pageblog':
                return (bool)$settings->getSetting('suggest.link.pageblog', 1);
                break;
            case 'link_pagedocument':
                return (bool)$settings->getSetting('suggest.link.pagedocument', 1);
                break;
            case 'link_pageevent':
                return (bool)$settings->getSetting('suggest.link.pageevent', 1);
                break;
            case 'link_pagevideo':
                return (bool)$settings->getSetting('suggest.link.pagevideo', 1);
                break;
            case 'link_ynmusic_album' :
                return (bool)$settings->getSetting('suggest.link.ynmusic.album', 1);
                break;
            case 'link_avp_video' :
                return (bool)$settings->getSetting('suggest.link.avp.video', 1);
                break;
            case 'link_artarticle' :
                return (bool)$settings->getSetting('suggest.link.artarticle', 1);
                break;
            case 'link_list_listing' :
                return (bool)$settings->getSetting('suggest.link.list.listing', 1);
                break;
            case 'link_document' :
                return (bool)$settings->getSetting('suggest.link.document', 1);
                break;
            case 'link_job' :
                return (bool)$settings->getSetting('suggest.link.job', 1);
                break;
            case 'link_offer':
                return (bool)$settings->getSetting('suggest.link.offer', 1);
                break;
            case 'link_hecontest_photo':
                return (bool)$settings->getSetting('suggest.link.hecontest_photo', 1);
                break;
            default:
                return false;
                break;
    ...
    public function getFriends($params)
    {
        ...
        $select = $userTable->select()
            ...
            ->where($MTname . '.user_id NOT IN (?)', array($user['id'], $owner->getIdentity()))
    ...
    public function getInviteFriends($params)
    {
        $paramsNeeded = array('resource_type', 'user_id', 'resource_id');


        if (!is_array($params) || array_diff($paramsNeeded, array_keys($params))) {
            return false;
        }

        $engine = Engine_Api::_();

        $user = $engine->getDbTable('users', 'user');

        $membership = $engine->getDbTable('membership', 'user');

        $coreLikes = $engine->getDbTable('likes', 'core');

        $storage = $engine->getDbTable('files', 'storage');

        $select = $user->select()
            ->setIntegrityCheck(false)
            ->from(array('u' => $user->info('name')), array('u.user_id', 'u.displayname'))
            ->joinLeft(array('m' => $membership->info('name')), 'u.user_id = m.user_id', array('m.user_id'))
            ->joinLeft(array('l' => $coreLikes->info('name')), 'l.poster_id = m.user_id and l.resource_type = "' . $params['resource_type'] . '" and l.poster_type = "user" and l.resource_id = ' . $params['resource_id'], array('l.resource_id'))
            ->joinLeft(array('s' => $storage->info('name')), 's.parent_file_id = u.photo_id  and s.type="thumb.icon"', array('s.storage_path'))
            ->where('m.resource_id = ?', $params['user_id'])
            ->where('m.resource_approved = 1')
            ->where('m.user_approved = 1');

        if ($params['liked'])
            $select->where('l.resource_id is not null');

        if (isset($params['keyword']) && !empty($params['keyword'])) {
            $select->where('(u.displayname LIKE ? OR u.username LIKE ?)', "%{$params['keyword']}%");
        }
        $select
            ->order(array('u.displayname'))
