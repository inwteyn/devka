...
//              $signature_secret .= $token_secret;
  ...
  public function getLinkedInContacts($token, $start = 0, $count = 2)
  {
    ...
    $client->setParameterGet('scope', 'r_network');
    $response = $client->request();
  ...
  {
    $hotmailPlugin = Engine_Api::_()->loadClass('Inviter_Plugin_Hotmail');
    $hotmailPlugin->init();

    $contactsObjs = $hotmailPlugin->getContacts($token);

    $contacts = array();

    foreach ($contactsObjs as $contactObj) {
      $email = $hotmailPlugin->getUserEmail($contactObj);
      if (!$email)
        continue;
      $contacts[$email] = array(
        'id' => $email,
        'nid' => substr($contactObj->id, 8),
...
//    $facebook = self::getFBInstance();
      ...
      try {
          $_contacts = $fbApi->getFriends($token);
    } catch (Exception $e) {
      return false;
    }
    $contacts = array();
...
//        $contacts[$name] = array(
        ...
        'nid' => $start,
        'name' => $name,
        'pic_square' => $friend_info->picture->data->url,
//        'profile_url' => $friend_info['profile_url']
  ...
  public function getGMailContacts($token, $start = 0, $count = 2)
  {
    ...
    $contacts = $plugin->getContacts($token->oauth_token, $start, $count);

    if (!$contacts) {
      return false;
    }

    $xml = simplexml_load_string($contacts);
  ...
  public function getNoneFriendContacts($token, $provider, $count = 9)
  {
    ...
    if ($provider == 'gmail' || $provider == 'yahoo' || $provider == 'hotmail' || $provider == 'foursquare' || $provider == 'mailru') {
      ...
      foreach ($contact_emails as $emails) {
        ...
        $resContacts = array();
        foreach ($already_members as $tmp) {
          if ($tmp && $tmp->search && $tmp->authorization()->isAllowed($viewer, 'view')) {
            $resContacts[] = $tmp;
          }
        }
        $already_members = $resContacts;
...
XML_MESSAGE;
  ...
  public function sendTwitterInvites($token, $contact_ids, $page_id = null)
  {
    ...
    foreach ($contact_ids as $contact_id) {
      ...
      $client->setUri('https://api.twitter.com/1.1/direct_messages/new.json');
      $client->setMethod(Zend_Http_Client::POST);
