...
{
    public function getProvidersSystem($include_disabled = false)
    {
  ...
  public function getProviders2($include_disabled = false) {
    return $this->getProvidersSystem();
  }

    public function getIntegratedProviders($touch = true)
    {
        $providerApi = Engine_Api::_()->getApi('provider', 'inviter');

        $rows = $this->getProvidersSystem();
        ...
        foreach ($rows as $row) {
            ...
            if ($providerApi->checkIntegratedProvider($provider_title)) {
              ...
              if($touch) {
                if($provider_title == 'facebook') {
                  $settings = Engine_Api::_()->getDbTable('settings', 'core');
                  $res[$provider_title] = $settings->getSetting('inviter.facebook.consumer.key', false);
                } else {
                  $res[$provider_title] = $row->provider_enabled;
                }
              } else {
                $res[] = $row;
              }
    ...
    {
        $provider_row = $this->getProvidersSystem();
        $providers = array();
        $i = 0;
