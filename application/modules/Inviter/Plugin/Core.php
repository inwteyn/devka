...
class Inviter_Plugin_Core
{
  ...
  public function onRenderLayoutDefault($event)
  {
    ...
    $view->headScript()->appendFile( (_ENGINE_SSL ? 'https://' : 'http://') . 'connect.facebook.net/en_US/all.js' );

    $view->fb_settings = Engine_Api::_()->inviter()->getFacebookSettings($view);

    $session = new Zend_Session_Namespace('inviter');

    $view->success = $session->__get('success', false);
    $view->message = $session->__get('message', false);
    $session->__set('success', false);
    $session->__set('message', false);
    $view->providers = $providers = Engine_Api::_()->inviter()->getIntegratedProviders(true);
    $view->count = count($providers);

    $viewer = Engine_Api::_()->user()->getViewer();
    $codes_tbl = Engine_Api::_()->getDbTable('codes','inviter');
    $view->referralLink = $referral_link = $codes_tbl->getUserReferralLink($viewer->getIdentity());

    $view->layout()->content .= $view->render('application/modules/Inviter/views/scripts/_providers_settings.tpl');
  }
}