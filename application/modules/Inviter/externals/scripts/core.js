...
var inviter = {
  ...
  showLnPopUp: function () {
    try {
      $('lnScreen').setStyle('display', 'block');
      $('lnPopUp').setStyle('display', 'block');
    } catch (e) {

    }

  },
  hideLnPopUp: function () {
    try {
      $('lnScreen').setStyle('display', 'none');
      $('lnPopUp').setStyle('display', 'none');
    }
    catch(e) {
  ...
  markField: function (id) {
    var field = $(id);
    field.setStyle('background-color', 'rgb(242, 211, 211)');
    setTimeout(function () {
      field.setStyle('background-color', '');
    }, 2000);
  },
  checkLnParams: function () {
    var title = $('ln_title').value.trim();
    var descr = $('ln_description').value.trim();
    var link = $('ln_link').value.trim();
    var flag = true;
    if (title.length <= 0) {
      this.markField('ln_title');
      flag = false;
    }
    if (descr.length <= 0) {
      this.markField('ln_description');
      flag = false;
    }
    if (link.length <= 0) {
      this.markField('ln_link');
      flag = false;
    }

    return flag;
  },
  shareWithLn: function () {
    var self = this;
    if (!self.checkLnParams()) {
      return;
    }

    var title = $('ln_title').value;
    var descr = $('ln_description').value;


    var params = '{"content":{"title":"' + title + '","description":"' + descr + '","submitted-url":"' + $('ln_link').value + '"},"visibility":{"code":"connections-only"}}';
    ...
    if (!IN.User.isAuthorized()) {
      IN.User.authorize(function (e) {
        IN.API.Raw("people/~/shares?format=json")
          ...
          .result(function (r) {
            self.hideLnPopUp();
            if (r.updateKey && r.updateUrl) {
              he_show_message($('ln-success-message').value, '', 5000);
            } else {
              he_show_message($('ln-fail-message').value, 'error', 5000);
            }
          });
...
var provider = {
  ...
  oauth_url: en4.core.baseUrl + 'inviter/oauth/request',
  ...
  submit_form: function ($node) {
    ...
    if (this.integrated_providers[label]) {
      ...
      /*if(label=='Live/Hotmail' || label == 'MSN') {
       alert(label);
       console.log(this.integrated_providers);
       return false;
       }*/
  ...
  open_connect: function (label) {
    ...
    if (label == 'live/hotmail' || label == 'msn' || label == 'msn/live/hotmail') {
      label = 'hotmail';
    }
...
{
  url: en4.core.baseUrl + 'inviter/introduce/ajax-request',
