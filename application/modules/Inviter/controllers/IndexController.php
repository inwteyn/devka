...
class Inviter_IndexController extends Core_Controller_Action_Standard
{
    ...
    public function processAction()
    {
        ...
        $all_providers = Engine_Api::_()->inviter()->getProvidersSystem();
        ...
        if ($this->getRequest()->isPost() && $form->isValid($post_params)) {
            ...
            if ($providerApi->checkIntegratedProvider($post_params['provider_box'])) {
                ...
                $this->view->form->addError("INVITER_An error occurred.");
            }
        } else {
            $this->_redirectCustom(array('route' => 'inviter_general'));
        }
    ...
    public function uploadContactsAction()
    {
      ...
      $session = new Zend_Session_Namespace('inviter');
      $session->__set('contacts', $contactRows);
    ...
    public function writeContactsAction()
    {
        ...
        if ($page_id) {
            $sent = (int)Engine_Api::_()->getApi('provider', 'inviter')->sendPageEmails($session, $message, $contacts, $page_id);
        } else {
            $sent = (int)Engine_Api::_()->getApi('provider', 'inviter')->sendEmails($session, $message, $contacts);
        }
    ...
    public function membersAction()
    {
      ...
      $resContacts = array();
      foreach($userSl as $tmp) {
        if($tmp->search && $tmp->authorization()->isAllowed($viewer, 'view')) {
          $resContacts[] = $tmp;
        }
      }
      $members = Zend_Paginator::factory($resContacts);
    ...
    public function contactsAction()
    {
        ...
        $this->view->invitation_url_length = strlen($providerApi->getInvitationUrl("XXXXXXX", null, null));
...
//        case 'facebook':
    ...
    public function invitationsendAction()
    {
      ...
      $contact_ids = $this->_getParam('contact_ids', false);
      if(!$contact_ids) {
        $contact_ids = $session->__get('contact_ids', array());
      } else {
        $contact_ids = (is_array($contact_ids)) ? $contact_ids : explode(',', $contact_ids);
        $session->__set('contact_ids', $contact_ids);
      }

      $message = $this->_getParam('message', $this->_getParam('message_box'));
      $translate = Zend_Registry::get('Zend_Translate');
      ...
      $form = new Inviter_Form_Send(array('params' => false));
      $form->setAction($this->view->baseUrl . $this->view->url(array('module'=>'inviter', 'controller'=>'index', 'action'=>'invitationsend'), 'default', 1));
      $this->view->form = $form;
      ...
      if ($provider && $providerApi->checkIntegratedProvider($provider)) {
        ...
        $providerApi->sendEmails($session, $message, $arr );
      }
    ...
    public function pageinviterAction()
    {
        ...
        $this->view->providers = Engine_Api::_()->inviter()->getIntegratedProviders(false);
