...
class Inviter_OauthController extends Core_Controller_Action_Standard
{
  ...
  {
    $this->view->provider = $this->provider = $this->_getParam('provider', 'twitter');
    ...
    $this->hotmailPlugin = Engine_Api::_()->loadClass('Inviter_Plugin_Hotmail');
    $this->hotmailPlugin->init();
  }
  ...
  public function requestAction()
  {
    ...
    if ($tokenRow && !$new_token) {
      if ($this->provider == 'hotmail') {

        $tmp = strtotime($tokenRow->expiration_date);
        if ($tmp <= time()) {
          $params = array('module' => 'inviter', 'controller' => 'oauth', 'action' => 'callback');
          $params['action'] = 'request';
          $params['provider'] = 'hotmail';
          $params['new'] = 1;
          $host_url = (_ENGINE_SSL ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'];
          $url = $host_url . $this->view->url($params, 'default', 1);
        }
      }
      $this->view->tokenRow = $tokenRow;
      $this->view->form = $this->_generateForm($tokenRow, $this->_getParam('empty', 0));
      return;
    }

    if ($this->provider == 'mail.ru') {
    ...
    if ($this->provider == 'facebook') {
    ...
    } elseif ($this->provider == 'linkedin') {

      $lnApi = Engine_Api::_()->loadClass('Inviter_Plugin_Linkedin');
      $lnApi->init();

      $client_id = $settings->getSetting('inviter.linkedin.consumer.key');

      $url = $this->view->url(array('module' => 'inviter', 'controller' => 'oauth', 'action' => 'access', 'provider' => $this->provider), 'default', 1);
      $redirect_url = $host_url . $url;
      $auth_url = $lnApi->getAuthUrl($redirect_url);
      ...
      $vkApi = Engine_Api::_()->loadClass('Inviter_Plugin_Vk');
      $vkApi->init();


      $url = $this->view->url(array('module' => 'inviter', 'controller' => 'oauth', 'action' => 'access', 'provider' => 'vk'), 'default', 1);
      $redirect_url = $host_url . $url;
      $auth_url = $vkApi->getAuthUrl($redirect_url);
      $this->redirect($auth_url);
      ...
      $url = $this->view->url(array('module' => 'inviter', 'controller' => 'oauth', 'action' => 'access', 'provider' => $this->provider), 'default', 1);
      $redirect_url = $host_url . $url;
      $logout_url = 'https://foursquare.com/oauth2/authorize?client_id=' . $client_id . '&response_type=code&redirect_uri=' . $redirect_url;
      ...
      $url = $this->view->url(array('module' => 'inviter', 'controller' => 'oauth', 'action' => 'access', 'provider' => 'mailru'), 'default', 1);
      $redirect_url = $host_url . $url;
      ...
      $logout_url = 'https://auth.mail.ru/cgi-bin/logout?next=1&Page=' . urlencode($auth_url);
    ...
    } elseif ($this->provider == 'gmail') {

      if ($new_token && $tokenRow) {
        $tokenRow->delete();
      }

      $redirectUrl = $host_url . $this->view->url(array('module' => 'inviter', 'controller' => 'oauth', 'action' => 'access', 'provider' => 'gmail', 'format' => null, 'new' => null), 'default');
      $plugin = new Inviter_Plugin_Gmail();
      $plugin->setRedirectUrl($redirectUrl);
      $this->redirect($plugin->getAuthUrl());
      ...
      $redirectUrl = $host_url . $this->view->url(array('module' => 'inviter', 'controller' => 'oauth', 'action' => 'access', 'provider' => 'orkut', 'format' => null, 'new' => null), 'default');
      $plugin = new Inviter_Plugin_Gmail();
      $plugin->setRedirectUrl($redirectUrl);
      $this->redirect($plugin->getAuthUrl());
    }
  ...
  public function accessAction()
  {
    ...
    if (isset($params['oauth_problem']) && $params['oauth_problem'] == 'user_refused'
      && $this->provider == 'linkedin'
    ) {
      $url = $this->view->url(array('module' => 'inviter', 'controller' => 'oauth', 'action' => 'callback', 'provider' => $this->provider, 'logout' => true), 'default');
      $this->redirect($host_url . $url);
      return;
    }
    ...
    if ($this->provider == 'facebook') {
      ...
      if ($code) {
        ...
        if ((!$token))
          $this->redirect($host_url . $this->view->url(array('module' => 'inviter', 'controller' => 'oauth', 'action' => 'request', 'provider' => $this->provider), 'default'));
      }

    } elseif ($this->provider == 'hotmail') {
      $code = $this->_getParam('code', null);
      if ($code) {
        $redirectUrl = $host_url . $this->view->url(array('module' => 'inviter', 'controller' => 'oauth', 'action' => 'access', 'provider' => $this->provider, 'format' => null), 'default', 1);
        $token = $this->hotmailPlugin->getAccessToken($code, $redirectUrl);
      } else {
      ...
      if (!$old) {
        ...
        if ($code) {
          $url = $this->view->url(array('module' => 'inviter', 'controller' => 'oauth', 'action' => 'access', 'provider' => $this->provider), 'default');
          $redirect_url = $host_url . $url;
          $plugin = Engine_Api::_()->loadClass('Inviter_Plugin_Linkedin');
          $plugin->init();
          $token = $plugin->getAccessToken($code, $redirect_url);

          $plugin->getSomeInfo($token);
          $plugin->shareSomething($token);
        }
      } else {
        $tokenRow = $this->tokensTbl->findUserToken($viewer->getIdentity(), $this->providerApi->checkProvider($this->provider));
      ...
      if ($error) {
        $redirectUrl = $host_url . $this->view->url(array('module' => 'inviter', 'controller' => 'oauth', 'action' => 'callback', 'provider' => 'gmail', 'format' => null, 'exit' => 'true'), 'default');
        $this->redirect($redirectUrl);
      }
      if ($code) {
        $redirectUrl = $host_url . $this->view->url(array('module' => 'inviter', 'controller' => 'oauth', 'action' => 'access', 'provider' => 'gmail', 'format' => null), 'default');
        $plugin = new Inviter_Plugin_Gmail();
        $token = $plugin->getAccessToken($redirectUrl, $code);
      } else {
        exit('Invalid callback request. Oops. Sorry.');
      }
    ...
    if ($account_info === false) {
      ...
      $this->redirect($this->view->url($params, 'default'));
    }
  ...
  public function callbackAction()
  {
    ...
    $host_url = (_ENGINE_SSL ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'];
    $logout = $this->_getParam('logout', false);

    if ($this->provider == 'gmail' && $this->_getParam('exit')) {
      $this->view->exit = true;
      return;
    }
    ...
    if ($contacts === false) {
      ...
      $this->redirect($host_url . $this->view->url(array('module' => 'inviter', 'controller' => 'oauth', 'action' => 'request', 'provider' => $this->provider, 'empty' => 1), 'default'));
      return;
    }
  ...
  private function _generateForm($tokenRow, $empty = 0)
  {
    ...
    $object_name = ($tokenRow->provider != 'gmail' && $tokenRow->provider != 'yahoo') ? $tokenRow->object_name : "{$tokenRow->object_name}";

    switch ($empty) {
      case 1:
        $lang = 'INVITER_FORM_CONFIRM_ACCOUNT_DESC_NO_CONTACTS';
        break;
      case 2:
        $lang = 'INVITER_FORM_CONFIRM_ACCOUNT_DESC_NO_CONTACTS';
        break;
      default:
        $lang = 'INVITER_FORM_CONFIRM_ACCOUNT_DESC';
    }

    $form->setDescription($this->view->translate($lang, $object_name));
    ...
    if ($provider == 'hotmail') {
      $params = array('module' => 'inviter', 'controller' => 'oauth', 'action' => 'request');
      $params['provider'] = $provider;
      $params['new'] = 1;

      $redirect = $host_url . $this->view->url($params, 'default', 1);

      $logout = $this->hotmailPlugin->logout($redirect);
      $form->addElement('Cancel', 'cancel', array(
        'label' => 'INVTTER_Use another account',
        'link' => true,
        'prependText' => Zend_Registry::get('Zend_Translate')->_(' or '),
        'href' => "javascript:void(0);",
        'onclick' => "inviter.live_logout('" . $logout . "');",
        'decorators' => array(
          'ViewHelper'
        )
      ));
    } elseif ($provider == 'mailru' || $provider == 'mail.ru') {
      $click = 'mailru.connect.logout();';
      $click .= "setTimeout(function() {location.href='{$url}'}, 1000);";
      $form->addElement('Cancel', 'cancel', array(
        'label' => 'INVTTER_Use another account',
        'link' => true,
        'prependText' => Zend_Registry::get('Zend_Translate')->_(' or '),
  ...
  private function _getAccountInfo($token)
  {
    ...
    switch ($this->provider) {
      ...
      case 'gmail':
        $plugin = new Inviter_Plugin_Gmail();
        $accouintInfo = $plugin->getAccountInfo($token);
        $user_info = array_merge($user_info, $accouintInfo);
        break;
      ...
      case 'yahoo':
        ...
        $client = $token->getHttpClient($this->config);
        $client->setUri('https://query.yahooapis.com/v1/yql');
        $client->setMethod(Zend_Http_Client::GET);
        ...
        $user_info['oauth_token'] = $token->getParam('oauth_token');
        $user_info['oauth_token_secret'] = $token->getParam('oauth_token_secret');
        $user_info['object_id'] = $yahoo_account['guid'];
        $user_info['object_name'] = $yahoo_account['nickname'];

        break;

      case 'hotmail':
        $userInfo = $this->hotmailPlugin->getUserInfo($token);
  ...
  private function _getFacebookRequest($tokenRow)
  {
    ...
    if ($new_token) {
      ...
      $this->redirect($facebook->getLogoutUrl(array(
        'display' => 'popup',
    ...
    if (isset($access_token) && $access_token) {
      ...
      $this->redirect($host_url . $this->view->url($next, 'default'), array('exit' => true));
    } else {
  ...
  private function _getHotmailRequest($tokenRow, $new_token)
  {
    ...
    $settings = Engine_Api::_()->getApi('settings', 'core');
    $clientId = $settings->getSetting('inviter.hotmail.consumer.key', false);

    $scope = "wl.signin wl.basic wl.contacts_emails";
    $scope = rawurlencode($scope);
    $redirectUrl = $host_url . $this->view->url(array('module' => 'inviter', 'controller' => 'oauth', 'action' => 'access', 'provider' => $this->provider), 'default', 1);

    if ($tokenRow && $new_token) {
      $tokenRow->delete();
    }

    $authUrl = "https://login.live.com/oauth20_authorize.srf?client_id={$clientId}&scope={$scope}&response_type=code&redirect_uri={$redirectUrl}";
    $this->redirect($authUrl, array('exit' => true));
  }
