    ...
    if ($subject && $subject->getType()) {
      ...
      if ($subject_type == 'user') {
        ...
        $select = $db->select()
                      ...
                      ->joinLeft(array('ufv' => 'engine4_user_fields_values'), 'hm.tip_type = \'user\' AND ufv.field_id = hm.tip_id', array('value', 'privacy'))
        ...
        foreach($items as $item){
          if(!empty($item['privacy']) && $relationship != 'self'){
            if($item['privacy'] == 'self' || ($item['privacy'] == 'friends' && $relationship != 'friends') || ($item['privacy'] == 'registered' && $relationship == 'everyone')){
              continue;
            }
          }
