...
<?php elseif($this->step == 'details'): ?>
  ...
  <?php if (!empty($this->item->description)):?>
    <div style="margin-top:5px;">
      <?php echo Engine_String::substr(Engine_String::strip_tags($this->item->getDescription()), 0, 90);
            echo (Engine_String::strlen($this->item->getDescription())>89)? "...":'';
      ?>
    </div>
  <?php endif; ?>
...
<?php elseif($this->step == 'more_link'): ?>
  ...
  <div align="right">
    <a href="<?php echo $this->url(array('module'=>'quizzes'), 'default', true); ?>" style="text-decoration:underline;color:<?php echo $this->linkColor; ?>">
      <?php echo $this->translate('UPDATES_More quizzes...'); ?>
    </a>
  </div>
