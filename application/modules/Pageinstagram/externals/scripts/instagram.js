var instagram = {
    ...
    more_photos_show_for_users_random :function(page_id,random){

        var ids_on_page = $$('.instagram_id').get('value');
    ...
    if(!random){
        ...
        (new Request.HTML({
            ...
            data: {page: page , page_id: page_id,random: random, ids: ids_on_page},
            onSuccess: function (responseTree, responseElements, responseHTML, responseJavaScript) {
    ...
    more_photos :function(){
        ...
        var next_url = $('next_url').get('html');

        (new Request.HTML({
            url: en4.core.baseUrl + 'page-instagram?format=html',
            data: { tag : tag, page: page,next_url: next_url},
            onSuccess: function (responseTree, responseElements, responseHTML, responseJavaScript) {
                var test = new Element('div',{'html':responseHTML});
                test.getElements('li').each(function(el) {
                    el.inject($('photos_instagram'));
                });
                $('next_url').set('html',test.querySelector('#next_url').get('html'));
                $$('.no_check').removeEvents().addEvent('click',function(e){
