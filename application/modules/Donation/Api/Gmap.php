...
class Donation_Api_Gmap extends Core_Api_Abstract
{
  ...
  public function getMarker($address)
  {
    ...
    if ( ($result = file_get_contents($url)) != false ){
      ...
      $marker->latitude = $coordinates->results[0]->geometry->location->lat;;
      $marker->longitude = $coordinates->results[0]->geometry->location->lng;
    }
    return $marker;
  }
  ...
  public function getGMapUrl($address)
  {
    ...
    return 'http://maps.googleapis.com/maps/api/geocode/json?sensor=true&address=' . $address . '&output=csv';
  }
