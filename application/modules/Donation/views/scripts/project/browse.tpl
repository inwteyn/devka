...
<?php if( $this->paginator->getTotalItemCount() > 0 ): ?>
  <ul class="projects HeCubeView" >
    <?php foreach($this->paginator as $donation):?>
      <?php if($counter == 3): ?>
        <li class="third" style="height: 430px;">
        <?php $counter = 0; ?>
      <?php else: ?>
        <li style="height: 430px;">
    <?php endif; ?>
  <div class="photo">
      ...
      <?php if (strtotime($donation->expiry_date)):?>
        <div class="raised">
          ...
          <?php
            ...
            $year = (int)$left->format('%y');
            $month = (int)$left->format('%m');
            $day = (int)$left->format('%d');
          ?>
          <span>
            <?php if($year > 0){?>
              <?php echo $this->translate(array("%s year", "%s years", $year), $year);?>
            <?php }?>
            <?php if($month > 0): ?>
