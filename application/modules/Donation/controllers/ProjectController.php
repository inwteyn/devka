...
// Set photo
      ...
      if ($donation->save()) {
        ...
        $address = urlencode($address);
          $url = 'http://maps.googleapis.com/maps/api/geocode/json?sensor=true&address=' . $address . '&output=csv';
          if (($result = file_get_contents($url)) != false) {
              $coordinates = json_decode($result);//Engine_Api::_()->getApi('core', 'donation')->str_getcsv($result);
              $donationMarker = $donation->getMarker(true);
              if ($coordinates->status == 'OK') {
                  $donationMarker->latitude = $coordinates->results[0]->geometry->location->lat;
                  $donationMarker->longitude = $coordinates->results[0]->geometry->location->lng;
              } else {
...
// Make form
  ...
  public function fininfoAction()
  {
    ...
    $email = $donation->getPayPalEmail();
    $form->populate(array('pemail' => $email));
    $this->view->form = $form;
    ...
    try {
      ...
      $fininfos = Engine_Api::_()->getDbTable('fininfos', 'donation');
      $select  = $fininfos->select()->where('donation_id = ?',$donation_id);
      $row = $fininfos->fetchRow($select);
      if($row) {
        $row->pemail = $values['pemail'];
        $row->donation_id = $values['donation_id'];
      }else{
        $row_table= Engine_Api::_()->getDbTable('fininfos', 'donation');
        $row = $row_table->createRow();
