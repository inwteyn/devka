...
class Page_Widget_BrowsePagesController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
    ...
    $params['ipp'] = $settings->getSetting('page.browse_count', 10);
    $params['fields']['profile_type'] =(int) ( (isset($params['category']) && $params['category']) ? $params['category'] : 0 );
    ...
    $params_pin = $params['ipp'];
    $params['page'] = $request->getParam('page_pin', 1);
    $this->view->paginator_pin = $p_pin =  $table->getPaginator($params);
    $this->view->not =   $request->getParam('not', 0);
    $count_page_paginator =  ceil($p_pin->getTotalItemCount()/$params_pin);
    $this->view->pagin_true = 0;
    if($count_page_paginator<=$params['page']){
      $this->view->pagin_true = 1;
    }
