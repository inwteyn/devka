...
//            $file = $tmpRow->temporary();
    ...
    public function getMarker($insert = false)
    {
        ...
        $marker = $markersTbl->fetchAll($select);
...
// New Timeline Page
    ...
    public function getCategory()
    {
      ...
      LIMIT 1";
        ...
        $row = Engine_Db_Table::getDefaultAdapter()->query($q)->fetchAll();
        return ($row[0]) ? $row[0] : array('cat_id' => 0);
    }

    public function setAdditionalAddresses($addresses, $additional_coordinates)
    {
        Engine_Api::_()->getDbTable('addresses', 'page')->setPageAddresses($addresses, $this->getIdentity(), $additional_coordinates);
    }

    public function getAdditionalAddresses()
    {
        return Engine_Api::_()->getDbTable('addresses', 'page')->getPageAddresses($this->getIdentity());
    }

    public function getAdditionalAddressesText()
    {
        $addresses = '';

        $page_addresses = Engine_Api::_()->getDbTable('addresses', 'page')->getPageAddresses($this->getIdentity());

        foreach ($page_addresses as $address) {
            $array = array();
            ...
            if ($address['street']) {
                array_push($array, $address['street']);
            }
            if ($address['state']) {
                array_push($array, $address['state']);
            }
