...
window.he_p_timeline_hack_mouse_down = false;

function restyleCoverWidgets() {
    var widgets = $$('.pages_content_widget_timeline\\.new-cover.pages_content_buildable');
    widgets.each(function (el) {
        if (!el.hasClass('he-ready')) {
            ...
            el.getElement('span.pages_layoutbox_widget_tabbed_overtext').set('text', 'Adds a container with a tab menu. Any other blocks you drop inside it will become tabs.');
            el.addClass('he-ready');
        }
    });
}
function restyleFeedWidgets() {
    var widgets = $$('.pages_content_widget_timeline\\.new-feed.pages_content_buildable');
    widgets.each(function (el) {
        if (!el.hasClass('he-ready')) {
...
window.addEvent('domready', function () {
    $$('.pages_content_widget_timeline\\.new-cover').each(function (el) {
        el.addClass('pages_content_widget_core.container-tabs');
    });
    $$('.pages_content_widget_timeline\\.new-feed').each(function (el) {
        el.addClass('pages_content_widget_core.container-tabs');
    });


    document.body.addEvent('mouseup', function (e) {
        if (!window.he_p_timeline_hack_mouse_down)
            return;
        if (e.target.hasClass('pages_content_widget_timeline.new-cover')) {
            setTimeout(function () {
                restyleCoverWidgets();
            }, 100);
        }
        if (e.target.hasClass('pages_content_widget_timeline.new-feed')) {
            setTimeout(function () {
                restyleFeedWidgets();
            }, 100);
        }
    ...
    $$('.admin_content_stock_draggable').each(function (el) {
        el.addEvent('mousedown', function (e) {
            window.he_p_timeline_hack_mouse_down = true;
        });
    });
...
var pushModification = function(type) {
    if( !currentModifications.contains(type) ) {
        $('pages_layoutbox_menu_savechanges').getElement('a').setStyle('font-style', 'italic');
...
var saveChanges = function()
{
    ...
    var request = new Request.JSON({
        ...
        onComplete : function(responseJSON) {
            ...
            $('pages_layoutbox_menu_savechanges').getElement('a').setStyle('font-style', '');
        }
    });
...
var expandSpecialBlock = function(element)
{
    if( element.hasClass('pages_content_widget_core.container-tabs') ) {
        /*var class_cover = 'pages_content_widget_timeline.new-cover';
         var class_feed = 'pages_content_widget_timeline.new-feed';
         if(element.hasClass(class_cover)) {

         } else if(element.hasClass(class_feed)) {
...
echo $this->render('_page_options_menu.tpl'); ?>
