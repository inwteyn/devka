...
class Page_PageController extends Core_Controller_Action_Standard
{
    ...
    public function editAction()
    {
        ...
        $product_result = $hecoreApi->checkProduct('page');
        if (isset($product_result['result']) && !$product_result['result']) {
            ...
            return $this->_helper->redirector->gotoRoute(array(), 'page_browse');
        ...
        $bounds = Engine_Api::_()->getApi('gmap', 'page')->getMapBounds($pageMarker);
        $markers = Zend_Json_Encoder::encode($pageMarker);
        ...
        $values_for_populate = $page->toArray();
        $iterator = 1;
        foreach ($page->getAdditionalAddresses() as $additional_addresses) {
            $values_for_populate['additional_country_' . $iterator] = $additional_addresses['country'];
            $values_for_populate['additional_state_' . $iterator] = $additional_addresses['state'];
            $values_for_populate['additional_city_' . $iterator] = $additional_addresses['city'];
            $values_for_populate['additional_street_' . $iterator++] = $additional_addresses['street'];
        }

        $form->populate($values_for_populate);
        ...
        if ($table->checkUrl($values['url']) && $values['url'] != $page->url) {
        ...
        try {
            ...
            for ($i = 1; $i < 5; $i++) {
                $address = array($values['additional_country_' . $i], $values['additional_state_' . $i], $values['additional_city_' . $i], $values['additional_street_' . $i]);
                array_push($additional_addresses, $address);
                array_push($additional_coordinates, $values['coordinates_' . $i]);
            }

            $page->setAdditionalAddresses($additional_addresses, $additional_coordinates);
