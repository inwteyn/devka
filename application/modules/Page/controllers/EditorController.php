...
class Page_EditorController extends Core_Controller_Action_Standard
{
    ...
    public function indexAction()
    {
        ...
        $this->view->isForse = $isForse = ($settings->getSetting('timeline.usageonpage', 'choise') == 'force') ? 1 : 0;
        if($isForse) {
            $istimeline = true;
        }
        ...
        $flag = ($istimeline && $pageObject->timeline_converted) || $isForse;

        $select = $contentTable->select()
            ->where('page_id = ?', $pageObject->page_id)
            ->where('name NOT LIKE ?', 'hecontest%')
            ->where('is_timeline = ?', $flag)
            ->order('order ASC');
    ...
    public function updateAction()
    {
        ...
        try {
            ...
            $select = $contentTable->select()->where('page_id = ?', $pageObject->page_id)->where('is_timeline = ?', $istimeline);
            $contentRowset = $contentTable->fetchAll($select);
    ...
    protected function _convertToTimeline($page = null)
    {
        ...
        if ($row) {
            ...
            if ($tabs) {
                foreach ($tabs as $tab) {
                    $tab = $tab->toArray();
                    if($tab['name'] == 'page.feed') continue;
                    $tab['is_timeline'] = 1;
