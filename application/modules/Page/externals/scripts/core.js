...
//pin view
array = [0, 0, 0];
function pinfeed_page(options) {
    start = 0;
    if ($$('.paginationControl')) $$('.paginationControl').setStyle('display', 'none');


    column_count = 3;

    var block_arr = [];
    for (var i = 0; i < column_count; block_arr[i++] = $('pin_page' + (i)));
    ...
    for (var k = start; k < options.item.length; k++) {
        ...
        array[min_col] += parseInt(options.item[k].getComputedSize().height);
    }
    start = options.item.length;
    if ($('temp_page_el')) $('temp_page_el').set('html', '');


}
function loader_pin(stat) {
    if (stat == 1) {
        $('pin-loader').setStyle('display', 'block');
    } else {
        $('pin-loader').setStyle('display', 'none');
    }
}
...
//        }
    ...
    getPages: function () {
        ...
        new Request.HTML({
            ...
            onSuccess: function (responseTree, responseElements, responseHTML, responseJavaScript) {
                ...
                if (self.tag_id) {
                    if ($('tag_' + self.tag_id)) {
                        $('page_tag_info').innerHTML = '<span class="bold">' + self.truncate($('tag_' + self.tag_id).innerHTML.split('<')[0], 10) + '</span> ' + en4.core.language.translate('tag') + '. [<a href="javascript:void(0)" class="bold" onClick="page_manager.setTag(0);">x</a>]';
                        $('page_tag_info').removeClass('hidden');
                    }
                } else if (self.sort_type == 'tag') {
                ...
                } else if (self.sort_type == 'location') {
                ...
                } else if (self.sort_type == 'category') {
    ...
    setView: function (view, el) {
        ...
        if ($type(el) == 'element') {
            $$('.pages-view-types').removeClass('he_active');
            el.addClass('active');
        }
        ...
        if ($('page_list_cont') != null) {
            if (view == 'map') {
                ...
                $('page_icons_cont').setStyle('display', 'none');
                $$('.paginationControl').setStyle('display', 'block');
                ...
                pinfeed_page(options);
                if (send_request_pin != 1) {
                    if(!window.page_pin_index){
                        window.page_pin_index = 2;
                        window.load_pin_status = true;
                    }

                    window.addEventListener('scroll',window.pin_load_page = function(){
                        if (window.getScrollTop() + 5 >= window.getScrollSize().y - window.getSize().y-500 && window.load_pin_status && window.view == 'icons') self.setPage(window.page_pin_index);
                    })
    ...
    setPage: function (page) {
        if (this.view == 'list') {
            ...
            var self = this;
            var data = self.getData();
            console.log(window.page_pin_index);
            data.page_pin = page;
            data.not = 1;
            window.onscroll = '';
            window.load_pin_status = false;
            loader_pin(1);
            new Request.HTML({
                url: self.widget_url,
                data: data,
                ...
                onSuccess: function (responseTree, responseElements, responseHTML, responseJavaScript) {
                    ...
                    $('temp_page_el').set('html', responseHTML)
                    var options = {
                        autoResize: true, // This will auto-update the layout when the browser window is resized.
                        container: $('pinfeed'),
                        item: $('temp_page_el').getElements('.pin_items'),
                        offset: 2,
                        itemWidth: 255,
                        bottom: 1
                    ...
                    if (send_request_pin != 1) {
                        window.load_pin_status = true;
                        window.page_pin_index =  window.page_pin_index+1;
...
var list_like = {
    ...
    show_page_loader: function(id) {
        $$('.list_page_status_' + id).addClass('hidden');
        $$('.list_page_loader_like_' + id).removeClass('hidden');
    },

    hide_page_loader: function(id, liked) {
        $$('.list_page_status_' + id).removeClass('hidden');
        $$('.list_page_loader_like_' + id).addClass('hidden');

        var $item = $$('.list_page_status_' + id).getParent('.item');
        if (liked) { 
    ...
    init_list_like_buttons:function(key){
        if ($$('div.page_list_browser_likebox a.list_unlike')) {
            var $btns = $$('.page_list_browser_likebox a.list_unlike');
            $btns.removeEvents('click');
            $btns.each(function($item){
                if ($item.hasClass('list_unlike')){
                    $item.addEvent('click', function(){
                        var id = this.id;
                        var guid = id.substr(11);
                        var info = guid.split('_');
                        if (info.length == 3){
                            self.show_page_loader(info[2]);
                            self.page_list_unlike(info[0]+'_'+info[1], info[2], $item, function(liked){
                                self.hide_page_loader(info[2], liked);
                            }, key)
                        }else{
                            self.show_page_loader(info[1]);
                            self.page_list_unlike(info[0], info[1], $item, function(liked){
        ...
        if ($$('div.page_list_browser_likebox a.list_like')) {
            var self = this;
            var $btns = $$('.page_list_browser_likebox a.list_like');
            $btns.removeEvents('click');
            $btns.each(function($item){
                if ($item.hasClass('list_like')){
                    $item.addEvent('click', function(){
                            var id = this.id;
                            var guid = id.substr(9);
                            var info = guid.split('_');
                            if(info.length == 3){
                                self.show_page_loader(info[2])
                                self.page_list_like(info[0]+'_'+info[1], info[2], $item, function(liked){
                                    self.hide_page_loader(info[2], liked);
                                }, key)
                            }else{
                                self.show_page_loader(info[1]);
                                self.page_list_like(info[0], info[1], $item, function(liked){
                                    self.hide_page_loader(info[1], liked);
                                }, key)
                            }
                        }
    ...
    page_list_like: function(object, object_id, $element, callback, key){
        var self = this;
        $element = $($element);
        if (this.block){
            return false;
        ...
        new Request.JSON({
            ...
            onSuccess: function(response){
                ...
                if($('listUnlike_'+object+'_'+object_id) || $('listLike_'+object+'_'+object_id)){
                    $('listUnlike_'+object+'_'+object_id).getParent().setStyle('display', 'block');
                    $('listLike_'+object+'_'+object_id).getParent().setStyle('display', 'none');
                }
                if($('popuUnlike_'+object+'_'+object_id) || $('popuLike_'+object+'_'+object_id)){
                    $('popuUnlike_'+object+'_'+object_id).getParent().setStyle('display', 'block');
                    $('popuLike_'+object+'_'+object_id).getParent().setStyle('display', 'none');
                }
                if($('featUnlike_'+object+'_'+object_id) || $('featLike_'+object+'_'+object_id)){
                    $('featUnlike_'+object+'_'+object_id).getParent().setStyle('display', 'block');
                    $('featLike_'+object+'_'+object_id).getParent().setStyle('display', 'none');
                }
                if($('recnUnlike_'+object+'_'+object_id) || $('recnLike_'+object+'_'+object_id)){
                    $('recnUnlike_'+object+'_'+object_id).getParent().setStyle('display', 'block');
                    $('recnLike_'+object+'_'+object_id).getParent().setStyle('display', 'none');
                }
                if($('sponUnlike_'+object+'_'+object_id) || $('sponLike_'+object+'_'+object_id)){
                    $('sponUnlike_'+object+'_'+object_id).getParent().setStyle('display', 'block');
                    $('sponLike_'+object+'_'+object_id).getParent().setStyle('display', 'none');
                }
                if($('pageUnlike_'+object+'_'+object_id) || $('pageLike_'+object+'_'+object_id)){
                    $('pageUnlike_'+object+'_'+object_id).getParent().setStyle('display', 'block');
                    $('pageLike_'+object+'_'+object_id).getParent().setStyle('display', 'none');
                }
                if($('mapsUnlike_'+object+'_'+object_id) || $('mapsLike_'+object+'_'+object_id)){
                    $('mapsUnlike_'+object+'_'+object_id).getParent().setStyle('display', 'block');
                    $('mapsLike_'+object+'_'+object_id).getParent().setStyle('display', 'none');

                    if( typeof key != 'undefined' && $('page_map_like_' + key)) {
                        pages_map.markers_array[key].map_like = $('page_map_like_' + key);
                    }
    ...
    page_list_unlike: function(object, object_id, $element, callback, key) {
        var self = this;
        $element = $($element);
        if (this.block){
            return false;
        ...
        new Request.JSON({
            ...
            onSuccess: function(response){
                ...
                if($('listUnlike_'+object+'_'+object_id) || $('listLike_'+object+'_'+object_id)){
                    $('listUnlike_'+object+'_'+object_id).getParent().setStyle('display', 'none');
                    $('listLike_'+object+'_'+object_id).getParent().setStyle('display', 'block');
                }
                if($('popuUnlike_'+object+'_'+object_id) || $('popuLike_'+object+'_'+object_id)){
                    $('popuUnlike_'+object+'_'+object_id).getParent().setStyle('display', 'none');
                    $('popuLike_'+object+'_'+object_id).getParent().setStyle('display', 'block');
                }
                if($('featUnlike_'+object+'_'+object_id) || $('featLike_'+object+'_'+object_id)){
                    $('featUnlike_'+object+'_'+object_id).getParent().setStyle('display', 'none');
                    $('featLike_'+object+'_'+object_id).getParent().setStyle('display', 'block');
                }
                if($('recnUnlike_'+object+'_'+object_id) || $('recnLike_'+object+'_'+object_id)){
                    $('recnUnlike_'+object+'_'+object_id).getParent().setStyle('display', 'none');
                    $('recnLike_'+object+'_'+object_id).getParent().setStyle('display', 'block');
                }
                if($('sponUnlike_'+object+'_'+object_id) || $('sponLike_'+object+'_'+object_id)){
                    $('sponUnlike_'+object+'_'+object_id).getParent().setStyle('display', 'none');
                    $('sponLike_'+object+'_'+object_id).getParent().setStyle('display', 'block');
                }
                if($('pageUnlike_'+object+'_'+object_id) || $('pageLike_'+object+'_'+object_id)){
                    $('pageUnlike_'+object+'_'+object_id).getParent().setStyle('display', 'none');
                    $('pageLike_'+object+'_'+object_id).getParent().setStyle('display', 'block');
                }
                if($('mapsUnlike_'+object+'_'+object_id) || $('mapsLike_'+object+'_'+object_id)){
                    $('mapsUnlike_'+object+'_'+object_id).getParent().setStyle('display', 'none');
                    $('mapsLike_'+object+'_'+object_id).getParent().setStyle('display', 'block');
