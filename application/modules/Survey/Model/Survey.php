 ...
 * @version    $Id: Survey.php 2010-07-02 19:53 ermek $
 * @author     Ermek
 */
...
class Survey_Model_Survey extends Core_Model_Item_Abstract
{
  ...
  public function getHref($params = array())
  {
    ...
    $params = array_merge(array('route' => 'survey_view', 'reset' => true, 'survey_id' => $this->survey_id, 'slug' => $slug), $params);
  ...
  public function getTitle($truncate = false, $length = 50)
  {
    return $this->title;
  }
  ...
  public function setPhoto($photo)
  {
    if ($photo instanceof Zend_Form_Element_File) {
      ...
      throw new Survey_Model_Exception('invalid argument passed to setPhoto');
    }
  ...
  public function getResultList()
  {
    ...
    $select = $table->select()
      ->where('survey_id = ?', $this->getIdentity())
      ->order('result_id ASC');
  ...
  public function getQuestionList($fetch_answers = false)
  {
    ...
    $table = Engine_Api::_()->getDbtable('answers', 'survey');
    $select = $table->select()
  ...
  {
    $survey_info = array('result_count' => 0, 'question_count' => 0, 'answer_count' => 0);
        
    $resultTable = Engine_Api::_()->getDbtable('results', 'survey');
    $select = $resultTable->select()
      ->from($resultTable->info('name'), array('result_id'))
      ->where("survey_id = ?", $this->getIdentity());

    $result_ids = $resultTable->getAdapter()->fetchCol($select);
    $survey_info['result_count'] = count($result_ids);

    if ($survey_info['result_count'] == 0) {
      return $survey_info;
    }

    $questionTable = Engine_Api::_()->getDbtable('questions', 'survey');
    ...
    $survey_info['question_count'] = count($question_ids);
    
    if ($survey_info['question_count'] == 0) {
      return $survey_info;
    }
    ...
    $survey_info['answer_count'] = $table->getAdapter()->fetchOne($select);
  ...
  public function getUserResult($user_id) 
  {
    ...
    $select = $table->select()
      ->where("survey_id = ?", $this->getIdentity(), 'INTEGER')
  ...
  {
    return Engine_Api::_()->getDbtable('categories', 'survey')->find($this->category_id)->current();
  }
  ...
  public function userMatches($user_id, $level = 1)
  {
    ...
    $select = $userTable->select()
      ...
      ->where("$choicesTable.survey_id = ?", $this->getIdentity())
  ...
  public function isCompleted()
  {
    ...
    $minResultCount = $settings->getSetting('surveyzes.min.result.count', 2);
    $minQuestionCount = $settings->getSetting('surveyzes.min.question.count', 1);
