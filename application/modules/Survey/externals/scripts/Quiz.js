...
/* $Id: Survey.js 2010-05-25 01:44 ermek $ */
...
var survey =
{
  ...
  manage_result: function()
  {
    ...
    $$('.view_survey_result .delete_result_btn').addEvent('click', function()
    {
      var $result_node = $(this).getParent('.view_survey_result');
    ...
    $$('.view_survey_result .edit_result_btn').addEvent('click', function()
    {
      var $result_node = $(this).getParent('.view_survey_result');
      var result_id = $result_node.getProperty('id').substr(7);
    ...
    en4.core.request.send(new Request.JSON({
      ...
      onSuccess : function(result){
        ...
        if (result && result.status) {
          ...
          var result_count = $$('.view_survey_results .view_survey_result').length;
  ...
  manage_question: function()
  {
    ...
    $$('.view_survey_question .delete_question_btn').addEvent('click', function()
    {
      var $question_node = $(this).getParent('.view_survey_question');
      ...
      var title = self.translate('survey_Are you sure you want to delete this question?');
    ...
    $$('.view_survey_question .edit_question_btn').addEvent('click', function()
    {
      var $question_node = $(this).getParent('.view_survey_question');
      var question_id = $question_node.getProperty('id').substr(9);
    ...
    en4.core.request.send(new Request.JSON({
      ...
      onSuccess : function(question){
        ...
        if (question && question.status) {
          ...
          var question_count = $$('.view_survey_questions .view_survey_question').length;
  ...
  {
    if (step_info && step_info.next_error && $('survey_next_btn')) {
      $('survey_next_btn').addEvent('click', function() {
        he_show_message(step_info.message, 'error');
      });
    } else if (step_info && !step_info.next_error && step_info.next && $('survey_next_btn')) {
      $('survey_next_btn').addEvent('click', function() {
    ...
    var $disabled_pages = $$('.survey_tabs .disabled');
  ...
  take_survey: function()
  {
    ...
    this.$questions = $$('.take_survey .survey_question');
    this.count = this.$questions.length;
    this.$container = $$('.take_survey .form-elements')[0];
    this.$question_number = $('answered_questions');
    this.$progress_status = $$('.survey_progress_bar .progress_status');
    this.$progress_line = $$('.survey_progress_bar .progress_line')[0];
    ...
    this.$container.getElements('.form-options-wrapper .survey_answer').setProperty('checked', false);
    ...
    this.$container.getElements('.form-options-wrapper li').addEvent('click', function()
    {
      ...
      this.getElement('.survey_answer').setProperty('checked', true);
      ...
      window.setTimeout(function(){
        ...
        if (self.check_take_result(true)) {
          ...
          he_show_message(self.translate('You successfully passed the survey.'));
  ...
  get_take_result: function()
  {
    if (!this.check_take_result()) {
      he_show_message(this.translate('survey_Please answer for all questions!'), 'error');
      return false;
    }
    
    he_show_message(this.translate('You successfully passed the survey.'));
  ...
  update_progress_bar: function()
  {
    ...
    if (percent > 0) {
      $$('.survey_progress_bar .progress_invite_text').dispose();
    }
  ...
  view_survey: function(survey_id)
  {
    var self = this;
    ...
    this.$survey_tabs = $$('.view_survey_tabs .survey_tab');
    this.$survey_contents = $$('.survey_content');

    this.$survey_tabs.addEvent('click', function()
    {
      var $node = $(this);
      ...
      $$('.view_survey_tabs li.active').removeClass('active');
      self.$survey_contents.addClass('display_none');
