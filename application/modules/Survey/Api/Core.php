...
class Survey_Api_Core extends Core_Api_Abstract
{  
  /**
   * Gets a paginator for surveyzes
   *
   * @param Core_Model_Item_Abstract $user The user to get the messages for
   * @return Zend_Paginator
   */
  public function getSurveyzesPaginator($params = array())
  {
    $paginator = Zend_Paginator::factory($this->getSurveyzesSelect($params));
  ...
  /**
   * Gets a select object for the user's surveyzes
   *
   * @param Core_Model_Item_Abstract $user The user to get the messages for
   * @return Zend_Db_Table_Select
   */
  public function getSurveyzesSelect($params = array())
  {
    ...
    if (!empty($params['tag'])) {
      $select
        ...
        ->from($rName)
        ->joinLeft($tmName, "$tmName.resource_id = $rName.survey_id")
        ->where($tmName . '.resource_type = ?', 'survey')
        ->where($tmName . '.survey_id = ?', $params['tag']);
    }
  ...
  /**
   * Returns a collection of all the categories in the survey plugin
   *
   * @return Zend_Db_Table_Select
   */
  public function getCategories()
  {
    return Engine_Api::_()->getDbtable('categories', 'survey')->fetchAll();
  }
  ...
  {
    return Engine_Api::_()->getDbtable('categories', 'survey')->find( $category_id)->current();
  }

  public function getSurveyTakers($parameters)
  {
    ...
    if (isset($parameters['list_type']) && $parameters['list_type'] == 'mutual') {
      ...
      $select = $userTable->select()
        ...
        ->where("$takersTable.survey_id = ?", $parameters['survey_id'], 'INTEGER')
