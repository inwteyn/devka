	...
	<div class="right">
		...
		<?php if ($friendCount > 0): ?>
			<div class="likes">
				...
				<div class="item">
					<?php echo $this->htmlLink($user->getHref(), $this->itemPhoto($user, 'thumb.icon', '', array('width' => '100px', 'height' => '100px')), array('class' => 'wall_liketips wp_init like_action_tip_'.$this->object->getGuid(), 'id' => $this->object->getGuid().'_like_action_item_'.$user->getGuid(),'rev'=>'user_'.$user->getIdentity())); ?>
				</div>
				<?php endforeach; ?>
				<div class="clr"></div>
			</div>
