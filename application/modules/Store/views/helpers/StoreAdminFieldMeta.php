 ...
 * @author     John
 */
class Store_View_Helper_StoreAdminFieldMeta extends Zend_View_Helper_Abstract
{
  public function storeAdminFieldMeta($map)
  {
    $meta = $map->getChild();
  ...
  <div class="field_extraoptions_contents_wrapper">
    <div class="field_extraoptions_contents">
      <div class="field_extraoptions_add">
        {$this->view->formText('text', '', array('title' => $this->view->translate(''), 'onkeypress' => 'void(0);',  'onmousedown' => "void(0);"))}
      </div>
EOF;
...
EOF;
        foreach( $options as $option ) {
          $dependentFieldContent .= $this->view->storeAdminFieldOption($option, $map);
        }
      }
