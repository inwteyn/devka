...
class Credit_Api_Core extends Core_Api_Abstract
{
  ...
  public function updateItemCredits(User_Model_User $user, $object)
  {
    ...
    } elseif ($action->action_type == 'album_photo') {
      $object_type = 'album_photo';
      $object_id = $object->photo_id;
    } else {
      return 0;
    }
  ...
  public function buyHighlight($user, $credits)
  {
    $actionTypes = Engine_Api::_()->getDbTable('actionTypes', 'credit');
    $table = Engine_Api::_()->getDbTable('logs', 'credit');

    $senderBalance = Engine_Api::_()->getItem('credit_balance', $user->getIdentity());
    if (!$senderBalance) {
      $senderBalance = Engine_Api::_()->getItemTable('credit_balance')->createRow();
      $senderBalance->balance_id = $user->getIdentity();
      $senderBalance->save();
    }

    $buyHighlight = $actionTypes->getActionType('buy_highlight');

    $row = $table->createRow();
    $row->user_id = $user->getIdentity();
    $row->action_id = $buyHighlight->action_id;
    $row->credit = $credits;
    $row->object_type = '';
    $row->object_id = 0;
    $row->creation_date = new Zend_Db_Expr('NOW()');
  ...
  public function updatePageVisitCredits($page)
  {
    ...
    if (!$logsTable->checkCredit($actionType, $visitor->getIdentity())) {
      return 0;
    }
  ...
  public function buyEmoticon(User_Model_User $buyer, $credits)
  {
    /**
     * @var $table Credit_Model_DbTable_Logs
     * @var $actionTypes Credit_Model_DbTable_ActionTypes
    ...
    $buy_emoticon = $actionTypes->getActionType('buy_emoticon');
