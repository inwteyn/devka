    ...
    renderActions: function (actions, end) {
      ...
      if (actions.length) {
        ...
        for (var i = 0; i < actions.length; i++) {
          ...
          var body = $(actionEl).find('.body');
          var poster = $(actionEl).find('.poster');
          var feed_item_bodytext = body.find('.feed_item_bodytext')[0];
          if(typeof feed_item_bodytext !== 'undefined'){
                poster.insertBefore(feed_item_bodytext);
          }
...
Wall.services = {
    ...
    logout: function (service, $this) {
        $.post(core.baseUrl + 'wall/' + service + '/logout', {format: 'json'}, function(obj){
            $this.prevAll('.wall-share-' + service).removeClass('active');
            $this.hide();
            $.mobile.hidePageLoadingMsg();
        });
    }
...
Wall.composerAlbum = {
  ...
  init: function () {
    var self = this;
    self.createAlbum = $('#feed-create-album-button');
    self.cancelAlbum = $('#feed-album-cancel');
    self.albumPhoto = $('#feed-album-photo');
    self.wrapper = $('#feed-album-photos');
    self.capture = $('#capture');
    self.cancelVideo = $('#feed-video-cancel');
    self.bindEvents();

    self.createAlbum.css('display', '');
  },

  bindEvents: function () {
    var self = this;
    self.createAlbum.unbind('click').bind('click', function (e) {
      $('#photoFakeButton').css('display', 'none');
      $('#feed-create-album-wrapper').css('display', 'block');
      $('#isalbum').val(1);

      self.createAlbum.css('display', 'none');
      self.cancelAlbum.css('display', '');


      $(document).find('.feed-body').css('display', 'none');
      $(document).find('.feed-people.content.content_compose').css('display', 'none');
      $(document).find('.feed-checkin.content.content_compose').css('display', 'none');

      $(document).find('.share_services').css('display', 'none');

    });

    self.cancelAlbum.unbind('click').bind('click', function () {
      $('#photoFakeButton').css('display', 'block');
      $('#feed-create-album-wrapper').css('display', 'none');
      $('#isalbum').val(0);

      self.createAlbum.css('display', '');
      self.cancelAlbum.css('display', 'none');

      $('#feed_album_name').val('');
      $('#feed_album_description').val('');

      self.wrapper.find('.remove_preview_item').click();
      $(document).find('.feed-body').css('display', '');
      $(document).find('.feed-people.content.content_compose').css('display', '');
      $(document).find('.feed-checkin.content.content_compose').css('display', '');

      $(document).find('.share_services').css('display', '');
    });
    self.cancelVideo.unbind('click').bind('click', function () {
//      $(document).find('#feed-video-upload').attr('action', '');
      $('#captureFakeButton').css('display', 'block');
      $('#captureFakeButton').find('.ui-btn-text').text('From my device');
      self.capture.val("");
      $(document).find('#feed-create-video-wrapper').css('display', 'none');
      $(document).find('.feed-body').css('display', '');
      $(document).find('.feed-video').css('display', '');
      $(this).css('display', 'none');

      $('#feed_video_name').val('');
      $('#feed_video_description').val('');

      self.wrapper.find('.remove_preview_item').click();
      $(document).find('.feed-body').css('display', '');
      $(document).find('.feed-people.content.content_compose').css('display', '');
      $(document).find('.feed-checkin.content.content_compose').css('display', '');
      $(document).find('.ui-header .compose').css('display', '');
      $(document).find('.share_services').css('display', '');
    });
    self.capture.bind('change', function(){
        var params = wall_form.getParams();
//        $(document).find('#feed-video-upload').attr('action', params.composer.videoFullUploaderUrl);
        $(document).find('.feed-people.content.content_compose').css('display', 'none');
        $(document).find('.feed-checkin.content.content_compose').css('display', 'none');
        $(document).find('.feed-video').css('display', 'none');
        $(document).find('#feed-create-video-wrapper').css('display', '');
        $(document).find('.feed-body').css('display', 'none');
        self.cancelVideo.css('display', '');
        wall_form.setAttachedType('video');
        $('#isUploadVideo').val(1);
        $(document).find('.ui-header .compose').css('display', 'none');
    });
    self.albumPhoto.bind('change', self.change);
  },
  ...
  change: function (e) {
    ...
    for (var i = 0, f; f = files[i]; i++) {
      ...
      reader.onload = (function (theFile) {
        return function (e) {
          Wall.composerAlbum.createPreview(e.target.result, theFile);
        };
      })(f);
    ...
    setTimeout(function () {
      var inputs = $('#feed-album-photoFakeButton').find('input');
      var newElement = inputs[inputs.length - 1];
      $(newElement).bind('change', Wall.composerAlbum.change);
    },50);

  },

  createPreview: function (src, file) {
    var self = this;
    var div = $('<div class="feed-album-photo-preview"></div>');
    div.css('background-image', 'url('+src+')');
    div.attr('data-name', file.name);
    div.attr('data-size', file.size);
    var rW = $('<div class="feed-album-photo-remove"></div>');
  ...
  remove:function(el) {
    var parent = $(el).closest('.feed-album-photo-preview');

    var name = parent.attr('data-name');
    var size = parent.attr('data-size');
    var str = name+':'+size+'|';

    $('#remove').val($('#remove').val() + str);
    parent.remove();
  }
};
  ...
  composeInit: function (e) {
    ...
    $(this.form.getServices()).each(function (key, value) {
      ...
      $share.show().find('.wall-share-' + value).show().attr('href', service.url).data('type', value).attr('target', '_blank').unbind().bind('vclick', function (event) {
       var service = $(this).attr('rev');
       // to login
        if (window.wall_form.params.services[$(this).data('type')].enabled) {
            if($(this).hasClass('active')){
                $(this).nextAll('.provider_logout').hide();
            }else{
                $(this).nextAll('.provider_logout').show().bind('vclick', function(){
                    $.mobile.showPageLoadingMsg();
                    window.wall_form.params.services[service].enabled = false;
                    Wall.services.logout(service, $(this));
                });
            }
        }
...
//      Wall.customAnimate($wm.find('.shareForm').show());
    ...
    activate: function (type) {
      ...
      if(type != 'video'){
          $composer.find('#feed-video-upload').addClass('display_none');
      }else{
          $composer.find('#feed-video-upload').removeClass('display_none');
      }
    ...
    getForm: function () {
      return this.getComposer().find('#main-composer-form');
    },
    ...
    resetAttachments: function () {
      ...
      $.each(plugins, function (index, item) {
        ...
        try {
          ...
          self.getForm().find('#feed-create-video-wrapper').hide();
          self.getForm().find('.feed-body').css('display', '');
          self.getForm().find('#isuploadvideo').val(0);
...
//        $composer.find('.feed-link').find('.image_choose').hide();
    ...
    video: {
      ...
      click: function () {
        ...
        $.mobile.showPageLoadingMsg();
        if(type == 3){
            var $form = $composer.find('#feed-video-upload');
            var video_name = $form.find('#feed_video_name').val();
            var video_desc = $form.find('#feed_video_description').val();
            var isUploadVideo = $('#isUploadVideo').val();
            var fileinput = document.getElementById('capture');

            var file = fileinput.files[0];
            var formData = new FormData();
            formData.append('file', file);
            formData.append('title', video_name);
            formData.append('description', video_desc);
            formData.append('type', 3);
            formData.append('isUploadVideo', isUploadVideo);
            ...
            request.onreadystatechange = function(){
              ...
              if(resp) {
                  ...
                  if(resp.status){
                      alert('Your video is now in converting. We will send you a notification when it is ready to view.');
                  }else if(typeof resp.status === 'undefined'){
                      alert('Your file probably exceeds the defined ini size.');
                  }
                  else{
                      alert('You have already uploaded the maximum number of videos allowed.');
                  }
                $.mobile.hidePageLoadingMsg();
                Wall.events._hideModalContainer();
          ...
          request.open('POST', params.composer.videoFullUploaderUrl);
...
//            $list.empty();
          ...
          injectChoice: function (index, value) {
            ...
            win._apactoid = setTimeout(function () {
              $("#add_people").blur()
            }, 400);
            return $li;
          },
...
//          }
        ...
        $page.find('.checkinDetect').unbind().bind('vclick', function () {
          ...
          if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
              ...
              function (error) {
                alert(core.lang.get('APPTOUCH_GEOLOCATION_ERROR_' + error.code));
                $.mobile.hidePageLoadingMsg();
    ...
    submit: function (e) {
      ...
      $.post(url, data, function (obj) {
        ...
        var $li = $(obj.action.html);
        var body = $li.find('.body');
        var poster = $li.find('.poster');
        var feed_item_bodytext = body.find('.feed_item_bodytext')[0];
        if(typeof feed_item_bodytext !== 'undefined'){
           poster.insertBefore(feed_item_bodytext);
        }
...
Wall.heemoticons = {
    ...
    getStickers: function($template) {
        var self = this;
        var $heemoticon = $template.find('.heemoticon');
        var url = $($heemoticon).attr('href');
        $($heemoticon).removeAttr('href');
        var $heemoticons_tab = $template.find('.smiles_tab');
        var $heemoticons_body = $template.find('.stickers_body');
        $.post(url, {
            format: 'json'
        }, function(response){
            $.mobile.hidePageLoadingMsg();
            $heemoticons_body.append("<div class='smiles_container smiles_colection_standart'><div class='wall_data_comment'></div></div>");
            var $smiles_container = $heemoticons_body.find('.smiles_container .wall_data_comment');
            $.each(response.standart, function(i, item){
                if(i == 0){
                    $heemoticons_tab.append("<a href='javascript:void(0)' onclick='Wall.heemoticons.showEmoticonById(-1,this);' class='heemoticons_smiles ui-btn-active'>" + item.html + "</a>");
                }
                $smiles_container.append("<a href='javascript:void(0);'  class='smiles_standart' rev=" + item.index_tag + ">" + item.html + "</a>");
            });
            $.each(response.titles, function(i, item){
                $heemoticons_tab.append("<a href='javascript:void(0)' onclick='Wall.heemoticons.showEmoticonById("+ item.collection_id +",this);' class='heemoticons_smiles'><img src=" + item.cover + "/></a>");

            });
            $.each(response.emoticons, function(i, item){
                var smiles_id = item.collection_id;
                $heemoticons_body.append("<div id='smile_id_"+smiles_id+ "'  class='smiles_container smiles_new display_none smiles_id_" + smiles_id + "'><div class='wall_data_comment'></div></div>");
                var $smiles_container_new = $heemoticons_body.find('#smile_id_' + smiles_id);
                var $smiles_container_block = $smiles_container_new.find('.wall_data_comment');
                $.each(item.smiles, function(j, smiles){
                        $smiles_container_block.append("<a href='javascript:void(0);' data-id=" + smiles.id + " class='smiles_NEW'><img src=" + smiles.url + "></a>");
                });
            });

            $smiles_container.find('.smiles_standart').bind('vclick', function(){
               var body = $template.find('form textarea').val();
               $template.find('form textarea').val(body + ' ' + $(this).attr('rev') + ' ');
            });

            $heemoticons_body.find('.smiles_NEW').bind('vclick', function(e){
                $.mobile.showPageLoadingMsg();
                var $this = $(this);
                if($template.hasClass('dialog_reply')){
                    window.postReplySticker(e, $this);
                }else{
                    window.postSticker(e, $this);
                }
    ...
    showEmoticonById: function(id, element){
        $(element).addClass('ui-btn-active').siblings().removeClass('ui-btn-active');
        var $smiles_container = $('.smiles_container');
        $smiles_container.addClass('display_none');
        if (id == -1) {
            $('.smiles_colection_standart').removeClass('display_none');
        } else {
            var cont = $('.smiles_id_' + id);
            cont.removeClass('display_none');
        }
    }
};