...
//      });
  ...
  cache:{
    ...
    clearDialog:function (except) {
      console.log('All dialog has been cleared');
      var rh = renderer.helper;
      if ('string' == typeof except)
          rh.removeElements($(document.querySelectorAll('.ui-dialog')));
      else
          rh.removeElements($(document.querySelectorAll('.ui-dialog')));
    },
...
//      console.log(response);
    ...
    boardIn:function ($fromPage) {
      if(window._dashboardScroll){
          window._dashboardScroll.destroy();
      }
...
//        $board.data('iscroll').disable();
    ...
    setIconBadgeNumber:function (name, num, bgcolor) {
      ...
      if (!document.querySelector(selector))
        return;
      if(name == 'messages'){
          selector = [' .messages_main_inbox .ui-btn-inner span.ui-btn-text:after', selector].join();
      }
...
var renderer = {
  ...
  _bindEvents:function () {
    var $doc = $(document);
    $doc.bind ('pageshow', function (e, jQPageData) {
        var activePage = $.mobile.activePage;
        var pageId = jQPageData.prevPage.attr('id');
        if((activePage.attr('id') == 'core_board_index_rewrite' || activePage.attr('id') == 'core_board_index_action_id_rewrite') && (pageId.indexOf('activity_index_view_action_id_rewrite_activity_action_') >= 0 || pageId == 'blog_blogs_manage_rewrite')){
            setTimeout(function(){
                $(document.querySelectorAll('.ui-dialog')).removeClass('ui-page-active');
            },400);
...
//    } Hide Modal container
      ...
      if (opt.reloadPage) {
        ...
        try{
            if($($tp).is('#core_board_index_rewrite') || $($tp).is('#activity_notifications_index_rewrite') || $($tp).is('#messages_messages_inbox_rewrite')){
                setTimeout(function () {
                    $($tp[0]).addClass('ui-page-active');
                }, 700 );
            }
        }catch(e){
        }
        ...
        if (response.message) {
          if ($.type(response.message) === 'array') {
            ...
            for (var i = 0; i < response.message.length; i++) {
              setTimeout(function () {
                $.mobile.showPageLoadingMsg('b', $(document.createElement('p')).html(response.message[i]).text(), true);
              }, minWait * i);
            }
          } else {
            wait = minWait;
            $.mobile.showPageLoadingMsg('b', $(document.createElement('p')).html(response.message).text(), true);
          }
        }
