    ...
    activity: {
        ...
        notificationsIndex: function (page, jQPageData, $page) {
            ...
            $switcher.find('.showNotifications').bind('vclick', function (e) {
                ...
                $('#activity_notifications_index_rewrite .component-paginator').show();
            })
...
//                $form.find('#all').bind('touchstart', function (event) {
//                    alert(112);
...
//
//                });
                $form.find('#all-element').bind('vmouseup', (function(){
                    var checkboxes = $form.find('.ui-checkbox label').not($(this).find('label'));
                    checkboxes.trigger('vclick');
                }));
            }
        }
    },
    ...
    hegift: {
        indexSend: function (page, jQPageData, $page) {
            ...
            if ($page.find('#gift_price').length) {
                ...
                $page.find('label').bind('vclick', function () {
                    var cost = parseInt($page.find('#gift_price').html());
                    var current_balance = parseInt($page.find('#current_balance').html());
                    if ($(this).hasClass('ui-checkbox-on')) {
                        if (cost + price <= credits) {
                            $page.find('#gift_price').html(cost + price);
                            $page.find('#current_balance').html(current_balance - price);
                        } else {
    ...
    inviter: {
        indexIndex: function (page, jQPageData, $page) {
            //FB load
            core.helper.loadScript(window.location.protocol + '//connect.facebook.net/en_US/all.js', function () {
                FB.init({
...
//          link: '', // link to be attached to each result
    ...
    store: {
        ...
        panelAddress: function(page, jQPageData, $page){
            var countries = $page.find('#country-element select');
            var states = $page.find('#state-element #state');
            countries.change(function(){
                    var parent_id = $(this).val();
                    var url = "/store/panel/address";
                    $.post(url, {
                        'format': 'json',
                        'just_locations': 1,
                        'parent_id': parent_id
                    }, function(response){
                        var html = response.page.layout.content[0].params;
                        if(html){
                            $('#state-element').html(html);
                            $('#state-element select').addClass('store_states');
                        }
                    })
                }
        ...
        indexCreate: function (page, jQPageData, $page) {
            ...
            var video = {
                ...
                attach: function () {
                    ...
                    var lastMatch = '';
                    var video_element = $page.find('#type');
                    body.unbind().bind('change', function () {
...
//            }
    ...
    donation: {
        donationDonate: function(page, jQPageData, $page)
        {
            var anon = false;
            var name =  page.info.donation.name;
            var email = '';
            var amount = null;
            var donation_text = '';
            var min_amount = page.info.donation.min_amount;
            var object_id = page.info.donation.object_id;
            var gateway_id = 0;
            var selected_id = 0;

            $page.find('#submit').unbind('vclick').bind('vclick', function(e){
                gateway_id = $(this).attr('data-id');
                if ($page.find('#anon')) {
                    anon = $page.find('#anon').prop("checked");
                    if(anon){
                        if($page.find('#name')){
                ...
                if(selected_id && selected_id != 'own'){
                    amount = selected_id;
                }
                else{
                    var $active = $page.find('#inputOwn');
                    if($active && $active.hasClass('active')){
                        amount = $active.val();
                    }
                }
                donation_text = $page.find('#donation_text').val();
                $page.find('#submit').attr('disabled', true);

                $.ajax({
                    url: page.info.donation.donate_url,
                    type: 'post',
                    data: {
                        'format': 'json',
                        'anon': anon,
                    ...
                    success: function( response ) {
                        ...
                        window.setTimeout(function(){
                            $page.find('#submit').attr('disabled',false);
                        },3000);
                    },
                    dataType: 'json'
                });
            ...
            $page.find('#inputOwn').unbind('vclick').bind('click', function(e){
                $page.find('.donation_select > .btn.active').removeClass('active');
                $(this).addClass('active');
            });
            $page.find('.donation_select > .btn').unbind('vclick').bind('vclick', function(e){
                $page.find('.donation_select > .btn.active').removeClass('active');
                $page.find('#inputOwn').removeClass('active');
                $(this).addClass('active');
                selected_id = $(this).attr('id');
            });
        },

        donationProcess: function(page, jQPageData, $page)
        {
            $page.find('form').submit();
        }
    }
