UIComponent = {
  ...
  dashboard: function (params, $template) {
    ...
    for (var i = 0; i < to; i++) {
      ...
      if (attrs['class']  && core.helper.isExternalUrl(attrs['href'])) {
  ...
  subjectPhoto: function (params, $template) {
    ...
    if (subject) {
      if (subject.photo.profile) {
        $template.find('a').attr('href', subject.href);
        $template.find('img').attr('src', subject.photo.profile);
      }
//      if(!subject.cover_photo){
//          $template.find('.subject_photo_cover').css('display', 'none');
//      }
      $template.find('.subject_title').html(subject.title);
      $template.find('.subject_status').html(subject.status);
      $('.component-heEventCover.page-component').find('.subject_title').remove();
    }
...
//    $form.find('select').attr('data-native-menu', 'false');
    ...
    var $formAdvOptions = $form.find('#adv_options-wrapper');
    if ($formAdvOptions) {
      $formAdvOptions.find('.ui-icon').remove();
    }
...
//      $calendars[order].type = 'text';
    ...
    $form.find('#advOptions-element').click(function () {
      $form.find('#search-wrapper, #auth_view-wrapper, #auth_comment-wrapper, #auth_tag-wrapper').toggle('fast');
    });
    ...
    if ($fileInputs.length > 0) {
      ...
      if (core.device.getGrade() == 'a')
        var multiUpload = function ($fileInput, isPicup, data) {
          var filesArray = document.getElementById($fileInput.attr('id')).files;
          ...
          if (!isPicup) {
            ...
            if (val) {
              if (filesArray.length > 1) {
                $file.find('.value').html("List of images").after('<span class="file-list-icon ui-icon-caret-down">&nbsp;</span>');
                var names = '';
                for (var j = 0; j < filesArray.length; j++) {
                  names += '<span>' + filesArray[j].name + '</span><br>';
                }
                $file.after('<div class="file-list-block" style="display: none">' + names + '</div>');
                $file.unbind('vclick').bind('vclick', function (e) {
                  $file.next().slideToggle('fast');
                });
              } else {
          ...
          $file.find('.delete').unbind('click').bind('click', function () {
            var $this = $(this);
        ...
        for (var i = 0; i < $fileInputs.length; i++) {
          ...
          $fileInput.bind('change', function () {
            ...
            if (name.indexOf('[]') > -1) {
              ...
              $($this[0].form).find(['#', name, 'FakeButton .ui-btn-text'].join('')).html($this.attr('value'));
            }
          });
          $fakeButton.unbind();
  ...
  html: function (params, $template) {
    ...
    $template.find('.storeHtmlPrice').parent('.component-html').addClass('storePriceParent');
  ...
  inviter: function (params, $template) {
    ...
    for (var key in params.items) {
      ...
      if (item.provider == 'hotmail') {
        $item.find('a').attr('data-ajax', false);
      }
      if (item.provider != 'facebook') {
        if (item.photoUrl)
...
//            $div.append($item);
  ...
  timelineCover: function (params, $template) {
    ...
    if (params.user) {
      ...
      $template.find('.subject_status').html(params.user.status);
    }
...
//        }
  ...
  creditCheckout: function (params, $template) {
    ...
    if (params.no_credit_lang) {
      var message = $('<div></div>').html(params.no_credit_lang);
      message.addClass('apptouch_no_credit_class');
      $template.find('a#cart-checkout-confirm').parent().html(
        message
      );
...
//    var is_manage_list = (this.pageResponseData.info.type == 'manage');
    ...
    for (var key in params.items) {
      ...
      if (item.friendshipButton) {
        $item.find('.friendshipButton').css('display', 'block');
        $item.find('.friendshipButton').html(item.friendshipButton);
      }
      ...
      if (!item.manage || item.manage.length == 0 || !item.manage[0].label)
        ...
        $link.data('item', item);
        if(item.manage[0].attrs) {
          $link.attr(item.manage[0].attrs);
        } else {
          $link.attr('href', item.manage[0].href);
          $link.attr('class', item.manage[0].class);
          $link.attr('data-icon', item.manage[0].data_icon);
        }
        $link.html(item.manage[0].label);
      } else {
        var dialog_id = 'apptouch_local_manage_' + parseInt(Math.random() * 1000000);
        var $dialog = this.layout.$createBase('dialog', {
          'data-rel': 'dialog',
        ...
        if (core.helper.isTablet()) {
          $link.attr('data-icon', item['manage-icon']);
        }
        $link.attr('href', '#' + dialog_id);
        $link.data('item', item);
...
//    } empty html
    for (var key in photos) {
      ...
      var bgi = ['url(', subjectPhoto.profile, ')'].join('');
...
//      }
  ...
  registryWall: function (params, $template) {
    ...
    try {
      ...
      ins = new ActivityFeed($feed, params);
      if (window.__pageData.info.key == 'user_index_home_rewrite') {
    ...
    var emptyLi = $('#main-feed-composer-panel #wall_form_checkin')[0];
    if (typeof emptyLi === 'undefined') {
      setTimeout(function () {
          $('#main-feed-composer-panel li').css('width', '48%');
          $('#main-feed-composer-panel li:nth-child(2)').css('border-right', 'none');
        }, 10
      )
    }
    ...
    // Main Composer
    var MainFnVClickWEF = function (e) {
      Wall.events.showComposerPanel($cl);
      this._FnVClickWEF = FnVClickWEF;
      this._FnVClickWEF(e);
      delete this._FnVClickWEF;
    }
    var $mainComposerBtns = $template.find('.main-feed-composer-panel .wall-event-form');
    $mainComposerBtns.unbind('click', MainFnVClickWEF);
    $mainComposerBtns.bind('click', MainFnVClickWEF);
  ...
  feed: function (params, $template) {
    ...
    setTimeout(function () {
      var feedItem = $('.feedItem').find('.touch_wall_grouped_other');
      if (feedItem) {
        feedItem.bind('vclick', (function () {
          $(this).next('.touch_wall_grouped_other_html').toggle('fast');
        }));
      }
    }, 100);
  ...
  comments: function (params, $template) {
    ...
    var $btn_share = $widget.find('.share_this');
    var $btn_report = $widget.find('.report_this');
    var $btn_embed = $widget.find('.embed_this');
    var $ul_comments = $widget.find('ul.comments');
    var $heemoticon = $widget.find('.heemoticon');
    var $heemoticon_tab = $widget.find('.stickers_tab .sticker_close_panel');
    var $comment_photo = $widget.find('#comment-attach-photo');
    var $comments_reply = $widget.find('.comment_reply_block');
    $('.module-activity #main-feed-composer-panel').css('display', 'none');
...
//      $li.find('div.comment-like').controlgroup().controlgroup('refresh');
    ...
    var share = function($share){
        ...
        if(type != 'activity_action'){
            $btn_share.removeClass('display_none');
            var url = $share.attr('href');
            $.mobile.showPageLoadingMsg();
            url = url+'/type/'+ type +'/id/'+subject.id;
            $share.attr('href', url);
        }
    };
    ...
    var report = function($report){
        var type = subject.type;
        if(type != 'activity_action'){
            $btn_report.removeClass('display_none');
            var url = $report.attr('href');
            $.mobile.showPageLoadingMsg();
            url = url+'/subject/' + type +'_'+subject.id+'/id/'+subject.id;
            $report.attr('href', url);
        }
    };

    var video_embed = function($embed){
        var type = subject.type;
        if(type != 'activity_action'){
            $btn_embed.removeClass('display_none');
            var url = $embed.attr('href');
            $.mobile.showPageLoadingMsg();
            url = url+'/id/' + subject.id;
            $embed.attr('href', url);
        }
    };
    ...
    var comment_like_unlike = function ($li, comment) {
      ...
      var is_reply = $('#is_reply');
      var $reply = $li.find('.reply-comment');
      var $cancel_edit = $widget.find('.cancel-edit-button');
      var is_edit = $('#is_edit');
      var to_user = $('#to_user');
      $reply.unbind().bind('vclick', function(e){
            $form.find('.ui-btn-inner .ui-btn-text').text(core.lang.get('Reply Comment'));
            $form.find('.ui-btn').css('width', '130px');
            $form.find('.ui-btn .ui-btn-inner').css('padding-left', '10px');
            $cancel_edit.removeClass('display_none');
            $form.find("#body").scroll().focus();
            is_reply.val(comment.id);
            to_user.val(comment.poster.id);
        });
      $cancel_edit.unbind().bind('vclick', function(e){
        is_edit.val(0);
        is_reply.val(0);
        to_user.val(0);
        $form.find('.ui-btn').css('width', '220px');
        $form.find('.ui-btn-inner .ui-btn-text').text(core.lang.get('Post Comment'));
        setTimeout(function () {
            $form.find('.ui-btn').removeClass('ui-btn-hover-c');
            $form.find('.ui-btn').removeClass('ui-btn-down-c');
        }, 100);
        $('.remove_preview_item').closest('.comment-photo-preview').remove();
        $form.find("#body").val('');
        $widget.find('.comment').css({'opacity':'1', 'pointer-events': 'all'});
        $comment_photo.prop('disabled', false);
        $(this).addClass('display_none');
      });
      //        delete if allowed
      if (comment.options && comment.options['delete']) {
        var $delete_comment = $li.find('.comment-like').find('.delete-comment');
        var $edit_comment = $li.find('.comment-like').find('.edit-comment');
        var comment_clone = $li.attr('id', 'comment-' + comment.id ).clone();
        $edit_comment.unbind('vclick');
        $edit_comment.bind('vclick', function(e){
            var body = comment.body;
            $form.find('.ui-btn-inner .ui-btn-text').text(core.lang.get('Edit Comment'));
            $form.find('.ui-btn').css('width', '130px');
            $form.find('.ui-btn .ui-btn-inner').css('padding-left', '10px');
            $cancel_edit.removeClass('display_none');
            if(comment_clone.find('.comment_photo')[0]){
                var comment_photo = comment_clone.find('.comment_photo').clone();
                var photo_id = comment_photo.attr('href').split('/').pop();
                var img_photo = comment_photo.find('img');
                var src = img_photo.attr('src');
                createCommentPreview(null, src, comment_photo[0], photo_id);
                var comment_body = comment_clone.find('.comment-body').text();
                body = comment_body;
            }
            $form.find("#body").val(body).scroll().focus();
            is_edit.val(comment.id);
            $widget.find('.comment').css({'opacity':'0.3', 'pointer-events': 'none'});
        });
      ...
      // todo shit code
        ...
        $li.find('.comment_reply_block').bind('vclick', function(){
          var is_reply = $('#is_reply');
          var to_user = $('#to_user');
    ...
    var create_comment_item = function ($tpl, comment) {
      ...
      setTimeout(function () {
         $('.comments .smiles_NEW').closest('.ui-btn-text').find('.edit-comment').css('display','none');
      }, 100);
      if(is_edit != 0){
            var $edited_li = $ul_comments.find('#comment-' + is_edit);
            if($edited_li[0]){
                $edited_li.find('.comment-body').html(comment.body);
                return $edited_li;
            }else{
                $edited_li = $('ul.comments_reply').find('#comment-' + is_edit);
                $edited_li.find('.comment-body').html(comment.body);
                return $edited_li;
            }
      }
    ...
    $heemoticon.bind('vclick', function(e){
        var stikers_panel = $widget.find('.stickers_panel');
        if(stikers_panel.hasClass('display_none')){
            stikers_panel.removeClass('display_none');
        }else{
            stikers_panel.addClass('display_none');
        }
    });
    $heemoticon_tab.bind('vclick', function(e){
        $widget.find('.stickers_panel').addClass('display_none');
    });
    // } Functions

    $comment_photo.bind('change', function(e){
        $.mobile.showPageLoadingMsg();
        $widget.find('.stickers_panel').addClass('display_none');
        $heemoticon.css('pointer-events', 'none');
        var url = $widget.find('.comment-photo-button #add_comment_photo').val();
        ...
        request.onreadystatechange = function(){
            if(request.readyState == 4){
                try {
                    ...
                    alert('Oops error in uploading. Try later');
                    $.mobile.hidePageLoadingMsg();
                }
            }
            if(resp) {
                ...
                if(resp.status){
                    createCommentPreview(resp, null, null, null);
                    $widget.find('.cancel-edit-button').css('pointer-events', 'none');
                }
                else{
    ...
    var createCommentPreview = function(resp, src , photo_element, photo_id){
        var div = $('<div class="comment-photo-preview"></div>');
        var img = '';
        if(resp == null){
            div.css('background-image', 'url(' + src + ')');
            img = photo_element;
        }else{
            div.css('background-image', 'url(' + resp.src + ')');
            img = resp.photo_element;
        }

        var rW = $('<div class="comment-photo-remove"></div>');
        var remove = $('<i class="remove_preview_item ui-icon ui-icon-remove"></i>');

        if(resp == null){
            remove.bind('click', function() {
                remove_photo(remove, photo_id);
            });
        }else{
            remove.bind('click', function() {
                remove_photo(remove, resp.photo_id);
            });
        }

        div.append(img);
        div.find('.comment_photo').addClass('display_none');
        rW.append(remove);
        div.append(rW, null);
        $widget.find('.comment-buttons-block').append(div);
        $comment_photo.prop('disabled', true);
    };

    var remove_photo = function(el, photo_id) {
        $.mobile.showPageLoadingMsg();
        var parent = $(el).closest('.comment-photo-preview');
        var url = $widget.find('.comment-photo-button #del_comment_photo').val();
        $.post(url, {
          ...
          'format': 'json'
            ...
            if(response.status){
                $.mobile.hidePageLoadingMsg();
                $comment_photo.prop('disabled', false);
                $heemoticon.css('pointer-events', 'all');
                parent.remove();
            }else{
                alert('Something wrong. Try later.');
            }
    ...
    if (widget.main_form) {
      ...
      $widget.find('.stickers').removeClass('display_none');
    }
    ...
    $form.find('#submit').bind('vclick', function (e) {
      ...
      var img_href = $widget.find('.comment-photo-preview .comment_photo');
      var img_href_attr = img_href.attr('href');
      var img = img_href.find('img');
      var img_src = img.attr('src');
      img_href.removeClass('display_none');
      if(img_href){
          img_href.closest('.comment-photo-preview').remove();
          $comment_photo.prop('disabled', false);
          $heemoticon.css('pointer-events', 'all');
      }
      var is_edit = $('#is_edit');
      var is_reply = $('#is_reply');
      var to_user = $('#to_user');
      if (body.length > 50 || body.split(' ').join('').length || (img_href_attr && img_src)) {
        ...
        $.post($form.attr('action'), {
          ...
          if (response.status) {
            ...
            var $divider = $ul_comments.find('.comment-count');
            $form.find('.ui-btn-inner .ui-btn-text').text(core.lang.get('Post Comment'));
            if(response.comment.resource_id){
                var $reply_li = $ul_comments.find('#comment-' + response.comment.resource_id);
                $item.addClass('reply_li');
                $item.insertAfter($reply_li);
            }else if(is_edit.val() != 0){}
            else{
            ...
            $form.find('.ui-btn').css('width', '220px');
            $widget.find('.comment').css({'opacity':'1', 'pointer-events': 'all'});
            $widget.find('.cancel-edit-button').addClass('display_none');
            $widget.find('.cancel-edit-button').css('pointer-events', 'all');
          }
     ...
     window.postSticker = function(e, $this){
         var is_reply = $('#is_reply');
         var to_user = $('#to_user');
         var src = $this.find('img').attr('src');
         var id = $this.attr('data-id');
         var body = "<a class='smiles_NEW ui-link'><img src=" + src + " sticker_id=" + id +"></a>";
          $.post($form.attr('action'), {
              'format': 'json',
              'id': subject.id,
              'type': subject.type,
      ...
      window.postReplySticker = function(e, $this){
          var is_reply = $('#is_reply');
          var to_user = $('#to_user');
          var src = $this.find('img').attr('src');
          var id = $this.attr('data-id');
          ...
          $.post($form.attr('action'), {
              ...
              if (response.status) {
                  var $item = create_comment_item($li_tpl, response.comment);
                  var $dialog_reply = $('#comment_replies_' + response.comment.resource_id);
                  var $dialog_ul = $dialog_reply.find('.comments_reply');
                  $dialog_ul.append($item);
                  $dialog_ul.listview('refresh');
                  $btn_post_comment.find('a').find('.comment-count').html(response.comment_count);
              }
              $.mobile.hidePageLoadingMsg();
              $widget.find('.stickers_panel').addClass('display_none');
    ...
    if (pageResponseData.info.viewer.id) {
      ...
      var $share = $btn_share.find('a');
      var $report = $btn_report.find('a');
      var $embed = $btn_embed.find('a');
      if(params.can_embed){
          video_embed($embed);
      }
    ...
    if (widget.comments.length > 0){
      ...
      for (var key in widget.comments) {
        ...
        if(comment.is_reply){
            var $reply_li = create_comment_item($li_tpl, comment);
            $reply_li.addClass('reply-' + comment.parent_comment);
            $reply_li.addClass('reply-li');
            $reply_li.hide();
            $ul_comments.append($reply_li);
        }else{
            $ul_comments.append(create_comment_item($li_tpl, comment));
        }

        if(comment.replies_count){
            var $comment_li = $ul_comments.find('#comment-' + comment.id);
            var $comment_reply_block = $comment_li.find('.comment_reply_block ').removeClass('display_none');
            $comment_reply_block.find('a').text(comment.replies_count);
        }
      }
...
// Reply Dialog box
      ...
      for (var key in widget.comments) {
          var comment_reply = widget.comments[key];
          if(comment_reply.replies_count){
              var $comment_li = $ul_comments.find('#comment-' + comment_reply.id);
              var $comment_reply_block = $comment_li.find('.comment_reply_block');
              var $li_parent_reply_tpl = $comment_li.clone();
              var $li_reply_tpl = $ul_comments.find('.reply-' + comment_reply.id);
              var dialog_id = 'comment_replies_' + comment_reply.id;
              var $dialog = self.layout.$createBase('dialog', {
                  'data-rel': 'dialog',
                  'id': dialog_id,
                  'data-url': '#' + dialog_id,
                  'data-title': comment_reply.replies_count
              }, true);
              $dialog.addClass('dialog_reply');
              var $nav_box = $('<ul data-role="listview" class="comments comments_reply" data-theme="c" data-dividertheme="d"></ul>');
              var $form_reply = $('<form enctype="application/x-www-form-urlencoded" method="post" class="reply_form"><textarea name = "reply_body" class="reply_body" placeholder=" '+  core.lang.get('Write reply...') + '"></textarea><button class="reply_button" data-mini="true"> '+ core.lang.get('Reply') + '</button></form>');
              $nav_box.append($li_parent_reply_tpl);
              $nav_box.append('<div>' + core.lang.get('Replies:') + '</div>');
              $li_reply_tpl.each(function(){
                  var el = $(this);
                  el.show();
                  $nav_box.append(el);
              });
              var $buttons = $widget.find('.comment-buttons-block').clone();
              var $stickers_panel = $widget.find('.stickers_panel').clone();
              $buttons.addClass('reply_buttons');
              $dialog.append($nav_box);
              $dialog.append($form_reply);
              $dialog.append($buttons);
              $dialog.append($stickers_panel);
              var $link = $comment_reply_block.find('a.options');
              $link.attr('href', '#' + dialog_id);
              $link.data('item', comment_reply);
              $dialog.find('.comments_reply .edit-comment').bind('vclick', function(){
                  $('.template-dialog .ui-header a').attr('data-icon', 'delete').trigger('click');
              });

              Wall.heemoticons.getStickers($dialog);
          }
      }

     $('.reply_buttons').find('.heemoticon').bind('vclick', function(e){
          if($('.dialog_reply').find('.stickers_panel').hasClass('display_none')){
              $('.dialog_reply').find('.stickers_panel').removeClass('display_none');
          }else{
              $('.dialog_reply').find('.stickers_panel').addClass('display_none');
          }
      });

      var $dialog_reply = $('.dialog_reply');
      var $comment_photo_reply =  $dialog_reply.find('#comment-attach-photo');
      $dialog_reply.find('.ui-header').bind('vclick', function(){
            var is_reply = $('#is_reply');
            var to_user = $('#to_user');
            is_reply.val(0);
            to_user.val(0);
      ...
      var $heemoticon_tab_reply = $dialog_reply.find('.stickers_tab .sticker_close_panel');
      $heemoticon_tab_reply.bind('vclick', function(e){
          $dialog_reply.find('.stickers_panel').addClass('display_none');
      });

      $('.reply_form').find('.reply_button').bind('vclick', function(e){
            var $this = $(e.srcElement);
            var is_reply = $('#is_reply');
            var to_user = $('#to_user');
            var is_edit = $('#is_edit');
            var img_href = $dialog_reply.find('.comment-photo-preview .comment_photo');
            var img_href_attr = img_href.attr('href');
            var img = img_href.find('img');
            var img_src = img.attr('src');
            img_href.removeClass('display_none');
            if(img_href){
              img_href.closest('.comment-photo-preview').remove();
              $comment_photo_reply.prop('disabled', false);
              $heemoticon.css('pointer-events', 'all');
            }
            var body_textarea = $this.parents('.reply_form').find('.reply_body');
            var body = $this.parents('.reply_form').find('.reply_body').val();
            if (body.length > 50 || body.split(' ').join('').length || (img_href_attr && img_src)) {
                $.mobile.showPageLoadingMsg();
                $.post($form.attr('action'), {
                    'format': 'json',
                    ...
                    if(response.status){
                        ...
                        $dialog_ul.append($item);
                        $dialog_ul.listview('refresh');
                        $item.find('.edit-comment').bind('vclick', function(){
                            $('.template-dialog .ui-header a').attr('data-icon', 'delete').trigger('click');
                        });
                        body_textarea.val('');
                        $btn_post_comment.find('a').find('.comment-count').html(response.comment_count);
      ...
      $comment_photo_reply.bind('change', function(e){
            var self = $(this);
            $.mobile.showPageLoadingMsg();
            $dialog_reply.find('.stickers_panel').addClass('display_none');
            $heemoticon.css('pointer-events', 'none');
            ...
            request.onreadystatechange = function(){
                ...
                if(resp) {
                    resp = JSON.parse(resp);
                    if(resp.status){
                        createCommentReplyPreview(resp, null, null, null, self);
                        $dialog_reply.find('.cancel-edit-button').css('pointer-events', 'none');
                    }
                    else{
                        alert('Something wrong. Try later.');
                    }
        ...
        var createCommentReplyPreview = function(resp, src , photo_element, photo_id, self){
            var div = $('<div class="comment-photo-preview"></div>');
            var img = '';
            if(resp == null){
                div.css('background-image', 'url(' + src + ')');
            ...
            if(resp == null){
                remove.bind('click', function() {
                    remove_reply_photo($(this), photo_id);
                });
            }else{
                remove.bind('click', function() {
                    remove_reply_photo($(this), resp.photo_id);
                });
            }
            ...
            rW.append(remove);
            div.append(rW, null);
            var $current_dialog_reply = self.parents('.dialog_reply');
            $current_dialog_reply.find('.comment-buttons-block').append(div);
            $comment_photo_reply.prop('disabled', true);
        };

        var remove_reply_photo = function(el, photo_id) {
            $.mobile.showPageLoadingMsg();
            var parent = el.closest('.comment-photo-preview');
            var url = $dialog_reply.find('.comment-photo-button #del_comment_photo').val();
            $.post(url, {
                'photos_id_del': photo_id,
                'do': 1,
                'format': 'json'
 ...
 // End Reply Dialog box
    }
    if($heemoticon){
          Wall.heemoticons.getStickers($widget);
    }
    //    Handle Actions {
...
//        $template.controlgroup().controlgroup('refresh');
    ...
    /* On page links(share, suggest, manage pri) */
    if (params.menu) {
      var $link = $template.find('.page_links a');
      if (params.menu.length == 0) {
      } else if (params.menu.length == 1) {
        ...
        var $menu_item_div = $template.find('.page_links').clone();
        for (var key in params.menu) {
          var menu_item = params.menu[key];
          var $menu_item = $menu_item_div.clone();
          var $a = $menu_item.find('a');
          if (params.menu.length > 2 && $(window).width() < 340) {
            if (key == 1) {
              var $a_icon = $template.find('.page_icon_plus');
              $a_icon.css('display', '');
              $template.append($a_icon);
              var menu_id = ['apptouch_local_quick_menu_', this.pageResponseData.info.key].join('');
              $a_icon.find('a').attr('href', '#' + menu_id);
              break;
            }
...
//
//      });
      trackAudioElement.addEventListener("ended", playNextAudio);
      ...
      $track.bind('click', function (e) {
        var audioElement = $(this).closest('li');
        playlistItemClick(audioElement);
      });

      $tracks.append($track);
    }
    function playNextAudio() {
      if (core.device.platform.isIOS()) {
        this.pause();
      } else {
        var activeLi = $template.find('li.active');
        playlistItemClick(activeLi.closest('li').next('.track'));
      }
    }
...
//    ------------------ Control Functions ------------------
    var playPause = function ($mediaControls, play) {
      ...
      if (!params || !params.media) {
        $('.tracks li:nth-child(1)').trigger('click');
        return;
      }
...
//    ------------------ Control Functions ------------------
    $template.find('.playBtn').data('playPauseFn', playPause).unbind('mousedown').bind('mousedown', function () {
      var $this = $(this);
      $this.data('playPauseFn')($this.closest('.component-mediaControls'), true);

    });

    $template.find('.pauseBtn').data('playPauseFn', playPause).unbind('mousedown').bind('mousedown', function () {
      var $this = $(this);
...
//        }
  ...
  heEventCover: function (params, $template) {
    ...
    $template.find('.subject_title').html('');
    $template.find('.cover_photo_href').html(cover_photo);
    $template.find('.cover_photo_href').find('img').attr('style',
      'width: 100%; position: relative;' + $template.find('.cover_photo_href').find('img').attr('style')
    );
