...
class Apptouch_View_Helper_ApptouchItemRate extends Engine_View_Helper_HtmlElement
{
  public function apptouchItemRate($item_type, $item_id, $show_score = false, $score_br = true, $period = 'all')
  {
    ...
    if ($item_type == 'page') {
      ...
      if ($rate_info['count']) {
        $count_review = $this->view->translate(array('%s', '%s', $rate_info['count']),  $rate_info['count'] . '</b>');
      }
      $score_str = '';
      if ($show_score) {
        ...
        if($count_review) $score_str = '<div class="pagereview_count">(' . $count_review . ')</div>'.'<div class="he_rate_small_cont"><div class="rate_stars_cont">' . $stars_str;
        else $score_str .= '<div class="pagereview_count">' . $count_review . '</div>';
