...
//  }
    ...
    public function onMenuInitialize_CoreDashboardCometchat($row)
    {
        ...
        if (!$tables || empty($tables) || count($tables) < 1  && !Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('apptouch-cometchat'))
            return false;

        if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('apptouch-cometchat')){
            $view = Zend_Registry::get('Zend_View');
            $url = $view->url(array('action' => 'index'), 'apptouch_cometchat_general', true);
            return array(
                'uri' => $url
            );
        }else{
