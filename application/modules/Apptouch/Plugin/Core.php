...
class Apptouch_Plugin_Core extends Zend_Controller_Plugin_Abstract
{
    ...
    {
      print_log('-------------------------------------------------');
      print_log($_SERVER['HTTP_USER_AGENT']);
      print_log(print_r($request->getParams(), 1));
      print_log('++++++++++++++++++++++++++++++++++++++++++++++++++');
    ...
    protected function ignoreCustomizations(Zend_Controller_Request_Abstract $request)
    {
        ...
        if ($module == 'headvancedalbum') {
            $module = 'album';
            if($request->getActionName() == 'editphotos'){
            }
            //    todo blitz
            $c = $request->getControllerName();
            $a = $request->getActionName();
            if (($a == 'view' || $a == 'editphotos') && $c == 'index') {
        ...
        if ($module == 'sitemember' || $module == 'advancedmembers') {
            $module = 'user';
...
/*        if ($module == 'heevent') {
        ...
        if ($module == 'advancedsearch' || $module == 'siteadvsearch') {
            $module = 'core';
