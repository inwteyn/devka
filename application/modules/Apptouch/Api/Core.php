...
class Apptouch_Api_Core extends Core_Api_Abstract
{
  ...
  {
    if ($this->isApp()) {
      unset($this->supportedModules['hecontest']);
  ...
  public function isApptouchMode(Zend_Controller_Request_Abstract $request = null)
  {
    ...
    if (array_key_exists('HTTP_USER_AGENT', $_SERVER)) {
      ...
      if ($mode != 'standard') {
        ...
        if ($includeTablets || preg_match('/ip(hone|od)|windows (ce|phone)|bb10|android|iemobile\/9.0|iemobile\/10|nokia808pureview|imageuploader/i', $userAgent)) {
          return true;
        } else {
          return false;
        }
...
HTML;
  ...
  public function getStoreToken($flag = false)
  {
    ...
    if (method_exists(Engine_Api::_()->store(), 'getToken')) {
      $token = Engine_Api::_()->store()->getToken($flag);
      return $token;
    }
