  ...
  public function itemList(Zend_Paginator $paginator, $customizerFunction = null, $params = array())
  {
    ...
    if ($paginator->getTotalItemCount())
      foreach ($paginator as $item) {
        ...
        if($item->getType() == 'forum'){
            $last_topic =  $item->getLastUpdatedTopic();
            $last_post = null;
            if( $last_topic ) {
            ...
            $std = array(
                ...
                'href' => $item->getHref(),
                'photo' => $photoUrl,
                'creation_date' => isset($last_post->creation_date) ? $this->view->timestamp(strtotime($last_post->creation_date)) : "",
                'type' => $item->getType(),
            );
        }else{
            $std = array(
...
//    $user = Engine_Api::_()->core()->getSubject();
    ...
    $coverPhoto = Engine_Api::_()->timeline()->getTimelinePhoto($id, $type, 'cover');
    ...
    if ($type != 'page') {
      $component['cover_photo'] = $subject->getTimelinePhoto('cover');
    } else {
      $component['cover_photo'] = '<img id="cover-photo"  src=' . $coverPhoto['photoSrc'] . '>';
    }
    $component['user'] = $this->_bridge->subject($subject);

    $component['choose'] = $this->view->url(array('id' => $id), 'timeline_photo');
    $component['chooseLang'] = $this->view->translate('TIMELINE_Choose from Photos...');

    $component['upload'] = $this->view->url(array('action' => 'upload', 'id' => $id), 'timeline_photo');
    $component['uploadLang'] = $this->view->translate('TIMELINE_Upload Photo...');
    ...
    $component['remove'] = $this->view->url(array('action' => 'remove', 'id' => $id), 'timeline_photo');
    $component['removeLang'] = $this->view->translate('TIMELINE_Remove...');
  ...
  public function heEventCover()
  {
    ...
    if (!$modules->isModuleEnabled('heevent')) return false;
    $viewer = Engine_Api::_()->user()->getViewer();
  ...
  public function video(Core_Model_Item_Abstract $video)
  {
    ...
    if ($video instanceof Video_Model_Video || $video instanceof Pagevideo_Model_Pagevideo || $video instanceof Store_Model_Video) {
      ...
      if ($video->type == 3) {
        ...
        $rC = $video->getRichContent(true);
      if (Engine_Api::_()->apptouch()->isApp()) {
        $pos = strpos($rC, '<script');
        if ($pos > 0) {
          $rC = substr($rC, 0, $pos);
        }
      }
      $videoFormat['flashObject'] = $rC;
    }
...
// } HT
    ...
    if (Engine_Api::_()->getDbTable('modules', 'hecore')->isModuleEnabled('wall') || Engine_Api::_()->getDbTable('modules', 'hecore')->isModuleEnabled('timeline')) {
  ...
  public function comments(array $params = array())
  {
    ...
    if (null !== ($page = $this->_getParam('commentPage'))) {
      ...
      if ($subject->getType() == 'activity_action') {
        if (Engine_Api::_()->hasModuleBootstrap('wall'))
          $subject = Engine_Api::_()->getDbtable('actions', 'wall')->getActionById($subject->getIdentity());
        $comments = $subject->getComments(true);
      } else {
        $commentSelect = $subject->comments()->getCommentSelect();
    ...
    if ($viewer->getIdentity() && $canComment) {
      ...
      if($subject->getType() == 'video'){
          ...
          $widget_content['can_embed'] = $can_embed;
      }
    $comments_format = array();
    if(isset($comments) && $comments && count($comments) )
      foreach ($comments as $comment) {
        ...
        if($type == 'activity_action'){
            if (Engine_Api::_()->hasModuleBootstrap('wall')){
                $commentsReply = Engine_Api::_()->wall()->getCommentReply($action, $comment);
            }
        }else{
            $comments = Engine_Api::_()->getDbtable('comments', 'core');
            $select = $comments->select()->where('resource_type = ?','comment')->where('resource_id=?',$comment->getIdentity());
            $commentsReply = $comments->fetchAll($select);
        }
        ...
        if ($isWall) {
          ...
          $comment->body = $view->wallSmiles()->replaceContent($comment->body);
        }
        if($type == 'activity_action'){
            $canComment = $viewer->getIdentity() && ($subject->authorization()->isAllowed($viewer, 'comment'));
            $canDelete = $viewer->getIdentity() && (('user' == $action->subject_type && $viewer->getIdentity() == $comment->poster_id));
        }else{
            $canComment = $viewer->getIdentity() && ($subject->authorization()->isAllowed($viewer, 'comment'));
            $canDelete = $viewer->getIdentity() && (('user' == $action->subject_type && $viewer->getIdentity() == $action->subject_id) || ($viewer->getIdentity() == $comment->poster_id));
        }
        ...
        if($commentsReply){
            $replies_count = count($commentsReply);
            if($replies_count != 0){
                $comment_format['replies_count'] = $this->view->translate(array('%s reply', '%s replies', $replies_count), $replies_count);
                $comment_format['replies_count_digit'] = $replies_count;
            }
        }
        ...
        if($commentsReply){
              foreach($commentsReply as $reply){
                  if($type == 'activity_action'){
                      $canComment = $viewer->getIdentity() && ($subject->authorization()->isAllowed($viewer, 'comment'));
                      $canDelete = $viewer->getIdentity() && (('user' == $action->subject_type && $viewer->getIdentity() == $reply->poster_id));
                  }else{
                      $canComment = $viewer->getIdentity() && ($subject->authorization()->isAllowed($viewer, 'comment'));
                      $canDelete = $viewer->getIdentity() && (('user' == $action->subject_type && $viewer->getIdentity() == $action->subject_id) || ($viewer->getIdentity() == $reply->poster_id));
                  }
                  $reply['poster_type'] = 'user';
                  $comment_format = array();
                  $comment_format['id'] = $reply->getIdentity();
                  $comment_format['poster'] = $this->_bridge->subject($reply->getPoster());
                  $comment_format['likes'] = $reply->likes();
                  $comment_format['body'] = $reply->body;
                  $comment_format['creation_date'] = $this->view->timestamp($reply->creation_date);
                  $comment_format['options'] = array();
                  $comment_format['is_reply'] = 1;
                  $comment_format['parent_comment'] = $comment->getIdentity();
                  if ($canComment){
                      if ($reply instanceof Core_Model_Comment) {
                          $comment_format['options']['like'] = $reply->likes()->isLike($this->view->viewer());
                      }
                      if ($reply instanceof Wall_Model_Comment) {
                          $comment_format['options']['like'] = $reply->isLike($this->view->viewer());
                      }
                  }
                  $comment_format['options']['delete'] = $canDelete;
                  $comments_format[] = $comment_format;
  ...
  public function like(array $params = array())
  {
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
    $moduleName = $name = $this->_bridge->getModuleName();
    $subject = $params['subject'];

    $info = array();
    if ($moduleName == 'page') {
      $menu = $this->getNavigation('profile', false, 'menu_quick');
      if (empty($menu))
        $menu = @$this->navigations['page_profile'];
      $info['menu'] = $menu;
    }
...
//    $isTablet = false; //Engine_Api::_()->getApi('core', 'apptouch')->isTabletMode();
    ...
    foreach (@$tabs as $order => $tab) {
      if (strpos($tab['attrs']['class'], 'link')) {
        if (Engine_Api::_()->apptouch()->isApp()) {
          unset($tabs[$order]);
          continue;
        }
