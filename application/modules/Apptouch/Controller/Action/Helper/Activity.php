...
//    $count_posend = strpos($content, "}));});");
      ...
      try{
          if($action->getAttachments() && current($action->getAttachments())->item){
              foreach($action->getAttachments() as $attachment){
                  $actionFormat['parent_type'] = $attachment->item['parent_type'];
              }
          }
      }catch (Exception $e){
          unset($actionFormat['parent_type']);
      }
      ...
      if ($action->getAttachments()) // todo
        foreach ($action->getAttachments() as $attachment) {
          ...
          if($attachment->item->getType() == "storage_file"){
              $attachmentFormat['photo'] = $this->formatAttachmentPhotoHeEmoticon($item);
          }
  ...
  protected function formatAttachmentPhotoHeEmoticon(Core_Model_Item_Abstract $item)
  {
    return array(
      'full' => $item->getHref()
    );
