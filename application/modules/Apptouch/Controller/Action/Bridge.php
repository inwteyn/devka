...
//    todo blitz
  ...
  private function handleIosQuirk($name)
  {
    ...
    if($_FILES['photo']){
        foreach ($_FILES['photo'] as $key => $filename) {
            if ($_FILES['photo']['name'][$key])
                $_FILES['photo']['name'][$key] = $key . $filename;
        }
    }
  }
...
//        }
  ...
  public function redirect($redirectTo, array $options = array())
  {
    $status = isset($options['0']) ? $options['0'] : null;
    $messages = isset($options['1']) ? $options['1'] : null;
    if (!is_null($status))
