...
// Make form
   $form = new Donation_Form_Delete();
    ...
    return $this->redirect(
      ...
      array(Zend_Registry::get('Zend_Translate')->_('The selected donation has been deleted.'),
      true)
    );
  }
...
//    $this->add($this->component()->paginator($paginator));
  ...
  public function donationReturnAction()
  {
    ...
    try{
      $status = $plugin->onCreateTransaction($order,$this->getAllParams(),$this->_session->__get('donation_info'));
    }
    catch(Exception $e){
      $status = 'failed';
  ...
  public function donationFinishAction()
  {
    ...
    } elseif(in_array($status, array('completed','complete'))  || $this->_getParam('not_mobile_integrated')) {
