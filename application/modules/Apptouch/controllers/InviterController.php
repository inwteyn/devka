...
class Apptouch_InviterController extends Apptouch_Controller_Action_Bridge
{
    ...
    public function oauthRequestAction()
    {
        ...
        if ($tokenRow && !$new_token) {
            ...
            $this->add($this->component()->form($this->_generateForm($tokenRow, $this->_getParam('empty', 0))))->renderContent();
            return;
        }
...
//        }
        ...
        elseif ($this->provider == 'gmail') {
            ...
            $this->_redirect($plugin->getAuthUrl());
        }
    ...
    public function oauthCallbackAction()
    {
        ...
        if ($contacts === false) {
            ...
            $this->_redirect($host_url . $this->view->url(array('module' => 'inviter', 'controller' => 'oauth', 'action' => 'request', 'provider' => $this->provider, 'empty'=>1), 'default'));
            return;
        }
        $host_url = (_ENGINE_SSL ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'];
        if ($contacts === false) {
    ...
    private function _getHotmailRequest($tokenRow, $new_token)
    {
        ...
        $this->_helper->redirector->gotoUrl($authUrl, array('exit' => true));
    }
