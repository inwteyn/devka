...
//      ->add($this->component()->paginator($paginator))
  ...
  {
    $this->redirect($this->view->url(array('format' => 'json'), 'page_browse'));
    return;
    if (!$this->isAllowedPost)
      return $this->redirect($this->view->url(array('format' => 'json'), 'page_browse'));
  ...
  public function indexDeleteAction()
  {
    ...
    $playlist->delete();
    return $this->redirect($this->view->url(array('module' => 'pagemusic', 'controller' => 'index', 'action' => 'manage', 'page_id' => $page->getIdentity()), 'default'), array(Zend_Registry::get('Zend_Translate')->_('pagemusic_Playlist was deleted.'), true));
  }
