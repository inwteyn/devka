...
//    $this->add($this->component()->paginator($paginator));
  ...
  public function indexContentsAction()
  {
    ...
    if($categoryparent_id > 0)
      $categoryparent = Engine_Api::_()->getItem('ultimatenews_categoryparent',$categoryparent_id);
    $page = $this->_getParam('page', 1);
...
//    $this->add($this->component()->paginator($paginator));
  ...
  public function indexSubscribeAction() {
    ...
    $category -> save();
//    $message = Zend_Registry::get('Zend_Translate') -> _('You have just subscribed this RSS feed successfully.');
    return $this->redirect('refresh');
  }
  ...
  public function indexUnsubscribeAction() {
    ...
    $category -> subscribe = Zend_Json::encode($users);
    $category -> save();
//    $message = Zend_Registry::get('Zend_Translate') -> _('You have just unsubscribed this RSS feed successfully.');
    return $this->redirect('refresh');
  }
