...
// IndexController {
  ...
  public function indexBrowseAction()
  {
    ...
    $this->setFormat('manage')
      ...
      ->add($this->component()->itemList($paginator, 'browseUserData', array('listPaginator' => true,'attrs' => array('class' => $class))))
...
//      ->add($this->component()->paginator($paginator))
  ...
  public function browseUserData(Core_Model_Item_Abstract $item)
  {
    $customize_fields = array(
      ...
      'friendshipButton' => $this->userFriendshipBtn($item)
    );
//    if ($item->status != "")
    ...
    if (Engine_Api::_()->hasModuleBootstrap('payment')) {
      ...
      if (!$subscriptionsTable->check($user)) {
        ...
        return $this->redirect($this->view->url(array('module' => 'payment',
          'controller' => 'subscription', 'action' => 'choose'), 'default', true));
      }
    }
...
//      $session->__unset('mode');
    ...
    $this->redirect($this->view->url(array(), 'user_login'), array(true, Zend_Registry::get('Zend_Translate')->_('You are now logged out.')));
  }
      ...
      if (isset($_SESSION['twitter_token2'], $_SESSION['twitter_secret2'])) {
        ...
        if (!$viewer->getIdentity()) {
          ...
          if (empty($info)) {
            ...
            return $this->redirect($this->view->url(array('action' => 'home'), 'user_general'));
          }
        }
        // Success
        return $this->redirect($this->view->url(array('action' => 'home'), 'user_general'));
...
// Edit Controller {
  ...
  public function editExternalPhotoAction()
  {
    ...
    return $this->redirect('refresh', array(Zend_Registry::get('Zend_Translate')->_('Set as profile photo')));
  }
...
//  ProfileController {
  ...
  public function profileIndexAction()
  {
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
    $this->setPageTitle($subject->getTitle());
...
//      'paginationParam' => 'page',
    ...
    if ($active)
      $this->add($this->component()->customComponent('itemList', array(
          ...
          'listPaginator' => false,
          'items' => $items
      )), 7)//        ->add($this->component()->paginator($paginator), 8)
      ;
    //    } prepare
    return array(
      'showContent' => false,
...
// FriendsController {
  ...
  public function friendsAddAction()
  {
    ...
    if ($viewer->isSelf($user)) {
      return $this->redirect('parentRefresh', array(Zend_Registry::get('Zend_Translate')->_('You cannot befriend yourself.')));
    }

    // check that user is already friends with the member
    if ($user->membership()->isMember($viewer)) {
      return $this->redirect('parentRefresh', array(Zend_Registry::get('Zend_Translate')->_('You are already friends with this member.')));
    }

    // check that user has not blocked the member
    if ($viewer->isBlocked($user)) {
      return $this->redirect('parentRefresh', array(Zend_Registry::get('Zend_Translate')->_('Friendship request was not sent because you blocked this member.')));
    }

    // Make form
    $form = new User_Form_Friends_Add(array('user' => $user));
    ...
    try {
      ...
      return $this->redirect('refresh', array(Zend_Registry::get('Zend_Translate')->_('Your friend request has been sent.'), true));
  ...
  public function friendsCancelAction()
  {
    ...
    // Make form
      $form = new User_Form_Friends_Cancel(array('user' => $user));
    ...
    try {
      ...
      return $this->redirect('refresh', array(Zend_Registry::get('Zend_Translate')->_('Your friend request has been cancelled.'), true));
  ...
  public function friendsIgnoreAction()
  {
    ...
    try {
      ...
      $this->redirect('refresh', array($message, true));
    } catch (Exception $e) {
      $db->rollBack();
...
//      ->add($this->component()->html($this->dom()->new_('h2', array(), $this->view->translate('My Settings'))))
  ...
  public function settingsGeneralAction()
  {
    ...
    // Facebook
    if (false && 'none' != $settings->getSetting('core.facebook.enable', 'none')) {
      $facebookTable = Engine_Api::_()->getDbtable('facebook', 'user');
      $facebook = $facebookTable->getApi();
    ...
    // Twitter
    if (false && 'none' != $settings->getSetting('core.twitter.enable', 'none')) {
      $twitterTable = Engine_Api::_()->getDbtable('twitter', 'user');
      $twitter = $twitterTable->getApi();
