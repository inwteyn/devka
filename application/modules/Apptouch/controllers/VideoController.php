...
//    }
    ...
    $isUploadVideo = $this->getParam('isUploadVideo');
    if (($current_count >= $quota) && !empty($quota)){
//      if($this->getParam('format') == 'json'){
//            return $this->view->status = 'false';
//      }else{
...
//        }
    ...
    if (!$form->isValid($this->getRequest()->getPost()) && $isUploadVideo != '1') {
...
//      $this->view->error = Zend_Registry::get('Zend_Translate')->_('Invalid request method');
    ...
    return $this->redirect(Zend_Controller_Front::getInstance()->getRouter()->assemble(array('action' => 'manage'), 'video_general', true), array(Zend_Registry::get('Zend_Translate')->_('Video has been deleted.'), true));
  }

  public function indexEditAction()
  {
    return $this->redirect($this->view->url(array(), 'video_general', true));
    if (!$this->_helper->requireUser()->isValid()) return;
    $viewer = Engine_Api::_()->user()->getViewer();
...
//    }
    ...
    try
    {
      ...
      $video = Engine_Api::_()->apptouch()->createVideo($params, $_FILES['file'], $values);
