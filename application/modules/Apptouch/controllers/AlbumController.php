...
//  IndexController {
  ...
  public function indexBrowseAction()
  {
    ...
    if ($paginator->getTotalItemCount() > 0) {
      $this->add($this->component()->itemList($paginator, "browseItemData", array('listPaginator' => true, 'attrs' => array('class' => $class))))
...
//        ->add($this->component()->paginator($paginator))
  ...
  public function indexUploadAction()
  {
    ...
    // Get form
    $form = new Apptouch_Form_SeAlbum();
...
// AlbumController {
  ...
  public function albumViewAction()
  {
    ...
    $userElement = $this->dom()->new_('span', array('class' => 'albumOwner'));
    $userElement->append($this->dom()->new_('span', array(), $this->view->translate('By').' '.$album->getOwner()->getTitle()));
    $this->add($this->component()->html($userElement . '<br />'));
...
//    }
    ...
    $this->setPageTitle($album->getTitle());
    $this->renderContent();
  }
  ...
  public function albumDeleteAction()
  {
    ...
    return $this->redirect(
      ...
      array(Zend_Registry::get('Zend_Translate')->_('Album has been deleted.'),
      true)
    );
  }
...
// } PhotoController
  ...
  public function browseItemData(Core_Model_Item_Abstract $item)
  {
   $customize_fields = array(
      'descriptions' => array(
           $this->view->translate(array('%s photo', '%s photos', $item->count()), @$this->view->locale()->toNumber($item->count())),
      ),
    );
