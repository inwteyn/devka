...
// Index Controller {
  ...
  public function indexBrowseAction()
  {
    ...
    $canCreate = false;//Engine_Api::_()->authorization()->isAllowed('event', null, 'create');
    ...
    if(Engine_Api::_()->apptouch()->isTabletMode()){
        ...
        $class = 'touch-tile-view';
    }
    if ($paginator->getTotalItemCount()) {
      $this->add($this->component()->itemList($paginator, "browseItemData", array('listPaginator' => true, 'attrs' => array('class' => $class))))//        ->add($this->component()->paginator($paginator))
      ;
  ...
  public function indexManageAction()
  {
    ...
    if ($paginator->getTotalItemCount()) {
      $this
        ->add($this->component()->itemList($paginator, "manageItemData", array('listPaginator' => true,)))//        ->add($this->component()->paginator($paginator))
      ;
    } elseif ($this->_getParam('search', false)) {
      $this->add($this->component()->tip(
        $this->view->translate('Be the first to %1$screate%2$s one!', '<a href="' . $this->view->url(array('action' => 'create'), 'event_general') . '">', '</a>'),
  ...
  {
    $this->redirect($this->view->url(array(), 'event_general', true));
    return;
...
//        $this->view->paginator =
    ...
    // });
    // Check create
    $this->view->canCreate = false;//Engine_Api::_()->authorization()->isAllowed('event', null, 'create');
    // Render
...
//            ->add($this->component()->paginator($paginator))
  ...
  public function browseItemData(Core_Model_Item_Abstract $item)
  {
    $owner = $item->getOwner();
    $location = null;
    if($item->location) $location = '</span><span><i class="ui-icon-map-marker"></i>' . $item->location . '</span>';
    $customize_fields = array(
      'descriptions' => array(
        $this->view->translate(''),
        $this->view->translate(array('%s guest', '%s guests', $item->membership()->getMemberCount()),
        '<span><i class="ui-icon-users"></i>' . @$this->view->locale()->toNumber($item->membership()->getMemberCount())) . '</span><span><i class="ui-icon-calendar"></i>' . $this->view->locale()->toDateTime($item->starttime) . $location
      ),
      'photo' => $item->getPhotoUrl('thumb.profile'),
      'creation_date' => null,
    );
...
//  Profile Controller {
  ...
  public function profileIndexAction()
  {
    ...
    if (Engine_Api::_()->getApi('core', 'apptouch')->isTabletMode()) {
      ...
      $this->add($this->component()->heEventCover(), 0);
      if (Engine_Api::_()->apptouch()->isApp()) {
        $this->add($this->component()->html('<div style="clear: both;"></div>'), 2);
      }

    } else {
      $this->add($this->component()->heEventCover(), 0);
      if (Engine_Api::_()->apptouch()->isApp()) {
        $this->add($this->component()->html('<div style="clear: both;"></div>'), 3);
      }
    }
      $this->addPageInfo('contentTheme', 'd')
          ->add($this->component()->quickLinks('profile'))
          ->add($this->component()->subjectPhoto(), 3)
          ...
          ->add($this->component()->html($this->attendEventButtons($subject) . ''), 4)
          ->add($this->component()->html($this->widgetProfileInfo($subject) . ''), 5)
          ->add($this->component()->html($this->widgetProfileRsvp($subject) . ''), 6);
    $this->renderContent();
  }

  private function attendEventButtons($subject){
      $viewer = Engine_Api::_()->user()->getViewer();
      $controlgroup = '';
      $membership = $subject->membership();
      $member = $membership->getRow($viewer);

      if ($viewer->getIdentity() && (!$member || $member->resource_approved) && (!$subject->approval || $member)) {
          $url = array(
          ...
          $url['rsvp'] = 0;
          $not_atten = $this->view->htmlLink($url, $this->view->translate('Not Attending'), array(
              'class' => ($member && $member->rsvp == 0) ? 'ui-btn-active' : '',
              'data-role' => 'button',
          ));
          $url['rsvp'] = 1;
          $maybe_atten = $this->view->htmlLink($url, $this->view->translate('Maybe'), array(
              'class' => ($member && $member->rsvp == 1) ? 'ui-btn-active' : '',
              'data-role' => 'button',
          ));
          $url['rsvp'] = 2;
          $attend = $this->view->htmlLink($url, $this->view->translate('Attending'), array(
              'class' => ($member && $member->rsvp == 2) ? 'ui-btn-active' : '',
              'data-role' => 'button',
          ));

          $controlgroup = '<div data-role="controlgroup" data-type="horizontal" class="event_buttons" data-mini="true">' . $attend . $maybe_atten . $not_atten . '</div>';
      }
      return $controlgroup;
  }
...
//  Event Controller {
  ...
  public function eventInviteAction()
  {
    ...
    return $this->redirect('refresh', array(Zend_Registry::get('Zend_Translate')->_('Members invited')));
  }
  ...
  public function eventDeleteAction()
  {
    ...
    return $this->redirect(
      ...
      array($this->view->translate('The selected event has been deleted.'),
      true)
    );
  }
...
//    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      return $this->redirect($subject, array($this->view->translate('Event joined')));
//    } else
//      $this->add($this->component()->form($form))
//        ->renderContent();
  ...
  public function memberRequestAction()
  {
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      return $this->redirect(
        'refresh',
        array(Zend_Registry::get('Zend_Translate')->_('Your invite request has been sent.'),
        true)
      );
    }
  }
  ...
  public function memberCancelAction()
  {
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      return $this->redirect(
        $subject,
        array(Zend_Registry::get('Zend_Translate')->_('Your invite request has been cancelled.'),
        true)
      );
    } else
      $this->add($this->component()->form($form))
  ...
  public function memberLeaveAction()
  {
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      return $this->redirect($subject, array($this->view->translate('Event left')));
  ...
  public function memberAcceptAction()
  {
    ...
    return $this->redirect('refresh', array(Zend_Registry::get('Zend_Translate')->_('Event invite accepted'), true));
  }
  ...
  public function memberRejectAction()
  {
    ...
    return $this->redirect('refresh', array(Zend_Registry::get('Zend_Translate')->_('Event invite rejected'), true));
  }
  ...
  public function memberRemoveAction()
  {
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      return $this->redirect('refresh', array(Zend_Registry::get('Zend_Translate')->_('Event member removed.')));
    } else
      $this->add($this->component()->form($form))
        ->renderContent();
  ...
  public function memberInviteAction()
  {
    ...
    $this->redirect($event, array($this->view->translate('Members invited')));
  }
  ...
  public function memberApproveAction()
  {
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      return $this->redirect('refresh', array(Zend_Registry::get('Zend_Translate')->_('Event request approved')));
    } else
      $this->add($this->component()->form($form))
        ->renderContent();
...
//  Photo Controller {
  ...
  public function photoEditAction()
  {
    ...
    return $this->redirect('refresh', array(Zend_Registry::get('Zend_Translate')->_('Changes saved')));
  }
  ...
  public function photoDeleteAction()
  {
    ...
    return $this->redirect($event, array(Zend_Registry::get('Zend_Translate')->_('Photo deleted')));
  }
...
//  Post Controller {
  ...
  public function postEditAction()
  {
    ...
    // Try to get topic
    return $this->redirect('refresh', array(Zend_Registry::get('Zend_Translate')->_('Your changes have been saved.')));
  ...
  public function postDeleteAction()
  {
    ...
    return $this->redirect($href, array(Zend_Registry::get('Zend_Translate')->_('Post deleted.')));
...
//  } Topic Controller
  ...
  public function topicRenameAction()
  {
    ...
    return $this->redirect('refresh', array(Zend_Registry::get('Zend_Translate')->_('Topic renamed.')));
  }
  ...
  public function topicDeleteAction()
  {
    ...
    return $this->redirect($event, array(Zend_Registry::get('Zend_Translate')->_('Topic deleted.')));
