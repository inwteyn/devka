  ...
  public function buyLevelProcessAction()
  {
    ...
    if (!$this->_checkEnoughCredits($credits)) {
      $this->redirect(Zend_Controller_Front::getInstance()
        ...
        array(Zend_Registry::get('Zend_Translate')->_('CREDIT_not-enough-credit')));
    }
  ...
  private function topUsers($top_users)
  {
    ...
    foreach ($top_users as $item) {
      ...
      $listItem = array(
        ...
        'href' => $user->getHref(),
        'photo' => $photoUrl,
        'counter' => strtoupper( /*$this->view->translate('APPTOUCH_Balance') . ' ' . */
          @$this->view->locale()->toNumber($item->current_credit))
      );
      $items[] = $listItem;
    }
...
//    $this->view->faqs = $faqs;
    ...
    foreach ($actionTypes as $key => $type) {
      if ($key) {
        $is_module = ($type->action_module != $actionTypes[$key - 1]->action_module);
  ...
  public function storeIndexAction()
  {
    ...
    $balance = Engine_Api::_()->getItem('credit_balance', $viewer->getIdentity())->current_credit;
    if ($balance < Engine_Api::_()->store()->getCredits($order->item_amt + $order->tax_amt + $order->shipping_amt)) {
      $tmp['no_credit_lang'] = $this->view->translate('CREDIT_not-enough-credit');
    }
