  ...
  extends Apptouch_Controller_Action_Bridge
{
  ...
  {
    // Can create?
    $canCreate = false;//Engine_Api::_()->authorization()->isAllowed('music_playlist', null, 'create');
...
//              ->add($this->component()->paginator($paginator))
  ...
  {
    $this->redirect($this->view->url(array('format' => 'json'), 'music_general', 1));
    return;
...
//    ));
  ...
  public function playlistDeleteAction()
  {
    ...
    return $this->redirect(Zend_Controller_Front::getInstance()->getRouter()->assemble(array('action' => 'manage'), 'music_general', true), array(Zend_Registry::get('Zend_Translate')->_('The selected playlist has been deleted.'), true));
  }
