...
//                ->add($this->component()->paginator($paginator))
    ...
    public function indexManageAction()
    {
        ...
        $canCreate = false;//$this->_helper->requireAuth()->setAuthParams('blog', null, 'create')->checkRequire();
...
//                ->add($this->component()->paginator($paginator))
    ...
    {
        //return $this->redirect($this->view->url(array('action' => 'index'), 'blog_general', 1));
        if (!$this->_helper->requireUser()->isValid()) return;
    ...
    {
        //return $this->redirect($this->view->url(array('format' => 'json'), 'blog_general', 1));
        if (!$this->_helper->requireUser()->isValid()) return;
    ...
    public function indexDeleteAction()
    {
        ...
        return $this->redirect(
            ...
            array(Zend_Registry::get('Zend_Translate')->_('Your blog entry has been deleted.'),
            true)
        );
    }
...
//        }
    ...
    {
        $this->attrPage('data-role', 'dialog');
                // Must have a viewer
    ...
    public function subscriptionRemoveAction()
    {
        ...
        // Success
      return $this->redirect('refresh', array($this->view->translate('You are no longer subscribed to this member\'s blog.'), true));
    }
    // } Subscription Controller
