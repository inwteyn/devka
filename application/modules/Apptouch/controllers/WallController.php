    ...
    public function linkedinIndexAction()
{
    ...
    try {
        ...
        $request = $this->getRequest();
        $return_params = array('oauth_token' => false);
        $viewer = Engine_Api::_()->user()->getViewer();
        if($request->getParam('code')){
            $redirectUrl = 'https://www.linkedin.com/uas/oauth2/accessToken?grant_type=authorization_code&code='.$request->getParam('code').'&redirect_uri='.$redirect_uri.'&client_id='.$setting->getSetting('wall.service.linkedin.consumerkey').'&client_secret='.$setting->getSetting('wall.service.linkedin.consumersecret');
            $json = file_get_contents($redirectUrl);
            $params = json_decode($json, true);
            $return_params['oauth_token'] = $params['access_token'];
            $return_params['oauth_token_secret'] = $params['expires_in'];
        }


        if ($return_params['oauth_token']) {

            $token_request = new Zend_Oauth_Token_Request();
            $token_request->setParam(Zend_Oauth_Token::TOKEN_PARAM_KEY, $return_params['oauth_token']);
            $token_request->setParam(Zend_Oauth_Token::TOKEN_SECRET_PARAM_KEY, $return_params['oauth_token_secret']);

            $url = 'https://api.linkedin.com/v1/people/~:(id,first-name,last-name)?format=json';
            $profile_info = $this->request($url,$return_params['oauth_token']);
            ...
            $tokenRow->object_name = $object_name;
            $tokenRow->provider = $provider;
            $tokenRow->oauth_token = $return_params['oauth_token'];
            $tokenRow->oauth_token_secret = $return_params['oauth_token_secret'];
            $tokenRow->creation_date = date('Y-m-d H:i:s');
            $tokenRow->save();
            ...
            $session->request_token = $token_request->getToken();
            $session->request_token_secret = $token_request->getTokenSecret();
            $oauth->redirect(array('client_id' => $setting->getSetting('wall.service.linkedin.consumerkey'),'redirect_uri' => $redirect_uri.'/task/share', 'state'=>'DCEeFWf45A53sdfKef424','response_type'=>'code'));
...
CONTENT;
  ...
  public function indexCommentphotoAction()
  {
        ...
        // Get album
        $viewer = Engine_Api::_()->user()->getViewer();
        $enable_all = Engine_Api::_()->getDbTable('modules', 'core')->isModuleEnabled('advalbum');
        if ($enable_all) {
        ...
        try {
            ...
            $photo->save();
            $photo = $photo->setPhoto($_FILES['file']);
            ...
            $this->view->photo_element = '<a href="' . $photo->getHref() . '" class="comment_photo"><img   src="' . $photo->getPhotoUrl('thumb.profile') . '" class="img" /></a>';
      ...
      if ($this->_getParam('do')) {
          ...
          foreach ($files as $key => $file) {
              ...
              try {
                  ...
                  $this->view->status = true;
                  $this->view->message = Zend_Registry::get('Zend_Translate')->_('Photo deleted successfully');
              } catch (Exception $e) {
                  print_die($e->getMessage() . '');
                  throw $e;
              }
