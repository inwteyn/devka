  ...
  extends Apptouch_Controller_Action_Bridge
{
  ...
  public function indexIndexAction()
  {
    ...
    $this->add($this->component()->itemSearch($form))
      ->add($this->component()->itemList($paginator, 'browseProductList', array('listPaginator' => true)))
      ->renderContent();
  }
  ...
  public function indexProductsAction()
  {
    ...
    $this->setFormat('browse');
    $this->add($this->component()->itemSearch($form))
      ->add($this->component()->itemList($paginator, 'browseProductList', array('listPaginator' => true, 'attrs' => array('class' => 'tile-view'))))
      ->renderContent();
  }
  ...
  public function indexStoresAction()
  {
    ...
    $this->setFormat('browse');
    $this->add($this->component()->itemSearch($form))
      ->add($this->component()->itemList($paginator, array('listPaginator' => true)))
  ...
  public function productIndexAction()
  {
    ...
    $html = '<div class="store_info_box">';

    if ($subject->hasStore()) {
      $html .= $this->view->translate('From %s', $this->view->htmlLink($subject->getStore()->getHref(), $subject->getStore()->getTitle(), array())) .', ';
    }
...
//    $html .= $subject->getInfo();
    $htmlTax = '<span class="store_tax">' . $subject->getTaxInfo() . '</span>';
    $htmlPrice = '<div class="storeHtmlPrice">' . $this->view->translate('Price') . ' : ' . $this->view->getPrice($subject).'</div>';
    ...
    if ($allowOrder) {
      if (!$subject->isWished()) {
          $btn .= '<a class="add_to_wishlist" href="' . $this->view->url(array('module' => 'store', 'controller' => 'product', 'action' => 'wish', 'product_id' => $subject->getIdentity(), 'do' => 'add'), 'default', true) . '" data-role="button">' . $this->view->translate('STORE_Add to Wishlist') . '</a>';
      } else {
          $btn .= '<a class="add_to_wishlist" href="' . $this->view->url(array('module' => 'store', 'controller' => 'product', 'action' => 'wish', 'product_id' => $subject->getIdentity(), 'do' => 'remove'), 'default', true) . '" data-role="button">' . $this->view->translate('STORE_In Wishlist') . '</a>';
      }
      if (!$subject->isAddedToCart()) {
        ...
        $btn .= '<a class="store_add_to_cart" href="' . $href . '" data-role="button" data-rel="dialog">' . $this->view->translate('STORE_Add to Cart') . '</a>';
      }
    } else {
    ...
    if($subject->description){
       $htmlDescription ='<div class="store_product_description">' . $subject->description . '</div>';
    }
    ...
    $this->addPageInfo('contentTheme', 'd')
      ...
      ->add($this->component()->html($htmlPrice), 1)
      ->add($this->component()->rate(array('subject' => $this->view->subject())), 2)
      ->add($this->component()->html($htmlTax), 3)
//      ->add($this->component()->like(array('subject' => $this->view->subject())), 2) // Like plugin is not integrated with product profile
      ->add($this->component()->html($html), 4)
      ->add($this->component()->html($htmlDescription), 5)
      ->add($this->component()->tabs(array('ignorePermissions' => 1)), 6);
    $this->renderContent();
  }
...
//        $this->view->state = $state;
  ...
  public function panelPurchasesAction()
  {
    ...
    $token = Engine_Api::_()->apptouch()->getStoreToken();
...
//    $this->view->html = 'Items - ' . $totalCount . ', Total Price - ' . $totalPrice;
  ...
  {
    return $this->redirect($this->view->url(array(), 'store_general', 1));
    /**
     * @var $page Page_Model_Page
     */
    if (null != ($page = Engine_Api::_()->getItem('page', (int)$this->_getParam('page_id', 0)))) {
...
//    !$this->_helper->requireAuth()->setAuthParams($page, null, 'edit')->isValid() ||
  ...
  public function locationsRemoveAction()
  {
    ...
    if (null == $location = $table->findRow($location_id)) {
      return $this->redirect('parentRefresh', array($this->view->translate('STORE_No location found')));
    }
...
//    $this->navigation = $this->getNavigation(array('statistics'), array('page_id' => $page->getIdentity()));
  ...
  public function productLocationsAddAction()
  {
    ...
    return $this->redirect('parentRefresh', array($this->view->translate('STORE_Selected locations have been added successfully')));
  }
  ...
  public function transactionInit()
  {
    ...
    if (!$this->_user || !$this->_user->getIdentity()) {
      ...
      $token = Engine_Api::_()->apptouch()->getStoreToken(true);
    ...
    if ($item->isFree() && $allowFree) {
      ...
      if($item->isAddedToCart()){
          foreach ($items as $cart_item) {
              if($cart_item['product_id'] == $item->getIdentity()){
                  $href = $this->view->url(array('controller' => 'cart', 'action' => 'remove', 'product_id' => $item->getIdentity(), 'item_id' => $cart_item->getIdentity()), 'store_extended', true);
              }
          }
      }else{
...
//        ),
    ...
    if (in_array('prod_profile', $type)) {
      ...
      /* if ($subject->isOwner($viewer) || ($subject->getStore() && $subject->getStore()->isOwner($viewer))) {
         *f (null != ($page = Engine_Api::_()->getItem('page', $subject->page_id))) {
                     $navigation->addPages(array(
                         array(
