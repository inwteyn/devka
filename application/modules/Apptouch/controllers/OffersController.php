...
//      ->add($this->component()->paginator($paginator))
  ...
  public function tabDetails($active = false)
  {
    ...
    if( $active ) {
      ...
      if ($offer->getPrice()) {
        $priceHhtml = '<span class="offer-new-price">' . $this->view->locale()->toCurrency((double)$offer->price_item, $currency) . '</span><span class="offer-new-price-2  ">'
          . $this->view->getOfferPrice($offer) . '</span>';
      } else {
        $priceHhtml = $this->view->locale()->toCurrency((double)$offer->price_item, $currency);
      }
