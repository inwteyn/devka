  ...
  public function indexContactsAction()
  {
    ...
    if(!empty($contacts)){
      ...
      $frm = $this->getSearchForm();
        ...
        unset($allParams['params']['scriptpath']);
        unset($allParams['module']);
        unset($allParams['controller']);
        unset($allParams['action']);
        unset($allParams['rewrite']);
        unset($allParams['REQUEST_TYPE']);
      $frm->setAction(Zend_Controller_Front::getInstance()->getRouter()->assemble(array()) . '?' . http_build_query($allParams));
      $this
    ...
    $this->setPageTitle($this->view->translate('APPTOUCH_Suggest to friends'));
