...
//      ->add($this->component()->paginator($gifts))
  ...
  public function indexManageAction()
  {
    ...
    foreach($paginator as $rs) {
      ...
      $url = $this->view->url(array('action' => $gift->getTypeName(), 'gift_id' => $gift->getIdentity(), 'sender_id' => $rs->subject_id, 'action_name' => 'received'), 'hegift_own', true);
      if( $action_name == 'sent' ) {
        $url = $this->view->url(array('action' => $gift->getTypeName(), 'gift_id' => $gift->getIdentity(), 'reciever_id' => $rs->object_id, 'action_name' => 'sent'), 'hegift_own', true);
      }
...
//      ->add($this->component()->paginator($paginator))
  ...
  public function ownPhotoAction()
  {
    ...
    $action_name = $this->_getParam('action_name');
    $sender_name = Engine_Api::_()->user()->getUser((int)$sender_id);
    $reciever_name = Engine_Api::_()->user()->getUser((int)$reciever_id);
    if (!$this->checkGift($gift, $sender_id, $reciever_id)) {
      ...
      $action = 'To '.$reciever_name;
      if($action_name == 'received') {
          $action = 'From '.$sender_name;
      }
    $html = '<div class="hegift_view_gift">' . $action . '<br>' . $this->view->itemPhoto($gift, "thumb.profile") . '</div>';
  ...
  public function ownAudioAction()
  {
    ...
    $sender_name = Engine_Api::_()->user()->getUser((int)$sender_id);
    $reciever_name = Engine_Api::_()->user()->getUser((int)$reciever_id);
    if (!$this->checkGift($gift, $sender_id)) {
      $this->view->message = 'Invalid data or gift doesn\'t exist';
      return $this->redirect('parentRefresh');
    }
...
HTML;
        ...
        $this
            ->add($this->component()->html($action))
            ->add($this->component()->html($html))
  ...
  public function ownVideoAction()
  {
    ...
    if( !empty($file_id) ) {
      ...
      if( $storage_file ) {
        ...
        $this->add($this->component()->html($action))
