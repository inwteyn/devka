...
//        'owner' => $this->subject($owner)
      ...
      if ($page_pref)
        $std['descriptions'] = array(
            $this->view->translate(array('%s guest', '%s guests', $item->membership()->getMemberCount()),
                '<span><i class="ui-icon-users"></i>' . @$this->view->locale()->toNumber($item->membership()->getMemberCount())) . '</span><span><i class="ui-icon-calendar"></i>' . $this->view->locale()->toDateTime($item->starttime) . $location,
            $this->view->translate('On page ') . $this->view->htmlLink($item->getPage()->getHref(), $item->getPage()->getTitle())
        );
...
//      ->add($this->component()->paginator($paginator))
  ...
  public function indexDeleteAction()
  {
    ...
    return $this->redirect($this->view->url(array('action' => 'mine', 'page_id' => $this->page_id), 'page_event'), array(Zend_Registry::get('Zend_Translate')->_("PAGEEVENT_REMOVE_SUCCESS"), true));
  }
