...
//                $dashboardHeaderMenus['store'] = '1';
        ...
        $isStoreEnabled = Engine_Api::_()->getDbTable('modules', 'hecore')->isModuleEnabled('store');
        $token = ($isStoreEnabled) ? Engine_Api::_()->apptouch()->getStoreToken() : false;
...
//                if($commentedItem->poster_type == 'comment'){
//                    $action_id = $this->_getParam('id');
//                    $action = $this->_helper->api()->getDbtable('actions', 'wall')->getActionById($action_id);
//
//                    Engine_Api::_()->wall()->addLikeReplyComment($commentedItem, $viewer);
//                    // @todo make sure notifications work right
//                    $ch = 0;
//                    if ($commentedItem->poster_id != $viewer->getIdentity()) {
//                        if($commentedItem->poster_type=='comment'){$commentedItem->poster_type = 'user'; $ch=1;}
//                        Engine_Api::_()->getDbtable('notifications', 'activity')
//                            ->addNotification($commentedItem->getPoster(), $viewer, $commentedItem, 'liked', array(
//                                'label' => 'comment'
//                            ));
//                        if($ch == 1) $commentedItem->poster_type = 'comment';
//                    }
//
...
//
//                    }
//                    Engine_Api::_()->getDbtable('statistics', 'core')->increment('apptouch.likes');
...
//      $this->view->error = Zend_Registry::get('Zend_Translate')->_('Invalid request method');
    ...
    public function commentCreateAction()
    {
        ...
        if($isEmoticon && $this->getParam('isSticker')){
            $body = $this->getParam('body');
        }else{
            $body = $form->getValue('body');
            $body = $filter->filter($body);
        }
        if($this->getParam('img_src') && $this->getParam('img_href_attr')){
            $photo = '<br><a href="' . $this->getParam('img_href_attr') . '" class="comment_photo ui-link"><img src="' . $this->getParam('img_src') . '" class="img" /></a>';
            $body.= $photo;
        }
           ...
           $comment_format = array();
               ...
               try {
                   ...
                   if ($this->_getParam('is_edit')) {
                       try {
                           $table->update(array('body' => $body),
                               array(
                                   'comment_id = ?' => $this->_getParam('is_edit'))
                           );
                           $commentSelect = $table->select();
                           $commentSelect->where('comment_id = ?', $this->_getParam('is_edit'));
                           $comment = $table->fetchRow($commentSelect);
                           $comment['poster_type'] = 'user';
                           $comment_format['poster'] = $this->subject($comment->getPoster());
                           $comment_format['id'] = $this->_getParam('is_edit');
                       } catch (Exception $e) {
                           $this->view->status = false;
                           $this->view->message = $e->getMessage();
                           return;
                       }
                   }elseif($this->getParam('is_reply')){
                        try {
                           $comment = $table->createRow();

                           $subjectOwner = Engine_Api::_()->user()->getUser($this->_getParam('to_user'));

                           if ($table instanceof Core_Model_DbTable_Comments) {
                               $comment->setFromArray(
                                   array(
                                       'resource_type' => 'comment',
                                       'resource_id' => $this->_getParam('is_reply'),
                           ...
                           if ($table instanceof Activity_Model_DbTable_Comments) {
...
//                           // Add notification for owner of activity (if user and not viewer)
                           ...
                           $comment_format['resource_id'] = $this->getParam('is_reply');
                           $comment['poster_type'] = 'user';
                           $comment_format['poster'] = $this->subject($comment->getPoster());
                       } catch (Exception $e) {
                           $this->view->status = false;
                   ...
                   $canComment = $viewer->getIdentity() && ($viewer->getIdentity() == $comment->poster_id);
...
//    $this->view->message = Zend_Registry::get('Zend_Translate')->_('Link has been deleted.');
        ...
        return $this->redirect('refresh', array(Zend_Registry::get('Zend_Translate')->_('Link has been deleted.')));
    }
...
// Report Controller {
    ...
    public function reportCreateAction()
    {
        ...
        return $this->redirect($subject, array($this->view->translate('Your report has been submitted.')));
    }
// } Report Controller
...
// } Search Controller
  ...
  public function __call($methodName, $args)
  {
    ...
    // Get page
    $action = substr($methodName, 5, strlen($methodName) - 11);
    // Have to un inflect
    if( is_string($action) ) {
      $actionNormal = strtolower(preg_replace('/([A-Z])/', '-\1', lcfirst($action)));
    ...
    if( null !== $pageObject ) {
      ...
      if( $pageObject->custom && !$pageObject->allowedToView($viewer) ) {
        return $this->redirect($this->view->url(array('action' => 'requireauth', 'controller' => 'error', 'module' => 'core'), 'default', true));
      }
      // Render the page
      $this
        ->setPageTitle($pageObject->title)
        ->add($this->component()->html($this->_helper->content
              ->getContent()
              ->render($pageObject->page_id)))
      ->renderContent();
      return;
    }
