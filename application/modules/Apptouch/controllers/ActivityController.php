  ...
  extends Apptouch_Controller_Action_Bridge
{
  ...
  private function getRemovedList($string = null) {
    if(!$string) {
      return array();
    }
    ...
    foreach($tmp as $row) {
      $tmpRow = explode(':', $row);
      if(is_array($tmpRow) && count($tmpRow) == 2) {
        $removed[$tmpRow[0]] = $tmpRow[1];
      }
    }
  ...
  public function indexPostAction()
  {
    ...
    $product_result = $hecoreApi->checkProduct('apptouch');

    if (isset($product_result['result']) && !$product_result['result']) {
        $this->view->error = Zend_Registry::get('Zend_Translate')->_('Your subscription is expired.');
        return;
    }
...
//      $this->view->error = Zend_Registry::get('Zend_Translate')->_('Not post');
    ...
    try {
      ...
      $title = @$postData['feed_album_name'];
      $description = @$postData['feed_album_description'];

      $isAlbum = (boolean)@$postData['isalbum'];
      $removedPhotos = array();
      if (!$isAlbum) {
        $photo = $this->getPicupFiles('photo');
        $photoName = 'photo';
      } else {
        $removedPhotos = $this->getRemovedList($this->getRequest()->getParam('remove'));
        $photo = $this->getPicupFiles('feed-album-photo');
        $photoName = 'feed-album-photo';
      }
      ...
      if (!empty($photo)) {
        $photo = $photo[0];
        $upload_photo = $photo;
      } elseif (!empty($_FILES[$photoName]) && !empty($_FILES[$photoName]['name'])) {
        $upload_photo = $_FILES[$photoName];
      }
      ...
      if ($upload_photo) {
        ...
        if (Engine_Api::_()->getDbTable('modules', 'core')->isModuleEnabled('album')) {
          ...
          try {
            ...
            if ($isAlbum) {
              $album = $this->newAlbum($title, $description, $table, $type, $viewer, $this->_getParam('privacy'));
            } else {
            ...
            if ($isAlbum) {
              for($i=0;$i<count($upload_photo);$i++) {
                $name = $upload_photo['name'][$i];
                $size = $upload_photo['size'][$i];
                if( !$name || (is_array($removedPhotos) && count($removedPhotos) > 0
                  && isset($removedPhotos[$name]) && $removedPhotos[$name] == $size) ) {
                  continue;
                }
                $tmp = array(
                  'name' => $name,
                  'type' => $upload_photo['type'][$i],
                  'tmp_name' => $upload_photo['tmp_name'][$i],
                  'error' => $upload_photo['error'][$i],
                  'size' => $size
                );

                if($upload_photo['error'][$i] == 0)  {
                  $photo = $attachedPhotos[] = $this->createPhoto($album, $viewer, $tmp, $type, $photoTable);
                }
              }
            } else {
              $photo = $this->createPhoto($album, $viewer, $upload_photo, $type, $photoTable);
            }
          ...
          try {
            ...
            if ($isAlbum) {
              $album = $this->newAlbum($title, $description, $table, $type, $viewer);
            } else {
              $album = $table->getSpecialAlbum($viewer, $type);
            }
            ...
            if (is_array($upload_photo)) {
              $i = 0;
              foreach ($upload_photo as $tmpPhoto) {
                $name = $upload_photo['name'][$i];
                $size = $upload_photo['size'][$i];
                if( !$name || (is_array($removedPhotos) && count($removedPhotos) > 0
                    && isset($removedPhotos[$name]) && $removedPhotos[$name] == $size) ) {
      ...
      if (!$attachment && $viewer->isSelf($subject)) {
        ...
        // Add activity
        if($isAlbum) {
          $action = $actionTable->addActivity($viewer, $album, 'album_photo_new', null, array('count' => count($attachedPhotos))
            , $this->_getParam('privacy'));
        } else {
        ...
        if ($action && $attachment) {
          if($isAlbum) {
            ...
            foreach($attachedPhotos as $item) {
              $api->attachActivity($action, $item, Activity_Model_Action::ATTACH_MULTI);
            }
          } else {
  ...
  public function indexViewAction()
  {
    ...
    $controlGroup = $this->dom()->new_('div', array('data-role' => 'controlgroup', 'class' => 'comment_view_share', 'data-mini' => 'true', 'data-type' => 'horizontal', 'style' => 'text-align: center'));
    $viewer = Engine_Api::_()->user()->getViewer();
    if (!$canShare) {
      ...
      $controlGroup->append($this->dom()->new_('a',
        array(
          ...
          'href' => $this->view->url(array(
              ...
              'action' => 'share',
              'type' => $type,
              'id' => $id), 'default', true)), $this->view->translate('Share')));
...
//        continue;
      if (!$notification->getObject()) continue;
      $itemFormat = array(
...
//            $this->view->requestCount = Engine_Api::_()->getDbtable('notifications', 'activity')->getRequestsPaginator($viewer)->getTotalItemCount();
      ...
      $this->view->notificationCount = $notificationTbl->hasNotifications($viewer);
...
//  } Notification Controller
  ...
  public function indexNativePostAction()
  {
...
//      $this->view->error = Zend_Registry::get('Zend_Translate')->_('Not post');
    ...
    //$body = htmlentities($body, ENT_QUOTES, 'UTF-8');
    $postData['body'] = $body;
    $postData['composer'] = json_decode($postData['composer'], 1);
    $postData['attachment'] = json_decode($postData['attachment'], 1);
    /*if (!$form->isValid($postData)) {
      $this->view->status = false;
      $this->view->error = Zend_Registry::get('Zend_Translate')->_('Invalid data1');
      return;
    ...
    try {
      // Try attachment getting stuff
      $attachment = null;
      $attachmentData = $postData['attachment'];
      ...
      if ($this->isWall) {
        ...
        $composerData = $postData['composer'];
        if (!empty($composerData)) {
  ...
  private function newAlbum($title = null, $description = null, $table, $type, $user, $privacy = 'everyone')
  {
    ...
    $album->title = ($title || strlen($title) > 0) ? $title : $translate->_(ucfirst($type) . ' Photos');
    $album->description = ($description || strlen($description) > 0) ? $description : '';
    ...
    if ($type != 'message') {
      $auth = Engine_Api::_()->authorization()->context;
      $auth->setAllowed($album, $privacy, 'view', true);
      $auth->setAllowed($album, $privacy, 'comment', true);
    }
    return $album;
  }

  private function createPhoto($album, $viewer, $file, $type, $photoTable)
  {
    $photo = $photoTable->createRow();
    $photo->setFromArray(array(
      'owner_type' => 'user',
