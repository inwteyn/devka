...
// Vars for page edit. may be tmp
  ...
  public function indexIndexAction()
  {
    ...
    $this->setFormat('browse')
      ...
      ->add($this->component()->itemList($paginator, "browseItemData", array('listPaginator' => true, 'attrs' => array('class' => $class))))
    ...
    if ($content && $content_id && $isAllowedView) {
      ...
      if($enabledTimeline){
          ...
          if(($timelinePageSettings && $timelinePageSettings != 'choice') || $subject->isTimeline()){
              $this->add($this->component()->timelineCover(), 0)
...
//      $this->attrPage('data-dom-cache', false);
  ...
  public function packageChooseAction()
  {
    ...
    if( !empty($params['is_active']) && $params['is_active'] ) {
      ...
      $db->commit();
      return $this->redirect($this->view->url(array('action'=>'edit', 'page_id' => $page_id), array('page_team', true)));
    } else {
      try {
  ...
  public function packageReturnAction()
  {
    ...
    if( //!$this->_page ||
      ...
      return $this->redirect(array(), array('default', true));
    }
...
//    }
  ...
  {
    $this->_session->unsetAll();
    return $this->redirect($this->view->url(array(), array('page_browse', true)));
  }
 ...
 * -------------------------------------------
 * */
  public function viewPagealbumphoto()
  {
    $album_id = $this->_getParam('album_id', 0);
    if(!$album_id) {
      return;
    }
    $this->viewPagealbum($album_id);
  }
...
//    }
  ...
  public function viewReview($id)
  {
    ...
    foreach ($types as $type) {
      ...
      $html = $html . $this->view->touchReviewRate($type->value);
      $html = $html . '</div>';
    }
    ...
    $this->add($this->component()->quickLinks('gutter'))
      ...
      ->add($this->component()->html('<div class="pagereview-title-hack">' . $review->title . '</div>'))
 ...
 * */
  ...
  {
    $this->redirect($this->view->url(array(), array('page_browse', true)));
    return;
    if (!$this->_helper->requireUser()->isValid()) return;
  ...
  public function browseItemData(Core_Model_Item_Abstract $item)
  {
    ...
    */
    ...
    if(Engine_Api::_()->getDbTable('modules', 'hecore')->isModuleEnabled('rate')){
        $rate = $this->view->apptouchItemRate('page', $item->getIdentity());
    }
    if(Engine_Api::_()->getDbTable('modules', 'hecore')->isModuleEnabled('like')){
        $like = strtolower($item->getLikesCount() == 1 ? $item->getLikesCount() . ' ' . $this->view->translate('Like') : $item->getLikesCount() . ' ' . $this->view->translate('Likes'));
    }
...
//        ->add($this->component()->paginator($paginator), 20)
  ...
  public function tabStore($active = false)
  {
    ...
    if( !in_array('store', $page->getAllowedFeatures()) || Engine_Api::_()->apptouch()->isApp()) {
      return false;
    }
...
/*    $options[] = array(
  ...
  public function manageReviewList(Core_Model_Item_Abstract $item)
  {
    ...
    $customize_fields = array(
      'title' => $item->getTitle(),
      'descriptions' => array(
        '<div class="small_rate_star">' . $this->view->touchReviewRate($item->rating, true) . '</div>'
      ),
      'photo' => $photoUrl,
      'manage' => $options,
