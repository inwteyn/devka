window.addEvent('domready', function(){changeSearch();});
...
function changeSearch() {
    if ($('global_search_field') && (!$('global_search_field').get('data-type') || $('global_search_field').get('data-type') != 'mini')) {
        ...
        var div = '<span id="as_global_default_icon"><i style="display: inline-block;width: 1.25em; text-align: center;" class="hei hei-globe"></i></span> ' +
            '<input type="text" class="width_search" name="query" autocomplete="off" id="global_search_field" size="20" maxlength="100" placeholder="Search" alt="Search">' +
            ...
            '<div style="margin: 0px; position: relative; padding-left: 29px;"><div class="advancedsearch_types_list" id="advancedsearch_global_types_list" style="margin: 0px; overflow: hidden;">' +
...
function featuresAS() {
    ...
    $(document.body).addEvent('click', function (e) {
        var type_list = $('advancedsearch_global_types_list');
    ...
    function searchGlobal() {
        var type_list = $('advancedsearch_global_types_list'),
            query = $('global_search_field').get('value'),
            type = $('as_global_type').get('value');
        if (query != '') {
