    ...
    public function indexAction()
    {
        ...
        if (!$birthdate || empty($birthdate)) {
            if ($subject->getIdentity() != $viewer->getIdentity()) {
                $this->view->hide_profile_fields = true;
            } else {
                $this->view->no_birthdate = true;
                $this->view->no_birthdate_message = $this->view->translate("TIMELINE_Your birthdate was filled automatically. You may change it in Profile Settings.");
                $subject->setBirthdate();
            }
        }
