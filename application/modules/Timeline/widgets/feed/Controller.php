...
//      ));
      ...
      if (Engine_Api::_()->core()->hasSubject()){
        ...
        if(in_array($subject->getType(), $HEPluginsSupportet))
        {
        ...
        if (!isset($config['min_date']) || (strtotime($config['min_date']) < strtotime($birthdate))) {
            ...
            $config['min_date'] = date('Y-m-d', $time);
        }
      ...
      // Lists
        ...
        if($subject->getType()=='page') $this->view->privacy_type = $privacy_type = ($subject) ? $subject->getType() : 'page';
        else $this->view->privacy_type = $privacy_type = ($subject) ? $subject->getType() : 'user';
