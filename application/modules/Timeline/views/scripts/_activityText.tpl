...
foreach ($actions as $action): // (goes to the end of the file)
  ...
  try { // prevents a bad feed item from destroying the entire page
    ...
    <?php else : ?>
      ...
      <div class='feed_item_body feed_item_body_<?php $_item = is_object(current($cur_var))?current($cur_var)->item: null; echo $_item ? strtolower($_item->getModuleName()) . '_' . $_item->getType() : 'none' ?>'>
              ...
              <?php else: ?>
                <?php foreach ($action->getAttachments() as $attachment):
                  ...
                  if($attachment->item->getType() == "storage_file"){
                    ?>
                    <a rev="heemoticon_<?php echo $attachment->item->getIdentity()?>" class="wall_liketips wp_init" href="<?php echo $attachment->item->map()?>"><img class="thumb_profile heemoticon item_nophoto " alt="" src="<?php echo $attachment->item->map()?>"></a>
                    <?php
                    continue;
                  }
        ...
        <div class='comments wall-comments'>
          ...
          <?php if ($canComment):
            ...
            <div id="preview_comment_attach_wall">
              <div class="comment_attach_loading_wall" id="comment_attach_loading_wall<?php echo $this->action->action_id; ?>"></div>
              <div class="comment_attach_preview_image_wall" id="comment_attach_preview_image_wall<?php echo $this->action->action_id;?>"></div>
            </div>
            <script>
