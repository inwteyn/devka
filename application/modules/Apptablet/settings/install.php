...
class Apptablet_Installer extends Engine_Package_Installer_Module
{
  public function onPreInstall()
  {
    ...
    if (version_compare($hecore['version'], '4.2.0p4') < 0) {
    ...
    $apptouch = $db->fetchRow($select);

    if (!$apptouch) {
      $error_message = $translate->_('You should first install New Touch Mobile module.');
      return $this->_error($error_message);
    }
  ...
  private function _patchServerSctipts(Zend_Db_Adapter_Abstract $db)
  {
    try {
//      $db->query("UPDATE `engine4_core_menuitems` SET `module` = 'apptablet' WHERE `name` = 'core_footer_apptablet';");
    } catch (Exception $e) {
    }
  }
