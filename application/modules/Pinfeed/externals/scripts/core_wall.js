...
Wall.injectAbsolute_hash = function (element, container, direction) {
    ...
    var build = function () {
        ...
        try {
            ...
            if (direction) {
                container
                    .setStyle('right', page_width - left - width - 9);
            } else {
                container
                    .setStyle('left', left);
            }
...
Wall.TipFx = new Class({
    ...
    createDom: function () {
        ...
        this.$container = new Element('div', {
            'class': 'wall-tips ' + (this.options.class_var || ''),
            style: 'display:none'
        });
...
Wall.Slideshow = new Class({
    ...
    createDom: function () {
        ...
        this.prev = new Element('a', {
            'class': 'prev navigation wall_blurlink',
            'href': 'javascript:void(0);',
            'style': 'display: none'
        });
        this.next = new Element('a', {
            'class': 'next navigation wall_blurlink',
            'href': 'javascript:void(0);',
            'style': 'display: none'
        });
...
Wall.Feed = new Class({
    ...
    initializeFeed: function () {
        ...
        self.addEvent('share', function (service) {
            ...
            if (s_obj.isEnabled()) {
                ...
                Wall.request(en4.core.baseUrl + 'wall/index/service-share', {
                    'provider': service,
                    'status': 1
                }, function () {
                });
    ...
    initActionPrivacy: function (action) {
        ...
        $privacy.getElements('a').addEvent('click', function () {
            ...
            Wall.request(en4.core.baseUrl + 'wall/index/change-privacy', {
                'action_id': element_info.action_id,
                'privacy': value
            });
    ...
    initAction: function ($container, params) {
        ...
        $container.getElements('.action-like').addEvent('click', function () {
            ...
            Wall.request(en4.core.baseUrl + 'wall/index/like/subject/' + self.options.subject_guid, data, function (obj) {
                if (obj.status) {
                    element.action.set('html', obj.body);
                    self.initAction(element.action);
                    if (self.compose) self.compose.getPlugin('tag').second_activete()
                }
            });
        ...
        $container.getElements('.wall-comment-form').removeEvent('submit').addEvent('submit', function (e) {
            ...
            if (comment_container.get('html').length > 0) {
                ...
                $('select_photo_' + action_element.action_id).setStyle('display', 'block');
            }
            else {
                var img = '';
            }
            values += '&comment_pagination=' + action_element.comment_pagination + '&checkin=' + element.checkin + '&img=' + img + '&pinfeed=1';
        ...
        $container.getElements('.wall-comment-form-reply').removeEvent('submit').addEvent('submit', function (e) {
            ...
            if (comment_container.get('html').length > 0) {
                ...
                var img = comment_container.get('html');
                if ($('select_photo_' + action_element.action_id)) {
                    $('select_photo_' + newid).setStyle('display', 'block');
                }
            }
            else {
                var img = '';
        ...
        $container.getElements('.comment-like').addEvent('click', function () {
            ...
            Wall.request(en4.core.baseUrl + 'wall/index/like/subject/' + self.options.subject_guid, data, function (obj) {
                if (obj.status) {
                    ...
                    if (self.compose) {
                        self.compose.getPlugin('tag').second_activete();
                    }
                }
            });
        ...
        if (!this.getFeed().getElements('.wall-action-item').length) {
            if (!this.getFeed().getElement('.wall-empty-feed')) {
                var element = new Element('li', {
                    'class': 'wall-empty-feed',
                    html: '<div class="tip"><span>' + en4.core.language.translate('WALL_EMPTY_FEED') + '</span></div>'
                });
    ...
    loadList: function (html) {
        ...
        this.$types.getElements('.item').addEvent('click', function () {
            ...
            if (rev.substr(0, 4) == 'type') {
                 ...
                 */
                $('pinfeed_more').set('html', '<span class="pinfeed_loader" style="display: inline-block;  margin-top: -2px;"></span><i class="hei hei-chevron-down"></i>');
                ...
                self.loadFeed(self.params, null, function () {
                    $loader.removeClass('wall_loading');
                    $('pinfeed_more').set('html', en4.core.language.translate('More') + '<i class="hei hei-chevron-down"></i>');
                });
...
Wall.Stream.Abstract = new Class({
    ...
    initComposer: function () {
        ...
        container.getElement('form').addEvent('submit', function (e) {
            ...
            if (!$(this).body.get('value') || !$$('.compose-content')[0].textContent.trim().length) {
                return;
            }
...
Wall.Camera = new Class({
    ...
    createBox: function () {
        ...
        var webcamera_element = (new Element('div', {
            'class': 'embed',
            'id': 'wall_camera_embed'
    ...
    createDom: function () {
        ...
        this.freeze = new Element('a', {
            'href': 'javascript:void(0);',
            'class': 'freeze wall-button wall_blurlink',
            'html': '<span class="wall_icon">&nbsp;</span>' + en4.core.language.translate('WALL_CAMERA_FREEZE')
        });
        this.cancel = new Element('a', {
            'href': 'javascript:void(0);',
            'style': 'display:none',
            'class': 'cancel wall-button wall_blurlink',
            'html': '<span class="wall_icon">&nbsp;</span>' + en4.core.language.translate('WALL_CAMERA_CANCEL')
        });
        this.upload = new Element('a', {
            'href': 'javascript:void(0);',
            'style': 'display:none',
            'class': 'upload wall-button wall_blurlink',
            'html': '<span class="wall_icon">&nbsp;</span>' + en4.core.language.translate('WALL_CAMERA_UPLOAD')
        });
